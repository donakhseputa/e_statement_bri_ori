    ...
    <script>
		...
		function oncheck_norek(){
			if($("#checkbox_norek").attr('checked')==true){
				...
				Loaddiv('div_norek','script_split_jadwal_kirim_antrian.php','flagtrans=<?=$flagtrans?>&act=norek&blth='+blth+'&file='+nama_file);
			}else{
				$("#norek").val('');
				$("#norek").attr('disabled','disabled');
				clear_sample();
		...
		function goto(pr){
			window.location.href = "index_split_jadwal_kirim_antrian.php?pr="+pr;
		}
		...
		function email(){
			...
			if(norek=='' || $("#checkbox_norek").attr('checked')==false){
				...
				Loaddiv('email_address','script_split_jadwal_kirim_antrian.php','flagtrans=<?=$flagtrans?>&act=email&blth='+blth+'&nama_file='+nama_file+'&norek='+norek);
			}
			show_sample();
		}
