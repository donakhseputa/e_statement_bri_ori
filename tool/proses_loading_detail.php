...
<?
	...
	if ($product == 'CO')
	{	
		if ( preg_match("/adhoc/i", $_FILES["loading_file"]["name"])  )
		{
			$co_adhoc = "script_loading_detail_co_adhoc.php";
		}else if ( preg_match("/excel/i", $_FILES["loading_file"]["name"])  )
		{
			$co_adhoc = "script_loading_detail_co_excel.php";
		}else if ( preg_match("/_so_/i", $_FILES["loading_file"]["name"])  )
		{
			$co_adhoc = "script_loading_detail_co_summary_only.php";
		}else{
			$co_adhoc = "script_loading_detail_co.php";
		}
	}
	#die($co_adhoc);
?>
		...
		function upload_detail(val,flag,rad_cust){
			if(flag=='BC'){
				...
				if(rad_cust==2){
					...
					$.post('<?=$co_adhoc?>',{pr:val},function(respon){
						var arr_respon = respon.split('|');
						var pesan = arr_respon[0];
						var jml_record = arr_respon[1];
						var record_not_exists = arr_respon[2];
