...
<?php
	...
	$status_cyber_split = cek_status_cyber_split_ganjil_genap();
	if ( $status_cyber_split == 'on' ) die("STATUS SPLIT CYBER ON!!");
...
SELECT a.antrian_id, a.jadwal_id, a.flagtrans, a.blth, a.kode_kirim, a.email 
	...
	*/
	...
	$arr_kode_kirim_limit[] = 200;
	$arr_kode_kirim_limit[] = 300;//gmail
	$arr_kode_kirim_limit[] = 200;
	//$arr_kode_kirim_limit[] = 0;
	//$arr_kode_kirim_limit[] = 0;
	
	$tanggal_hari_ini = date('d');//die($tanggal_hari_ini);
	//$tanggal_hari_ini = (int)$tanggal_hari_ini;
	if ( $tanggal_hari_ini > 14 && $tanggal_hari_ini < 25 )
	{
		$order_x = " ORDER BY a.prioritas_kirim ASC ,a.antrian_id ASC ";
	}else{
		$order_x = " ORDER BY a.antrian_id ASC ";
	}
	...
	for($nnnn=0;$nnnn<count($arr_kode_kirim);$nnnn++)
	{
		...
		if ($nnnn==2) $flag_OO .= " and a.flagtrans != 'BC' and a.flagtrans != 'BE' ";
		if ($nnnn>0) $sql_sel_1 .= ' UNION ';
		
		
		$sql_sel_1 .="select * from(
								SELECT a.antrian_id, a.jadwal_id, a.flagtrans, a.blth, a.kode_kirim, a.email , a.loading_id
								FROM antrian_email a
									INNER JOIN m_jadwal b
										ON a.jadwal_id = b.jadwal_id
									LEFT JOIN log_approval e
										ON a.log_approval_id = e.log_approval_id
								WHERE b.tgl_jadwal 
								between ( (now() - interval '5 days')::date || ' 00:00:00')::timestamp without time zone  
								--AND now()
								AND  ( (now() + interval '1 days')::date || ' 00:00:00')::timestamp without time zone  
									and b.status = TRUE and e.is_approval = 't' 
									and a.email ~* '^[A-Za-z0-9._%\-+!#$&/=?^|~]+@[A-Za-z0-9.-]+[.][A-Za-z]+$' = 't'
	...
	$blth_balik = substr($arr_blth[$id],-4).substr($arr_blth[$id],0,2);
		...
		$sql_sel_antrian = "SELECT a.*,
									...
									d.nama as nama_x, d.alamat1, d.alamat2, d.alamat3, d.blth, d.flagtrans, d.nama_file, d.password_pdf, d.tipe_kartu, d.ket_produk, d.email AS emailsebenarnya, d.nomor_rekening $add_detail
							FROM antrian_email a
		...
		if ( $nama_x != $detail['nama']	 ) 
			
		//echo $sql_sel_antrian;
		die("Data nama tidak sesuai norek $nomor_rekening");
		...
		if($status_pdf == "Y"){
			...
			if(pg_affected_rows($qry_ins_tr_email)>0){
				if ($rev_tr_email == 'tr_email')///2
				{
					...
					$sql_sel_tr_email_id = "select tr_email_id from tr_email where lower(email) = lower('$email') order by tr_email_id desc limit 1";
				}else{
					//$sql_sel_tr_email_id = "SELECT last_value as tr_email_id FROM ".$rev_tr_email."_tr_email_id_seq";
					$sql_sel_tr_email_id = "select tr_email_id from ".$rev_tr_email." where lower(email) = lower('$email') order by tr_email_id desc limit 1";
				}
