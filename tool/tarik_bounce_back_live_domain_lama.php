...
require_once($www_location."include/rumus_tipe_gagal_rev1.php"); 
...
$con = pg_connect($connection) or die("Could not connect to database!");
	...
	function split_ket_error_delay($ket_error)
	{
			...
			return $yy[1]." - "."Delay Reason: ". substr($n[1], 0, 700);
	...
	function update_gagal( $con, $date_email_callback, $ket_error, $email)
	{
		...
		//$tipe_gagal = get_tipe_gagal_kirim ( $ket_error );
		$tipe_gagal = '';
		
		$rev_ket_error= split_ket_error_gagal($ket_error);
		$rev_mm= substr($rev_ket_error, 0, 550);
	...
	function update_gagal_last( $con, $date_email_callback, $ket_error, $email, $tr_email_id )
	{
		...
		$tipe_gagal = get_tipe_gagal_kirim ( $ket_error , $email );
	...
	function update_read($con, $date_email_callback, $ket_error, $email, $message_id1, $pengirim, $tipe_baca, $info_device='')
	{
		...
		$sql = "UPDATE tr_email
				...
				tgl_read2='$tgl_read2' ,
				body_email_read=E'".addslashes (str_replace ('--','', str_replace("'","",$ket_error)))." (id read)' " ;
				
		if ( !empty( $message_id1) )
		{
			$y1=(int) $message_id1;
			$sql.=" WHERE tr_email_id=".trim($y1)." AND lower(email)=lower('$email') AND email_callback IS NULL AND tgl_read IS NULL";
		}else if ( !empty( $email) ){
			$sql.=" WHERE lower(email)=lower('$email')  AND email_callback IS NULL AND tgl_read IS NULL
					AND 
			...
			$sql.=" WHERE lower(email)=lower('$pengirim')  AND email_callback IS NULL AND tgl_read IS NULL
					AND  
					(cast('$date_email_callback' as timestamp))-(cast( date_email_send as timestamp)) between  '- 5 hours' and '14 days' 
					";
		}else{
		...
		if(@pg_affected_rows($cek) == 0){
			...
			$sql = "UPDATE tr_email
				...
				read_method='$tipe_baca',
				tgl_read2='$tgl_read2' ,
				body_email_read=E'".addslashes (str_replace ('--','', str_replace("'","",$ket_error)))." (email read)' " ;
			$sql.=" WHERE lower(email)=lower('$email')  AND email_callback IS NULL AND tgl_read IS NULL
					AND 
					(cast('$date_email_callback' as timestamp))-(cast( date_email_send as timestamp)) between  '- 5 hours' and '14 days' 
...
//	$imap   = imap_open('{imap.example.com:993/imap/ssl}INBOX', 'foo@example.com', 'pass123', OP_READONLY);
	...
	//$mbox   = imap_search($mbox, 'SINCE "1 July 2024"', SE_UID);
	//$mbox = imap_search($email_inbox, $email_from, $email_pass) or die("can't connect: " . imap_last_error());
	//echo $email_inbox.'_'.$email_from.'__'.$email_pass;die();
	...
	if ( imap_num_msg($mbox) > 500 ) $xx=500;
	...
	for($i = 1 ; $i <= $xx ; $i++)
	{
			...
			)
			{
				...
				update_gagal_last_sk_tr_amazon( $con, $xyz, $rev_ket_error, $imil, $tr_email_id, $header_info->date );
				...
				) 
			{
				...
				*/
				...
				{
					update_gagal_last_sk_tr( $con, $xyz, $rev_ket_error, $imil, $tr_email_id , $header_info->date );
					
					//insert tabel log gagal baru
					$tabelnya_x1 = cek_tabel_log_gagal($tr_email_id);
					insert_tabel_gagal2($con, $header_info->subject, $rev_ket_error_full, $xyz, $blth, $tr_email_id, $loading_id, $flagtrans, $imil, 'f', $status,$tabelnya_x1,$header_info->date );
					...
					)
			{
				...
				*/
				...
				if ($message_id1 > 1000000000000 ) //1.000.000.000.000 (1 triliun)
				{
					...
					//insert tabel log read baru
					$tabelnya_x1 = cek_tabel_log_baca($tr_email_id);
					insert_tabel_read2($con, $tr_email_id, $xyz, $slice, 'outlook', $tabelnya_x1, $header_info->date);
				}else{
				...
				*/
				...
				if ($tr_email_id > 1000000000000 ) //1.000.000.000.000 (1 triliun)
				{
					$cek_delete2 = update_read_sk_tr( $con, $xyz, substr(imap_body($mbox, $i), 0, 1000), strtoupper($abcd[0]), $message_id1, strtoupper($pengirim) ,'img_email', $info_device, $header_info->date);
					
					//insert tabel log read baru
					$tabelnya_x1 = cek_tabel_log_baca($tr_email_id);
					insert_tabel_read2($con, $tr_email_id, $xyz, $slice, 'img', $tabelnya_x1, $header_info->date);
				}else{
					$cek_delete2 = update_read( $con, $xyz, substr(imap_body($mbox, $i), 0, 1000), strtoupper($abcd[0]), $message_id1, strtoupper($pengirim) ,'img_email', $info_device, $header_info->date);
				}
	...
	function insert_tabel_read($con, $tr_email_id, $tgl_kirim, $body_email, $ket)
	{
		$sql_ins = "INSERT INTO read_email (
						tr_email_id2, waktu_read_email, body_email, keterangan
					)VALUES(
	...
	function get_string_between($string, $start, $end)
	{
	 ...
	 function update_gagal_last_sk_tr( $con, $date_email_callback, $ket_error, $email, $tr_email_id , $tgl_read2='')
	{
...
function cek_tabel_tr_email($kode1_m_loading_id)
	...
	function update_read_sk_tr($con, $date_email_callback, $ket_error, $email, $tr_email_id, $pengirim, $tipe_baca, $info_device='' ,$tgl_read2='')
	{
		...
		$sql = "UPDATE $tabel_tr_email
				...
				info_device='$info_device' ,
				tgl_read2='$tgl_read2' ,
				body_email_read=E'".addslashes (str_replace ('--','', str_replace("'","",$ket_error)))." (id read2)' " ;
		...
		/*
		if ( strtolower($email) == 'ezz.chocolate.lg@gmail.com') {
		...
		*/
		...
		if(@pg_affected_rows($cek) == 0){
			...
			$sql = "UPDATE $tabel_tr_email
				...
				read_method='$tipe_baca',
				tgl_read2='$tgl_read2' ,
				body_email_read=E'".addslashes (str_replace ('--','', str_replace("'","",$ket_error)))." (email read2)' " ;
			$sql.=" WHERE lower(email)=lower('$email')  AND email_callback IS NULL AND tgl_read IS NULL
					AND 
					(cast('$date_email_callback' as timestamp))-(cast( date_email_send as timestamp)) between  '- 5 hours' and '14 days' 
	...
	function update_delay_last( $con, $date_email_callback, $ket_error, $email, $tr_email_id )
	{
		...
		$rev_ket_error= split_ket_error_gagal($ket_error);
		$rev_mm= substr($rev_ket_error, 0, 1000);
	...
	function user_agent_parser( $info_device )
	{
	 ...
	 function update_gagal_last_sk_tr_amazon( $con, $date_email_callback, $ket_error, $email, $tr_email_id, $tgl_read2='' )
	{
		$kode1 = $rev_nn= substr($tr_email_id, 0, 1);
        ...
        if ($jml > 0) {
			...
			$sql = "UPDATE $tabel_tr_email
				...
				//$cek = @pg_query($sql);
				$cek = pg_query($sql) or die('GAGAL INSERT 1 AWS: ' . $sql);
			...
			//die($sql);
			//$cek = @pg_query($sql);
			$cek = pg_query($sql) or die('GAGAL INSERT 2 AWS: ' . $sql);
			...
			if ($jml == 0) 
			{
					...
					//die($sql2);
					//$cek2 = @pg_query($sql2);
					$cek = pg_query($sql2) or die('GAGAL INSERT 3 AWS: ' . $sql2);
	...
	function cek_tabel_log_baca($tr_email_id)
	{
		$kode1 = substr($tr_email_id, 0, 1);
		...
		#die($tabel_tr_email);
			...
			$sql_cek_dat = "SELECT * FROM m_loading WHERE m_loading_id::integer = $kode1_m_loading_id AND m_loading_id > 1995 LIMIT 1";
				...
				if($jml == 0){
					...
					$tabeldetail = "_log_baca_";
				}else{
					//return "_log_baca_".trim($row['blth']).'_'.trim($row['flagtrans']);
					$tabeldetail = "_log_baca_".trim($row['blth']).'_'.trim($row['flagtrans']);;
				}
		create_tabel_log_baca($tabeldetail);
		return $tabeldetail;
	}
	
	function cek_tabel_log_gagal($tr_email_id)
	{
		$kode1 = substr($tr_email_id, 0, 1);
		...
		$message_id1 = $kode1_tr_email_id;
			$sql_cek_dat = "SELECT * FROM m_loading WHERE m_loading_id::integer = $kode1_m_loading_id AND m_loading_id > 1995 LIMIT 1";
				...
				if($jml == 0){
					#die("TABEL TR_EMAIL SK TR TIDAK DITEMUKAN $sql_cek_dat");
					$tabeldetail = "_log_gagal_";
				}else{
					$tabeldetail = "_log_gagal_".trim($row['blth']).'_'.trim($row['flagtrans']);;
					
				}
		create_tabel_log_gagal($tabeldetail);
		return $tabeldetail;
	}
	
	
	function create_tabel_log_baca($tabeldetail)
	{
	
	$tabelindex = $tabeldetail."_index";
	$tabeldetail_pl = $tabeldetail."_p_k";
	
	$cek_tabel = "SELECT read_email_id FROM $tabeldetail";
				...
				$buat_tabel = "
						...
						(
						  read_email_id serial NOT NULL,
						  tr_email_id integer,
						  waktu_read_email timestamp without time zone,
						  body_email text,
						  keterangan character varying(50),
						  tr_email_id2 bigint,
						  waktu_insert_data timestamp without time zone,
						  tanggal2 text,
						  CONSTRAINT $tabeldetail_pl PRIMARY KEY (read_email_id )
						)
						WITH (
						  OIDS=FALSE
						);
						ALTER TABLE read_email
						  OWNER TO postgres;
						COMMENT ON TABLE read_email
						  IS 'tabel untuk simpan history read email $tabeldetail';
						";
					...
					$sql_ind = "CREATE INDEX $tabelindex ON $tabeldetail  USING btree (read_email_id,tr_email_id2);";
					$exe_ind = pg_query($sql_ind)or die("ERROR: " . $sql_ind);
				}	
	}
	
	
	function create_tabel_log_gagal($tabeldetail)
	{
	
	$tabelindex = $tabeldetail."_index";
	$tabeldetail_pl = $tabeldetail."_p_k";
	
	$cek_tabel = "SELECT bounce_inbox_id FROM $tabeldetail";
				$exe_tabel = @pg_query($cek_tabel);
				if(!$exe_tabel){
				...
				$buat_tabel = "
						CREATE TABLE $tabeldetail
						(
						  bounce_inbox_id serial NOT NULL,
						  body_bounce text,
						  date_bounce timestamp without time zone,
						  header_bounce character varying(200),
						  flagtrans character varying(2),
						  blth character varying(6),
						  tr_email_id character varying(30),
						  ...
						  sample_status boolean,
						   waktu_insert_data timestamp without time zone,
						  tanggal2 text,
						  CONSTRAINT $tabeldetail_pl PRIMARY KEY (bounce_inbox_id )
						)
						WITH (
						  OIDS=FALSE
						);
						ALTER TABLE bounce_inbox
						  OWNER TO postgres;
						COMMENT ON TABLE bounce_inbox
						  IS 'tabel berisi feedback email';

						";
					$exe_tabel = pg_query($buat_tabel)or die("ERROR: " . $buat_tabel);
					//indexing
					//$sql_ind = "CREATE INDEX $tabelindex ON $tabeldetail  USING btree (blth, flagtrans, nama_file,nomor_rekening);";
					$sql_ind = "CREATE INDEX $tabelindex ON $tabeldetail  USING btree (bounce_inbox_id,tr_email_id);";
					$exe_ind = pg_query($sql_ind)or die("ERROR: " . $sql_ind);
				}	
	}
	
	
	function insert_tabel_read2($con, $tr_email_id, $tgl_kirim, $body_email, $ket, $tabelnya, $tanggal2)
	{
		$sql_ins = "INSERT INTO $tabelnya (
						tr_email_id2, waktu_read_email, body_email, keterangan
						,tanggal2,waktu_insert_data
					)VALUES(
						".$tr_email_id.", '".$tgl_kirim."', '".addslashes($body_email)."'::varchar(600), '$ket'
						,'$tanggal2',now()
					)";
	...
	function insert_tabel_gagal2($con, $header, $body, $tgl_read, $blth, $tr_email_id, $loading_id, $flagtrans, $email_bouncenya, $sample, $keterangan, $tabelnya,$tanggal2)
	{
		//if ( empty( $header ) && empty( $body ) ) return 'db_berhasil';
		//if ( empty( $tgl_read ) ) return 'db_gagal';
		...
		$kode1_m_loading_id = substr($tr_email_id, 8, 5);
		$kode1_m_loading_id = (int)$kode1_m_loading_id;
		$loading_id = $kode1_m_loading_id;
		
		if ( empty( $loading_id ) ) $loading_id=0;
		$sql_insert = " insert into $tabelnya(header_bounce, body_bounce, date_bounce, status, blth, tr_email_id, m_loading_id, flagtrans, email,sample_status
		,tanggal2, waktu_insert_data) 
							values(E'".addslashes ($header)."'::varchar(100), E'".addslashes ($body)."'::text, '$tgl_read', '$keterangan', '$blth', '$tr_email_id','$loading_id','$flagtrans', '".substr($email_bouncenya, 0, 39)."','$sample'
							,'$tanggal2',now()
							)";
