    ...
    <script>
		...
		function cekForm(){
			...
			if(confirm('Untuk menghindari kesalahan loading, tolong dicek kembali!\n\nApakah Anda ingin melanjutkan?')){
				if(product=='BC'){
					$("#form_upload_detail").attr('action','proses_loading_detail_group.php?pr='+pr+'|dat_to_pdf');
				}else if(product=='CO'){
					$("#form_upload_detail").attr('action','proses_loading_detail_group.php?pr='+pr+'|dat_to_pdf');
					//alert ('cek index_loading_detail.php line 41-43');
				}else if(product=='BD'){
					$("#form_upload_detail").attr('action','proses_loading_detail_group.php?pr='+pr+'|dat_to_pdf');
				}else{
		...
		function showRecord(){
			...
			if(product=='BC'){
				...
				$.post("script_loading_detail_bc_group.php",{pr:pr+'|show_record|'+blth},function(respon){
				...
				$.post("script_loading_detail_bc_group.php",{pr:pr+'|show_file_mcustomer|'+blth},function(respon){
					$("#span_file_mcustomer").html(respon);
				});
			}else if(product=='CO'){
				$("#span_record").html("<img src='../images/ajax.gif' align='center' >");
				$.post("script_loading_detail_bc.php",{pr:pr+'|show_record|'+blth},function(respon){
					$("#span_record").html('<input type="text" name="jml_record" id="jml_record" class="textbox_1" disabled="disabled" value="'+respon+'" />');
				});
				$("#span_file_mcustomer").html("<img src='../images/ajax.gif' align='center' >");
				$.post("script_loading_detail_bc.php",{pr:pr+'|show_file_mcustomer|'+blth},function(respon){
					$("#span_file_mcustomer").html(respon);
				});
				//alert ('cek index_loading_detail.php line 69-73');
			}else if(product=='BD'){
				$("#span_record").html("<img src='../images/ajax.gif' align='center' >");
				$.post("script_loading_detail_bc.php",{pr:pr+'|show_record|'+blth},function(respon){
					$("#span_record").html('<input type="text" name="jml_record" id="jml_record" class="textbox_1" disabled="disabled" value="'+respon+'" />');
		...
		function show_det(m_loading_id){
			...
			Loaddiv('div_detail','script_loading_detail_group.php','act=show_detail&m_loading_id='+m_loading_id+'&offset=0&input_search='+input_search);
		...
		function show_blth_log(){
			...
			input_search = input_search.replace(' ','8764346466435364647768799667654537543756');
			Loaddiv('div_detail','script_loading_detail_group.php','act=log_detail&flagtrans='+flagtrans+'&blth='+blth+'&offset=0&input_search='+input_search);
		...
		function del_data(m_loading_id){
			...
			if(confirm(cek)){
				Loaddiv('div_detail','script_loading_detail_group.php','act=del_data&m_loading_id='+m_loading_id);
				Loaddiv('div_detail','script_loading_detail_group.php','act=log_detail&flagtrans='+flagtrans+'&blth='+blth+'&offset=0');
			}
		}
                                        	...
                                        	<td colspan="3" align="left">
                                            	<table>
                                                    <tr>
                                                        ...
                                                        <td height="30">
                                                        	<label>
                                                        	<input type="radio" name="rad_cust" id="rad_cust2" value="2" />
