		...
		while(!feof($open_dat)){
			...
			if(trim($row_dat)!=''){
				...
				if ($regtype == '01')
				{
					...
					$OLS_CURR_BAL 				= $OLS_CURR_BAL.$OLS_CURR_BAL_SIGN; //TOTAL TAGIHAN BARU (UNTUK SELURUH KARTU)
					
					
					$CR_LIMIT 					= substr($row_dat,41,14); #CREDIT LIMIT GABUNGAN
					$CR_LIMIT_SIGN 				= substr($row_dat,55,1); #CREDIT LIMIT GABUNGAN SIGN
					$CR_LIMIT 					= $CR_LIMIT.$CR_LIMIT_SIGN; #CREDIT LIMIT GABUNGAN
					...
					$OLS_TOT_AMOUNT_DUE 		= $OLS_TOT_AMOUNT_DUE.$OLS_TOT_AMOUNT_DUE_SIGN; //TOTAL MINUM PAYMENT (UNTUK SELURUH KARTU)
					
					
					$CR_CASH_LINE 				= substr($row_dat,71,14);
					$CR_CASH_LINE_SIGN 			= substr($row_dat,85,1);
					$CR_CASH_LINE 				= $CR_CASH_LINE.$CR_CASH_LINE_SIGN;
					
					
					$CR_AVAIL_CASH 				= substr($row_dat,86,14); //SISA / BATAS PENGAMBILAN TUNAI
					$CR_AVAIL_CASH_SIGN 		= substr($row_dat,100,1);
					$CR_AVAIL_CASH 				= $CR_AVAIL_CASH.$CR_AVAIL_CASH_SIGN; //SISA / BATAS PENGAMBILAN TUNAI
					
					
					$CR_AVAIL_CREDIT 			= substr($row_dat,101,14); #SISA KREDIT LIMIT GABUNGAN
					$CR_AVAIL_CREDIT_SIGN		= substr($row_dat,115,1); 
					$CR_AVAIL_CREDIT 			= $CR_AVAIL_CREDIT.$CR_AVAIL_CREDIT_SIGN; #SISA KREDIT LIMIT GABUNGAN
					...
					$new_balance_summary = $new_balance_summary . $new_balance_sign_summary;
				...
				if($regtype == '02'){
					...
					if (substr($no_card_dat,0,1) == "4") //VISA
					{
						...
						if ( substr($no_card_dat,0,8) == "46874001" || substr($no_card_dat,0,8) == "46874002") //Infinite - 20160419 Bowo
						{
							...
							$url_img = "../images/layout_bri_penggabungan_1.jpg";
							$card_name="visa";
							$ket_produk="VISA";
							#$yimage = 3.5;//kz
							#$y = 24;//kz
					...
					elseif (substr($no_card_dat,0,1) == "5") //MASTER
					{
						...
						if (  substr($no_card_dat,0,8) == "51885633" ) //BNP GOLD - 20180109 kz //AGRO
						{
							$url_img = "../images/layout_bri_penggabungan_1_agro.jpg";
						}else{
							$url_img = "../images/layout_bri_penggabungan_1.jpg";
						}
					...
					elseif (substr($no_card_dat,0,1) == "3") //JCB
					{
						...
						$url_img = "../images/layout_bri_penggabungan_1_jcb.jpg";
						$card_name="jcb";
						$ket_produk="JCB";
						#$yimage = 3.5;
						$yimage = 0;
...
$kolektibilitas = NULL;
									...
									if ( $regtype == '02' )
									{
										...
										if ( $flag_summary )
										{
											...
											$ket2 = add_page2_sumamry_jpg($pdf, $url_pdf_jpg_summary, $arr_summary, $jml_hlm , $barcode, $posko, $kuri, $nourut, $kolektibilitas, $file); 
...
$dr_cr_indicator = NULL;
	...
	function add_page2_sumamry_jpg($pdf, $url_pdf='', $arr_summary, $jml_hlm , $barcode, $posko, $kurir, $nourut, $kolektibilitas, $file )
	{
		...
		if ( !empty($url_pdf) )
		{
			...
			$var_summary_jeda = 22;
			$x_kanan_ujung=100;
			$x_kolom['atas']				= array(24, 65, 103, 142);
			$x_kolom['atas_x']				= array('n', 'n', 'y', 'y');
			$x_kolom['bawah']				= array(24, 65, 103, 142);
			$x_kolom['bawah_x']				= array('n', 'n', 'y', 'y');
			
			$x_kolom['footer']				= array(23, 66-20, $x_kanan_ujung-(2*$var_summary_jeda)-30, $x_kanan_ujung-(1*$var_summary_jeda)-30, $x_kanan_ujung-30);
									...
									$pdf->Cell(67+1.3,4,$arr_isi[$x_tambahan],0,0,'R');
										...
										if ( $x_kolom['footer_x'][$x_tambahan]=='y' )
										{
											...
											if ( $x_kolom['footer_x'][$x_tambahan]=='nn')
											{
												...
												$pdf->MultiCell(25,3,$arr_isi[$x_tambahan],0,'L');
											}else{
												$pdf->Cell(67,4,$arr_isi[$x_tambahan],0,0,'L');
											}
