...
<?
	...
	function dat_to_pdf($flagtrans,$menu_id,$blth,$file,$url_directory,$pdftk_loc,$file_mcustomer){
			...
			if(trim($row_dat)!=''){
				...
				if($prevcardnb<>$no_card_dat){
					if($rectype=='01'){ //hanya akan masuk saat nosebelum tidak sama dengan nocurrent, dan rectype 01
						...
						$cdata['bunga_belanja']	= number_format(((int)substr($row_dat,458,5)/100),2,'.',',').'%';
						$cdata['bunga_tunai']	= number_format(((int)substr($row_dat,464,5)/100),2,'.',',').'%';
		...
		create_p01($blth,$file,$total_hlm_cetak,$customer_cetak);
		create_p02($blth,$file,$total_hlm_cetak,$customer_cetak);
		create_q01($blth,$file,$total_hlm_cetak,$customer_cetak);
		create_rec($blth,$file,$total_hlm_cetak,$customer_cetak);
		create_sip($blth,$file,$total_hlm_cetak,$customer_cetak,$daterecieve);
	...
	function doPDF($cdata,$header,$password_pdf,$blth,$file,$detail,$flag_case){
		...
		for($i=0;$i<$nheader;$i++){
			...
			$pdf->SetX($x_rincian_detail);
			$pdf->Cell(35, 5, $header['cdnumb'][$i], 0, 0, 'L');
			$pdf->Cell(35, 5, $header['cdname'][$i], 0, 1, 'L');
			...
			$pdf->SetFont('Sandy','',9);
			$pdf->SetX($x_rincian_detail + 50);
			$pdf->Cell(115, 5, "Tagihan Bulan Ini", 0, 0, 'L');
	...
	function create_p01($blth,$file,$total_hlm_cetak,$customer_cetak){
	...
	function create_p02($blth,$file,$total_hlm_cetak,$customer_cetak){
		$pdf = new MYPDF('L', PDF_UNIT, array(210, 297), true, 'UTF-8', false);
		$pdf->SetCreator(PDF_CREATOR);
		$pdf->SetAuthor(PDF_AUTHOR);
		$pdf->SetDefaultMonospacedFont(PDF_FONT_MONOSPACED);
	...
	function create_q01($blth,$file,$total_hlm_cetak,$customer_cetak){
		$pdf = new MYPDF('L', PDF_UNIT, array(210, 297), true, 'UTF-8', false);
		$pdf->SetCreator(PDF_CREATOR);
		$pdf->SetAuthor(PDF_AUTHOR);
		$pdf->SetDefaultMonospacedFont(PDF_FONT_MONOSPACED);
	...
	function create_rec($blth,$file,$total_hlm_cetak,$customer_cetak){
		$pdf = new MYPDF('L', PDF_UNIT, array(210, 297), true, 'UTF-8', false);
		$pdf->SetCreator(PDF_CREATOR);
		$pdf->SetAuthor(PDF_AUTHOR);
		$pdf->SetDefaultMonospacedFont(PDF_FONT_MONOSPACED);
	...
	function create_sip($blth,$file,$total_hlm_cetak,$customer_cetak,$daterecieve){
		$dm = chr(9);
