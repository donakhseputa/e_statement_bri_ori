...
<?
	...
	function insert_tmp_excel_bri_corp1($tmp_excel_bri_corp1){
		/*
		$sql = "
		TRUNCATE TABLE tmp_excel_bri_corp1;
		TRUNCATE TABLE tmp_excel_bri_corp2;
		TRUNCATE TABLE tmp_excel_bri_corp3;
		INSERT INTO tmp_excel_bri_corp1(nama, tagihan_sebelumnya, pembayaran, 
            pembelanjaan, pengambilan_tunai, tagihan_baru, sisa_kredit, bunga_pembelanjaan, 
            bunga_pengambilan_tunai, tanggal_cetak, jatuh_tempo_pembayaran, 
            batas_kredit, batas_pengambilan_tunai,nama_file,account_corp_number)
    VALUES ".$tmp_excel_bri_corp1.";
		
		";
		$qry = pg_query($sql) or die('ERROR insert insert_tmp_excel_bri_corp1: '.$sql);
		*/
	...
	function insert_tmp_excel_bri_corp2($tmp_excel_bri_corp2){
		$tmp_excel_bri_corp2 = str_replace('id_tmp_excel_bri_corp1','(SELECT id_tmp_excel_bri_corp FROM tmp_excel_bri_corp1 order by id_tmp_excel_bri_corp desc limit 1)',$tmp_excel_bri_corp2);
		$sql = "
		INSERT INTO tmp_excel_bri_corp2( cardholder_number, cardholder_name, tagihan_sebelumnya, 
            pembayaran_kredit, pembelanjaan_debit, pengambilan_tunai, finance_charge, 
            tagihan_baru_idr, id_tmp_excel_bri_corp1)
    VALUES ".$tmp_excel_bri_corp2.";
		
		";
		$qry = pg_query($sql) or die('ERROR insert insert_tmp_excel_bri_corp2: '.$sql);
		return true;
	}
	
	
	function insert_tmp_excel_bri_corp3($tmp_excel_bri_corp3){
		$tmp_excel_bri_corp3 = str_replace('id_tmp_excel_bri_corp1','(SELECT id_tmp_excel_bri_corp FROM tmp_excel_bri_corp1 order by id_tmp_excel_bri_corp desc limit 1)',$tmp_excel_bri_corp3);
		$sql = "
		INSERT INTO tmp_excel_bri_corp3(
            nama, pembelanjaan, pengambilan_tunai, 
            tagihan_baru, sisa_kredit, nomor_kartu, tanggal_cetak, jatuh_tempo_pembayaran, 
            batas_kredit, batas_pengambilan_tunai, tanggal_transaksi, tanggal_pembukuan, 
            keterangan, transaksi_valas, nilai_tukar, jumlah_idr, id_tmp_excel_bri_corp1)
    VALUES ".$tmp_excel_bri_corp3.";
		
		";
		$qry = pg_query($sql) or die('ERROR insert insert_tmp_excel_bri_corp3: '.$sql);
		return true;
	}
	
	
	function buat_excel_rincian_transaksi($flagtrans,$blth,$file,$file2,$output_excel_nama)
	{
	
		////////////////////////////////////////////////////////////
		//menghapus data convert excel yang tidak sesuai listing
		$sql = "
		DELETE FROM tmp_excel_bri_corp1
			WHERE account_corp_number NOT IN (SELECT nomor_rekening FROM _list_convert_excel);
		";
		$qry = pg_query($sql) or die('ERROR delete tmp_excel_bri_corp1: '.$sql);
		////////////////////////////////////////////////////////////
		...
		#$objPHPExcel->getActiveSheet()->setCellValue('A'.$n, $file);
		$objPHPExcel->getActiveSheet()->setTitle($file);
			
		$x1 = array();
		$x1[0]='nama';$x1[1]='tagihan_sebelumnya';$x1[2]='pembayaran';$x1[3]='pembelanjaan';$x1[4]='pengambilan_tunai';$x1[5]='tagihan_baru';
		$x1[6]='sisa_kredit';$x1[7]='bunga_pembelanjaan';$x1[8]='bunga_pengambilan_tunai';$x1[9]='tanggal_cetak';$x1[10]='jatuh_tempo_pembayaran';$x1[11]='batas_kredit';
		$x1[12]='batas_pengambilan_tunai';
		
		$x2[0]='A';$x2[1]='B';$x2[2]='C';$x2[3]='D';$x2[4]='E';$x2[5]='F';
		$x2[6]='G';$x2[7]='H';$x2[8]='I';$x2[9]='J';$x2[10]='K';$x2[11]='L';
		$x2[12]='M';$x2[13]='N';$x2[14]='O';$x2[15]='P';$x2[16]='Q';$x2[17]='R';
		
		
		$x3[0]='cardholder_number';$x3[1]='cardholder_name';$x3[2]='tagihan_sebelumnya';$x3[3]='pembayaran_kredit';$x3[4]='pembelanjaan_debit';$x3[5]='pengambilan_tunai';
		$x3[6]='finance_charge';$x3[7]='tagihan_baru_idr';
		
		$sql = "SELECT * FROM tmp_excel_bri_corp1 ORDER BY id_tmp_excel_bri_corp DESC LIMIT 1";
        $qry = pg_query($sql) or die('ERROR create excel: '.$sql);
		while($row=pg_fetch_array($qry)){
			
			
			for($i=0;$i<=12;$i++){
				$objPHPExcel->getActiveSheet()->getColumnDimension($x2[$i])->setWidth(20);
				$objPHPExcel->getActiveSheet()->setCellValue($x2[$i].'1', $x1[$i] );
				$objPHPExcel->getActiveSheet()->setCellValue($x2[$i].'2', $row[$x1[$i] ]   );
			}
		
			$objPHPExcel->getActiveSheet()->setCellValue('A'.'3', 'Rincian Transaksi' );
				
			$sql2 = "SELECT * FROM tmp_excel_bri_corp2 
			WHERE id_tmp_excel_bri_corp1 ='".$row['id_tmp_excel_bri_corp']."'
			ORDER BY id_tmp_excel_bri_corp ASC";
			$qry2 = pg_query($sql2) or die('ERROR create excel: '.$sql2);
			$c0 = 0;
			$c1 = 5;
			while($row2=pg_fetch_array($qry2)){
				...
				for($i=0;$i<=7;$i++){
					if($c0 == 0)
					{
						$objPHPExcel->getActiveSheet()->setCellValue($x2[$i].'4', $x3[$i] );
					}
					
					$objPHPExcel->getActiveSheet()->setCellValue($x2[$i].$c1, $row2[$x3[$i] ]   );
			...
			$c1 = $c1+2;
			$objPHPExcel->getActiveSheet()->setCellValue('A'.$c1, 'Detail Transaksi' );
			
			
			
			$x4[0]='nama';$x4[1]='pembelanjaan';$x4[2]='pengambilan_tunai';$x4[3]='tagihan_baru';$x4[4]='sisa_kredit';$x4[5]='nomor_kartu';
			$x4[6]='tanggal_cetak';$x4[7]='jatuh_tempo_pembayaran';$x4[8]='batas_kredit';
			$x4[9]='batas_pengambilan_tunai';$x4[10]='tanggal_transaksi';$x4[11]='tanggal_pembukuan';
			$x4[12]='keterangan';$x4[13]='transaksi_valas';$x4[14]='nilai_tukar';$x4[15]='jumlah_idr';
		
		
			$sql3 = "SELECT * FROM tmp_excel_bri_corp3 
			WHERE id_tmp_excel_bri_corp1 ='".$row['id_tmp_excel_bri_corp']."'
			ORDER BY id_tmp_excel_bri_corp ASC";
			$qry3 = pg_query($sql3) or die('ERROR create excel: '.$sql3);
			$c0 = 0;
			$c1 = $c1+1;
			$c2 = $c1+1;
			while($row3=pg_fetch_array($qry3)){
				...
				for($i=0;$i<=15;$i++){
					if($c0 == 0)
					{
						$objPHPExcel->getActiveSheet()->setCellValue($x2[$i].$c1, $x4[$i] );
					}
					$objPHPExcel->getActiveSheet()->setCellValue($x2[$i].$c2, $row3[$x4[$i] ]   );
		...
		//header('Content-Disposition: attachment;filename="rincian_transaksi_'.$file.'_'.$xyz1.'.xls"');
		//header('Cache-Control: max-age=0');
		$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');
		//$objWriter->save('php://output');
		$name = $blth_file_pdf.'rincian_transaksi_'.$output_excel_nama.'_'.$xyz1.'.xls';
		$objWriter->save($name);
	}
	...
	function dat_to_pdf($flagtrans,$menu_id,$blth,$file,$url_directory,$pdftk_loc,$file_mcustomer){
		...
		while($dt_kurir_c	= pg_fetch_object($pg_kurir_c))
		{
				...
				$input_folder_merge = $url_directory.'\\pdf\\'.$flagtrans.'\\'.$blth.'\\'.$file_merge.'\\cetak_clean\\'.$kurir_c.'\\*';
				$output_folder_merge = $url_directory.'\\pdf\\'.$flagtrans.'\\'.$blth.'\\'.$file_merge.'\\produksi\\'.$file_merge.'_'.$kurir_c.'_cetak.pdf';
		...
		#hapus_data_sesusai_listing_excel($m_loading_id,$tabeldetail);//tidak digunakan karena mengacu master customer
	...
	function hapus_data_sesusai_listing_excel($m_loading_id,$tabeldetail){
	
		$sql_upd_m_load = "DELETE FROM $tabeldetail
							WHERE m_loading_id::integer = $m_loading_id
							AND nomor_rekening NOT IN (SELECT nomor_rekening FROM _list_convert_excel)";
		$qry_upd_m_load = pg_query($sql_upd_m_load) or die('ERROR update hapus_data_sesusai_listing_excelm_loading1: '.$sql_upd_m_load);
		
		$sql_upd_m_load = "UPDATE m_loading
							SET
								total_halaman = (SELECT sum(jml_hlm) FROM $tabeldetail where m_loading_id::integer = $m_loading_id),
								total_customer = (SELECT count(*) FROM $tabeldetail where m_loading_id::integer = $m_loading_id),
								total_halaman_cetak = 0,
								total_customer_cetak = 0
							WHERE m_loading_id = $m_loading_id";
		$qry_upd_m_load = pg_query($sql_upd_m_load) or die('ERROR update hapus_data_sesusai_listing_excelm_loading2: '.$sql_upd_m_load);
	}
	...
	function doPDF($cdata,$header,$password_pdf,$blth,$file,$detail,$flag_case,$flagtrans,$barcode, $dt_kurir1){
		...
		$pdf->SetX($x_biodata);$pdf->MultiCell(50,5,$cdata['custname2'],0,'L',0,1);$tmp_excel_bri_corp1.="(E'".addslashes($cdata['custname2']);$output_excel_nama.=str_replace(' ','_',$cdata['custname2']);
		$pdf->SetX($x_biodata);$pdf->MultiCell(50,5,$cdata['addr1'],0,'L',0,1);$tmp_excel_bri_corp1.="(".addslashes($cdata['addr1']);
		$pdf->SetX($x_biodata);$pdf->MultiCell(50,5,$cdata['addr2'],0,'L',0,1);$tmp_excel_bri_corp1.=addslashes($cdata['addr2']);
		$pdf->SetX($x_biodata);$pdf->MultiCell(50,5,$cdata['addr3'],0,'L',0,1);$tmp_excel_bri_corp1.=addslashes($cdata['addr3']);
		$pdf->SetX($x_biodata);$pdf->MultiCell(50,5,$cdata['city'].' '.$cdata['zipcd'],0,'L',0,1);$tmp_excel_bri_corp1.=addslashes($cdata['city'])." ".addslashes($cdata['zipcd']).")',";
		...
		$pdf->SetFont('Sandy','');
		$pdf->SetX($x_kotak2);
		$pdf->Cell(35, 4, $cdata['total_tagihan_prev'], 0, 0, 'L');$tmp_excel_bri_corp1.="'".$cdata['total_tagihan_prev']."',";
		$pdf->Cell(35, 4, $cdata['total_pembayaran'], 0, 0, 'L');$tmp_excel_bri_corp1.="'".$cdata['total_pembayaran']."',";
		$pdf->Cell(35, 4, $cdata['total_pembelanjaan'], 0, 0, 'L');$tmp_excel_bri_corp1.="'".$cdata['total_pembelanjaan']."',";
		$pdf->Cell(35, 4, $cdata['total_ambil_tunai'], 0, 1, 'L');$tmp_excel_bri_corp1.="'".$cdata['total_ambil_tunai']."',";
		...
		$pdf->Cell(35, 8, $cdata['total_tagihan_next'], 0, 0, 'C');$tmp_excel_bri_corp1.="'".$cdata['total_tagihan_next']."',";
		...
		$pdf->Cell(35, 8, $cdata['total_kredit_sisa'], 0, 0, 'C');$tmp_excel_bri_corp1.="'".$cdata['total_kredit_sisa']."',";
		...
		$pdf->Cell(35, 8, $cdata['bunga_belanja'], 0, 0, 'C');$tmp_excel_bri_corp1.="'".$cdata['bunga_belanja']."',";
		...
		$pdf->Cell(35, 8, $cdata['bunga_tunai'], 0, 0, 'C');$tmp_excel_bri_corp1.="'".$cdata['bunga_tunai']."',";
		...
		$pdf->SetFont('Sandy','');
		$pdf->SetX($x_kotak);
		$pdf->Cell(40, 5, $cdata['dtcetak'], 0, 0, 'L');$tmp_excel_bri_corp1.="'".$cdata['dtcetak']."',";
		$pdf->Cell(55, 5, $cdata['dttempo'], 0, 0, 'L');$tmp_excel_bri_corp1.="'".$cdata['dttempo']."',";
		$pdf->Cell(40, 5, $cdata['total_kredit_limit'], 0, 0, 'L');$tmp_excel_bri_corp1.="'".$cdata['total_kredit_limit']."',";
		$pdf->Cell(55, 5, $cdata['total_batas_tunai'], 0, 1, 'L');$tmp_excel_bri_corp1.="'".$cdata['total_batas_tunai']."','".$file."','".$cdata['no_card_dat']."')";
		...
		for($i=0;$i<$nheader;$i++){
			...
			$tmp_excel_bri_corp2.="('".$header['cdnumb'][$i]."',";
			$tmp_excel_bri_corp2.="E'".addslashes($cdname)."',";
			$tmp_excel_bri_corp2.="'".$header['tagihan_prev'][$i]."',";
			$tmp_excel_bri_corp2.="'".$header['pembayaran'][$i]."',";
			$tmp_excel_bri_corp2.="'".$header['pembelanjaan'][$i]."',";
			$tmp_excel_bri_corp2.="'".$header['ambil_tunai'][$i]."',";
			$tmp_excel_bri_corp2.="'".$header['fin_charge'][$i]."',";
			$tmp_excel_bri_corp2.="'".$header['tagihan_next'][$i]."',id_tmp_excel_bri_corp1),";
		}
		$tbldetmst.='
				<tr>
					<td>&nbsp;</td>
		...
		$tmp_excel_bri_corp2.="('".''."',";
			$tmp_excel_bri_corp2.="'".'TOTAL'."',";
			$tmp_excel_bri_corp2.="'".$cdata['total_tagihan_prev']."',";
			$tmp_excel_bri_corp2.="'".$cdata['total_pembayaran']."',";
			$tmp_excel_bri_corp2.="'".$cdata['total_pembelanjaan']."',";
			$tmp_excel_bri_corp2.="'".$cdata['total_ambil_tunai']."',";
			$tmp_excel_bri_corp2.="'".$cdata['total_fin_charge']."',";
			$tmp_excel_bri_corp2.="'".$cdata['total_tagihan_next']."',id_tmp_excel_bri_corp1)";
			
			insert_tmp_excel_bri_corp1($tmp_excel_bri_corp1);
			insert_tmp_excel_bri_corp2($tmp_excel_bri_corp2);
		...
		for($i=0;$i<$nheader;$i++){
			...
			$pdf->SetFont('Sandy','',9);
			$pdf->SetXY($x_biodata, $y_biodata);
			$pdf->SetX($x_biodata);$pdf->MultiCell(50, 5, $cdata['custname2'], 0, 'L', 0, 1);$tmp_excel_bri_corp3.="(E'".addslashes(trim($header['cdname'][$i])).' - '.addslashes($cdata['custname2'])."',";
			...
			$pdf->SetFont('Sandy','');
			$pdf->SetX($x_kotak2);
			$pdf->Cell(35, 4, $header['pembelanjaan'][$i], 0, 0, 'L');$tmp_excel_bri_corp3.="'".$header['pembelanjaan'][$i]."',";
			$pdf->Cell(35, 4, $header['ambil_tunai'][$i], 0, 0, 'L');$tmp_excel_bri_corp3.="'".$header['ambil_tunai'][$i]."',";
			$pdf->Cell(35, 4, $header['tagihan_next'][$i], 0, 0, 'L');$tmp_excel_bri_corp3.="'".$header['tagihan_next'][$i]."',";
			$pdf->Cell(35, 4, $header['kredit_sisa'][$i], 0, 1, 'L');$tmp_excel_bri_corp3.="'".$header['kredit_sisa'][$i]."',";
			...
			$pdf->SetFont('Sandy','');
			$pdf->SetX($x_kotak);
			$pdf->Cell(35, 5, $header['cdnumb'][$i], 0, 0, 'L');$tmp_excel_bri_corp3.="'".$header['cdnumb'][$i]."',";
			$pdf->Cell(30, 5, $cdata['dtcetak'], 0, 0, 'L');$tmp_excel_bri_corp3.="'".$cdata['dtcetak']."',";
			$pdf->Cell(45, 5, $cdata['dttempo'], 0, 0, 'L');$tmp_excel_bri_corp3.="'".$cdata['dttempo']."',";
			$pdf->Cell(35, 5, $header['kredit_limit'][$i], 0, 0, 'L');$tmp_excel_bri_corp3.="'".$header['kredit_limit'][$i]."',";
			$pdf->Cell(45, 5, $header['batas_tunai'][$i], 0, 1, 'L');$tmp_excel_bri_corp3.="'".$header['batas_tunai'][$i]."',";
			
			
			$tmp_excel_bri_corp4 = $tmp_excel_bri_corp3;
			...
			$tmp_excel_bri_corp3.="'','','".'Tagihan Bulan Lalu'."','','','".$header['tagihan_prev'][$i]."',id_tmp_excel_bri_corp1)";
			insert_tmp_excel_bri_corp3($tmp_excel_bri_corp3);
			...
			for($x=0;$x<$ndetail;$x++){
				...
				$pdf->Cell(60, 5, $detail['trans'][$i][$x], 0, 0, 'L');
				$pdf->Cell(7, 5, $detail['forexcurr'][$i][$x], 0, 0, 'L');
				$pdf->Cell(23, 5, $detail['forexnominal'][$i][$x], 0, 0, 'R');
				...
				$tmp_excel_bri_corp3 = $tmp_excel_bri_corp4;
				$tmp_excel_bri_corp3 .="'".$detail['dttrans'][$i][$x]."',";
				$tmp_excel_bri_corp3 .="E'".addslashes($detail['dtbuku'][$i][$x])."',";
				$tmp_excel_bri_corp3 .="E'".addslashes($detail['trans'][$i][$x]).addslashes($detail['forexcurr'][$i][$x])."',";
				$tmp_excel_bri_corp3 .="'".$detail['forexnominal'][$i][$x]."',";
				$tmp_excel_bri_corp3 .="'".$detail['kursnominal'][$i][$x]."',";
				$tmp_excel_bri_corp3 .="'".$detail['jmltrans'][$i][$x].$cr."',";
				$tmp_excel_bri_corp3 .= "id_tmp_excel_bri_corp1)";
				insert_tmp_excel_bri_corp3($tmp_excel_bri_corp3);
			}
			...
			$tmp_excel_bri_corp3 = $tmp_excel_bri_corp4;
			$tmp_excel_bri_corp3.="'','','".'Total Interest & Service Charge'."','','','".$header['fin_charge'][$i]."',id_tmp_excel_bri_corp1)";
			insert_tmp_excel_bri_corp3($tmp_excel_bri_corp3);
			...
			$tmp_excel_bri_corp3 = $tmp_excel_bri_corp4;
			$tmp_excel_bri_corp3.="'','','".'Tagihan Bulan Ini'."','','','".$header['tagihan_next'][$i]."',id_tmp_excel_bri_corp1)";
			insert_tmp_excel_bri_corp3($tmp_excel_bri_corp3);
		...
		if(file_exists($cetak_output)){
			...
			/////////// membuat excel rincian transaksi sesuai listing
				$sql_cek_dat = "SELECT * FROM _list_convert_excel WHERE nomor_rekening like '%".$cdata['no_card_dat']."%'";
				$qry_cek_dat = pg_query($sql_cek_dat) or die('ERROR cek m_loading: '.$sql_cek_dat);
				$jml = pg_num_rows($qry_cek_dat);
				if($jml > 0){
					$yyyy1 = 1;
		...
		if ( preg_match("/X/i",  $pdf_name ) || preg_match("/x/i", $pdf_name ) )
		{
			...
			if( $yyyy1 == 1 ){
				...
				copy($cetak_output,  $blth_file_pdf.'cetak_'.$pdf_name.'_'.$output_excel_nama.'.pdf');
			}
		...
		if($split_pdf == "on"){
			if( $yyyy1 == 1 ){
				copy_insert_password_split_no_pass($blth_file_cetak2, $blth_file_pdf, $file_copy, $password_pdf, $pass_admin_pdf);
			}
		...
		if($flag_case == "Y"){
			if( $yyyy1 == 1 ){
				copy_insert_password($blth_file_cetak2, $blth_file_pdf, $file_copy, $password_pdf, $pass_admin_pdf);
			}
		...
		if($dt_kurir1 == 'FICC')
		{
			#gen_waybill_ficc($blth_file_cetak3, $blth, $header, $cdata, $detail, $barcode);
				...
				/////////// membuat excel rincian transaksi sesuai listing
				//$sql_cek_dat = "SELECT * FROM _list_convert_excel WHERE nomor_rekening like '%".$cdata['no_card_dat']."%'";
				$sql_cek_dat = "SELECT * FROM _list_convert_excel";
				$qry_cek_dat = pg_query($sql_cek_dat) or die('ERROR cek m_loading: '.$sql_cek_dat);
				$jml = pg_num_rows($qry_cek_dat);
				if($jml > 0){
					while($row_gwgwgw=pg_fetch_array($qry_cek_dat)){
						buat_excel_rincian_transaksi($flagtrans,$blth,$file,$file,$output_excel_nama);
					}
	...
	function insert_detail($m_loading_id,$cdata,
		...
		$sql_ins_detail = "INSERT INTO $tabeldetail(
								...
								E'".addslashes($cdata['custname1'])."',E'".addslashes($cdata['addr1'])."',E'".addslashes($cdata['addr2'])."',
								E'".addslashes($cdata['addr3'])."',E'".addslashes($cdata['city'])."','".$cdata['zipcd']."',
								'$flagtrans','$blth','$nama_file',
	...
	function copy_insert_password_split_no_pass($source_location, $destination_location, $pdf_name, $password_pdf, $pass_admin_pdf){
