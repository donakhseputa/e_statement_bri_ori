...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <script>
		function show_blth_rec(){
			...
			Loaddiv('span_blth_record','script_lihat_antrian_sk_manual.php','act=show_blth_rec&flagtrans='+flagtrans+'&blth='+blth);
		}
		...
		function show_nama_file_rec(){
			...
			Loaddiv('span_nama_file_rec','script_lihat_antrian_sk_manual.php','act=show_nama_file_rec&flagtrans='+flagtrans+'&blth='+blth+'&nama_file='+nama_file);
		}
		
		function show_nama_file(){
			var blth = $("#blth").val();
			var flagtrans = $("#flagtrans").val();
			Loaddiv('div_nama_file','script_lihat_antrian_sk_manual.php','act=show_nama_file&flagtrans='+flagtrans+'&blth='+blth);
		}
		...
		function show_antrian(){
			...
			if(jml_record == ""){
				...
				$("#div_filter").hide(500);
				$("#div_show").show(500);
				Loaddiv('div_show','script_lihat_antrian_sk_manual.php','act=show_antrian&menu_id='+menu_id+'&blth='+blth+'&flagtrans='+flagtrans+'&nama_file='+nama_file+'&tipe_jadwal='+tipe_jadwal+'&jml_record='+jml_record+'&offset=0');
			}
		}
		...
		function delete_all_antrian(){
			...
			if(list_antrian!=''){
				...
				if(cek){
					$("#div_show").html('<img src=../images/ajaxbig.gif>');
					$.post('script_lihat_antrian_sk_manual.php?act=delete_all_antrian&list='+list_antrian,'',function(respon_kirim){
						Loaddiv('div_show','script_lihat_antrian_sk_manual.php','act=show_antrian&menu_id='+menu_id+'&blth='+blth+'&flagtrans='+flagtrans+'&nama_file='+nama_file+'&tipe_jadwal='+tipe_jadwal+'&jml_record='+jml_record+'&offset=0');
					});
				}
			}else{
		...
		function delete_antrian(antrian_id){
			...
			if(cek){
				Loaddiv('div_show','script_lihat_antrian_sk_manual.php','act=delete_antrian&antrian_id='+antrian_id);
				show_antrian();
			}
		}
		...
		function view_template(val,vul,vel){
			...
			var flagtrans = vul;
			var blth = vel;
			$.post('script_lihat_antrian_sk_manual.php?act=view_template&antrian_id='+antrian_id+'&flagtrans='+flagtrans+'&blth='+blth,$("#form_template").serialize(),function(respon){
				$.fancybox(respon);
			});
		}
		...
		function check_qua(log_approval_id){
			...
			$.post('script_lihat_antrian_sk_manual.php?act=open_form_check&log_approval_id='+log_approval_id,$("#form_template").serialize(),function(respon){
				$.fancybox(respon);
			});
		}
		...
		function approve(log_approval_id){
			...
			if(cek){
				Loaddiv('div_show','script_lihat_antrian_sk_manual.php','act=approve_antrian&log_approval_id='+log_approval_id);
				show_antrian();
			}
		}
		...
		function unapprove(log_approval_id){
			...
			if(cek){
				Loaddiv('div_show','script_lihat_antrian_sk_manual.php','act=unapprove_antrian&log_approval_id='+log_approval_id);
				show_antrian();
			}
		}
		...
		function reschedule(log_approval_id){
			var log_approval_id = log_approval_id;
			var flagtrans = $("#flagtrans").val();
			$.post('script_lihat_antrian_sk_manual.php?act=open_form_reschedule&flagtrans='+flagtrans+'&log_approval_id='+log_approval_id,$("#form_template").serialize(),function(respon){
				$.fancybox(respon);
			});
		}
...
<script>
	...
	<?php if($hit_arr_pr>1){ 
	?>
	var blth_home = "<?php echo $blth_home?>";
	Loaddiv('div_blth','script_lihat_antrian_sk_manual.php','act=blth&flagtrans='+flagtrans+'&blth_home='+blth_home);
	$("#checkbox_nama_file").removeAttr('disabled');
	$("#checkbox_nama_file").attr("checked","checked");
	Loaddiv('span_nama_file_rec','script_lihat_antrian_sk_manual.php','act=show_nama_file_rec&flagtrans='+flagtrans+'&blth='+blth_home+'&nama_file=<?php echo $row_ml['loading_file'];?>');
	Loaddiv('span_blth_record','script_lihat_antrian_sk_manual.php','act=show_blth_rec&flagtrans='+flagtrans+'&blth='+blth_home);
	Loaddiv('div_nama_file','script_lihat_antrian_sk_manual.php','act=show_nama_file&flagtrans='+flagtrans+'&blth='+blth_home+'&mloading_home='+<?php echo $mloading_home;?>);
	Loaddiv('div_show','script_lihat_antrian_sk_manual.php','act=show_antrian&menu_id='+<?php echo $menu_id;?>+'&blth='+blth_home+'&flagtrans='+flagtrans+'&nama_file=<?php echo $row_ml['loading_file'];?>&tipe_jadwal=1&jml_record=50&offset=0');
	
	<?php } else { ?>
	Loaddiv('div_blth','script_lihat_antrian_sk_manual.php','act=blth&flagtrans='+flagtrans);
	<?php } ?>
</script>
<?
	pg_close($con);
