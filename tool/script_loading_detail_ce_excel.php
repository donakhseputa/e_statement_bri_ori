...
/** LOADING FILE DENGAN MENGGUNAKAN NOMOR REKENING (DISAMAKAN DGN TABEL m_customer) **/
	...
	//$GLOBALS['dir_root'] = 'E:/_CONVERT_BRI_CETAK_EMATERAI/';
	$GLOBALS['dir_root'] = 'C:/app/htdocs/_CONVERT_BRI_CETAK_EMATERAI/';
	...
	switch($act){
		case 'folder_loading':
			...
			break;
		case 'folder_loading2':
				folder_loading2($blth,$flagtrans,$par);
			break;
		case 'show_record':
	...
	function cycle_loading($blth,$flagtrans,$par)
	{
        ...
        $html .='<option value="'.''.'" disabled selected>'.'--- Apakah file sudah sesuai? ---'.'</option>';
        $html .='<option value="'.'ya'.'" >'.'YA SESUAI'.'</option>';
		$html .='</select>'; 
		  /*
	...
	function folder_loading($blth,$flagtrans,$par){
		...
		$row['folder'] = $GLOBALS['dir_root'].'PDF_C_DAN_CE_EMATERAI_FROM_BRI/';
		$nfile=0;
		if($row){
			$folder=$row['folder'];
			if(is_dir($folder)){
				if ($dh = opendir($folder)) {
					echo "<table border='0' style='border: 1px solid;width:700px;'>
					<tr valign='top' align='left'>
						<td style='font-family:century gothic;font-size:10pt;' colspan='3'>
							<b>DIREKTORI PDF HASIL EMATERAI DARI BRI (CETAK DAN CETAK ESTATEMENT)</b>
							<input type='hidden' id='hidden_dir_1' name='hidden_dir_1' value='".$folder."'>
						</td>
					</tr>";
					echo "<tr valign='top' align='left'><td style='font-family:century gothic;font-size:10pt;'>";
					echo "
					Alamat Direktori</td><td>:</td><td style='font-family:century gothic;font-size:10pt;font-weight:bold;'><i>".$folder."</i></td><tr valign='top'><td align='left' style='font-family:century gothic;font-size:10pt;'>File(s)**</td><td>:</td> ";
					$filesnm="";
	...
	function folder_loading2($blth,$flagtrans,$par){
		
		
		$row['folder'] = $GLOBALS['dir_root'].'PDF_PRODUKSI/';
		$nfile=0;
		if($row){
			$folder=$row['folder'];
			if(is_dir($folder)){
				if ($dh = opendir($folder)) {
					...
					<tr valign='top' align='left'>
						<td style='font-family:century gothic;font-size:10pt;' colspan='3'>
							<b>DIREKTORI PDF HASIL PRODUKSI</b>
							<input type='hidden' id='hidden_dir_2' name='hidden_dir_2' value='".$folder."'>
						</td>
					</tr>";
					echo "<tr valign='top' align='left'><td style='font-family:century gothic;font-size:10pt;'>";
					echo "
					...
					echo "<td><textarea id='isi_file2' name='isi_file2' cols='50' rows='30' readonly class='textarea_loading' style='height:60px;'>".$filesnm."</textarea><br/>
					TOTAL FILE : ($nfile)<br>
					<b><i><font color='brown'>**File yang tertera harus file berbentuk (<b>*.pdf</b>) <br><b>DENGAN PENAMAAN YANG SAMA MASING MASING NOMOR CIF</b>  yaitu:
					<br>- PMC_MTR_DAT.PDF
					<br>- KJN_MTR_DAT.PDF
