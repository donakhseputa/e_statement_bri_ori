...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <script>
        function cekFile(){
        	//var a= $("#loading_file").val().substr(-15).toLowerCase();
        	//alert ( a.substr(0,8).toLowerCase() );
            if(($("#loading_file")!='')){
				if ($("#loading_file").val().substr(-5).toLowerCase()=='.txt')
				{
					$(".button").show(500);
				}
        ...
        function cekFileCustomer(){
            if($("#loading_file_customer")!=''){
				if($("#loading_file_customer").val().substr(-4).toLowerCase()=='.csv'){
					...
					alert("FILE MASTER CUSTOMER HARUS BERTIPE *.CSV!");
		...
		function cekForm(){
			...
			if(confirm('Untuk menghindari kesalahan loading, tolong dicek kembali!\n\nApakah Anda ingin melanjutkan?')){
				if(product=='BE' ){
					$("#form_upload_detail").attr('action','proses_loading_detail_temp.php?pr='+pr+'|dat_to_pdf');
				}else{
					alert('PRODUK XX BELUM TERDAFTAR, HUBUNGI APPDEV');
					return false;
				}
				return true;
			}else{
		...
		function showRecord(){
			...
			if(product=='BE')
			{
				...
				$.post("script_loading_detail.php?act=show_record",{pr:pr+'|'+blth},function(respon)
					{
                            ...
                            <td>
                                <form method="post" enctype="multipart/form-data" id="form_upload_detail" name="form_upload_detail" onsubmit="return cekForm()">
                                    ...
                                    <table width="80%" border="0" cellpadding="2" cellspacing="2" align="center">
                                        ...
                                        <tr>
                                            <td width="20%"></td>
                                            <td colspan="2" style="font-size:11px;"><i>Silahkan masukkan file bertipe <b>*.csv</i><br/>Contoh Format:</b> <a target="_blank" href='../tmp/CONTOH_PASSWORD.csv'>klik disini</a></td>
                                        </tr>-->
                                        ...
                                        <tr>
                                            <td height="30" colspan="3" align="center" valign="middle">Silakan masukkan file *.txt<br>contoh nama file = samira_*.txt</td>
                                        </tr>
                                        <tr>
                                    ...
                                    <tr>
                                        ...
                                        <td align="right" height="50%">&nbsp;
                                        	...
                                        	<span id="span_search_det" style="display:none">
                                                <input type="text" class="textbox_3" id="input_search_det" name="input_search_det" onkeypress="cek(event,2)" />&nbsp;<a onclick="show_det2()"><img class="vtip" title="search" src="../images/icons/find_data.png" /></a>                                           </span>
                                        </td>
                                    </tr>
                                </table>
                            </td>
