...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <script>
		...
		function kirim(){
			var cek = cekForm();
			if(cek==true){
				//alert($("#form_antrian").serialize());
				//pr=BC%7C3&blth=022012&checkbox_nama_file=on&nama_file=ESTAT_20120119_test.02a.DAT&checkbox_multi_attach=on&text_sample_email=&combo_template=5&status=0
				//return false;
				...
				$.post('script_kirim_antrian.php?act=kirim',$("#form_antrian").serialize(),function(respon){
					//alert('res: '+respon);
					...
					alert(jumlah+' record telah dikirim ke antrian\n'+'Daftar nomor rekening yang sudah ada di antrian:\n'+exists);
					...
					//$("#div_button").html('<input type="buttn" id="button_selesai" name="button_selesai" class="button" value="SELESAI" onclick="goto(\''+flagtrans+'|'+menuid+'\')" />');
					$("#div_button").html('<input type="button" id="button_kirim" name="button_kirim" class="button" value="KIRIM KE ANTRIAN" onclick="kirim();" />');
					//var arr_respon = respon;
					//$("#div_respon").html(arr_respon);
				});
			}
		}
		...
		function oncheck_file(){
			if($("#checkbox_nama_file").attr('checked')==true){
				...
				var pr = $("#pr").val();
				var blth = $("#blth").val();
				Loaddiv('div_file_loading','script_kirim_antrian.php','act=nama_file&blth='+blth+'&flagtrans=<?=$flagtrans?>');
			}else{
				//alert('oncheck_file2');
				$("#nama_file").val('');
		...
		function oncheck_norek(){
			if($("#checkbox_norek").attr('checked')==true){
				...
				var arr_pr = pr.split('|');
				//alert('A');
				//alert($("#checkbox_multi_attach").attr('checked'));
				var multich = $("#checkbox_multi_attach").attr('checked');
				$("#text_sample_email").val('');
				Loaddiv('div_norek','script_kirim_antrian.php','flagtrans=<?=$flagtrans?>&act=norek&blth='+blth+'&file='+nama_file+'&multich='+multich);
			}else{
				$("#norek").val('');
				$("#norek").attr('disabled','disabled');
		...
		function goto(pr){
			window.location.href = "index_kirim_antrian.php?pr="+pr;
		}
		...
		function attach_file(){
			...
			Loaddiv('div_attach','script_kirim_antrian.php','flagtrans=<?=$flagtrans?>&act=attach_file&blth='+blth);
			$("#div_attach").show(500);
		}
		...
		function email(){
			...
			if(norek=='' || $("#checkbox_norek").attr('checked')==false){
				...
				Loaddiv('email_address','script_kirim_antrian.php','flagtrans=<?=$flagtrans?>&act=email&blth='+blth+'&nama_file='+nama_file+'&norek='+norek+'&multich='+multich);
				$("#checkbox_multi_attach").attr('disabled','disabled');
				$("#checkbox_multi_attach").removeAttr('checked');
			}
			show_sample();
		...
		function func_sample(){
			if($("#check_sample").attr('checked')==false){
				...
				Loaddiv('email_sample','script_kirim_antrian.php','act=sample');
			}
		}
		...
		function oncheck_multi(){
			...
			$("#norek").val('');
			$("#norek").attr('disabled','disabled');
			//if($("#checkbox_multi_attach").attr('checked')==false){}
		}
    </script>
    <body>
...
<script>
	Loaddiv('div_blth','script_kirim_antrian.php','act=blth&flagtrans=<?=$flagtrans?>');
	Loaddiv('div_template','script_kirim_antrian.php','act=template_email&flagtrans=<?=$flagtrans?>');
</script>
<?
	pg_close($con);
?>