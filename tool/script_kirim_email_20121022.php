	...
	function kirim_email($client){
		...
		for($id=0;$id<count($arr_antrian_id);$id++){
			...
			$sql_sel_antrian = "SELECT a.*,
										...
										d.nama, d.alamat1, d.alamat2, d.alamat3, d.blth, d.flagtrans, d.nama_file, d.nomor_rekening, d.password_pdf
			...
			$detail['nomor_rekening']	= $row_sel_antrian['nomor_rekening'];
			$detail['password_pdf']		= $row_sel_antrian['password_pdf'];
			...
			$mail->Mailer   = "smtp";
			//if(trim($email_reply)!='') 
			$mail->AddReplyTo($email_reply, 'No Reply');//$from_name);
			...
			$sql_ins_tr_email = "INSERT INTO tr_email(
									...
									TRUE, now(), 1,
									$loading_id, ".$arr_antrian_id[$id].", $template_id, '".addslashes(trim($listnme,'#'))."'
								)";
			...
			if(pg_affected_rows($qry_ins_tr_email)>0){
				...
				if(isset($tr_email_id)){
					...
					//$img_mail = 'image email: <img src="http://202.51.252.148/e_statement_bri/emailConfirm.php?pr='.$client.'|'.$flagtrans.'|'.$tr_email_id.'|'.$email.'" border="0" height="1" width="1">';
					//$img_mail = 'img: <img src="http://202.51.252.148/e_statement_bri/images/emailConfirm.php?pr='.$client.'|'.$flagtrans.'|'.$tr_email_id.'|'.$email.'" border="1">';
					//$img_mail = 'img: <img src="http://www.indointernal.com/e_statement_bri/images/emailConfirm.php?email='.$email.'" border="1">';
					//$img_mail = 'img: <img src="http://202.51.252.148/e_statement_bri/images/logo_bri.jpg" border="1">';
					$mail->MsgHTML("<html><body>".$hid_val.$isi_email."</body></html>");
					$mail->IsHTML(true);
					
					if($mail->Send()){
						$sql_ins_antrian_his = "
							INSERT INTO antrian_email_history(
						...
						if(trim($senderr)!='') $senderr.=", ";
						$senderr.=$email." ".$mail->ErrorInfo;
						//echo $mail->ErrorInfo;						
					}
				}
			}
