...
<?
	...
	function kirim_ke_antrian(){
		...
		//pr=BC%7C3&blth=022012&checkbox_nama_file=on&nama_file=ESTAT_20120119_test.02a.DAT
		//&checkbox_multi_attach=on&all_email=app_dev%40indointernal.com&text_sample_email=&combo_template=5
		//&status=0&check_attach=on&attach_file1%5B%5D=&attach_file2%5B%5D=29&attach_file2%5B%5D=31&attach_file2%5B%5D=34
		$pr = $_POST['pr'];
		$blth = $_POST['blth'];
			...
			for($i=0;$i<count($nomor_rekening);$i++){
				...
				for($k=0;$k<count($arr_email);$k++){
					...
					if(pg_num_rows($qry_sel_antrian)==0){
						//insert antrian_email 120227
						//disini akan diubah insert untuk antrian_email_attach_pdf
