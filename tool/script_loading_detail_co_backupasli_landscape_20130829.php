...
<?
	...
	class MYPDF extends TCPDF {
		function Header() {
			...
			$bMargin = 30;//$this->getBreakMargin();
	...
	function doPDF($cdata,$header,$password_pdf,$blth,$file,$detail,$flag_case){
		...
		$pdf->SetXY($x_teks_master + 152, $y_teks_master+2);
		$pesan = 'Nasabah Yth. Sehubungan makin maraknya penipuan dengan mengatasnamakan BRI, kami informasikan bahwa pengumuman pemenang undian berhadiah BRI hanya disampaikan melalui media massa Nasional & Website resmi BRI www.bri.co.id serta akun twitter @promo_BRI. Untuk info lebih lanjut hubungi Call BRI 14017.';
		$pdf->SetFont('','',6.5);
		#$pdf->MultiCell($w=0, $h=0, $txt, $border=1, $align='L', $fill=0, $ln=1, $x='', $y='', $reseth=true, $stretch=0, $ishtml=false, $autopadding=true, $maxh=0);		
		$pdf->MultiCell(70,50,$pesan,0,'C',0,0,$x='',$y='',true,0,true,false,50, 'M');
		$pdf->SetFont('','',11);
		$pdf->SetY($y_teks_master+5);
		...
		for($i=0;$i<$nheader;$i++){
			...
			//hitung perkiraan total tinggi dari setiap CH
			$ndetail=count($detail['dttrans'][$i]);
			$baris_detail = $ndetail + 7;
			$y_baris_detail = ($baris_detail * 5);
			if($ndetail <=36){
				if(($pdf->GetY() > (210 - 30 - $y_baris_detail)) && ($cdata['spcust']==0) && ($i>0)){
					$pdf->AddPage();
					$pdf->SetFont('','',11);
					$pdf->MultiCell(0,5,'CORPORATE CARD DETAILS',0,'C',0,1);
					$pdf->SetFont('','',10);
