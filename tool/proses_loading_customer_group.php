...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <script>
		function upload_customer(val,flag){
			if(flag=='BC'){
				$.post("script_loading_customer_bc.php",{pr:val},function(respon){
					...
					var pesan = arr_respon[4];
					
					//jml_record record_exists waktu flagtrans pesan
					
					$("#div_message").html("-- "+ pesan + "--<br /><br /> Jumlah Record = " + jml_record + " record --<br /><br />record yang sudah pernah di-loading:<br />" + record_exists + "<br /><br />-- Total waktu impor data = " + menit + " menit " + detik + " detik --");
				...
				$.post("script_loading_customer_bd.php",{pr:val},function(respon){
					var arr_respon = respon.split('|');
					var jml_record = arr_respon[0];
					var record_exists = arr_respon[1];
					var waktu = arr_respon[2];
...
<script>
	<?
		if($error==0){
	?>
			upload_customer('<?=$pr.'|'.$blth.'|'.$file?>','<?=$product?>');
	<?
		}
		//echo "$product";
	?>
