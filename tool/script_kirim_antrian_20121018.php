...
<?
	...
	function kirim_ke_antrian(){
		...
		if(isset($_POST['checkbox_multi_attach'])) $multiacc=1; else $multiacc=0;
		echo "<b>".$multiacc."</b>";//1 = 1 rekening attach banyak pdf akoun
		...
		if(pg_num_rows($qry_sel_cust_det)>0){
			...
			for($i=0;$i<count($nomor_rekening);$i++){
				if($multiacc==1){
						...
						if(pg_num_rows($qry_sel_antrian)==0){ //nomor_rekening, nama, pdf_name, pdf_location, diwakilkan oleh detail_id di antrian_email_attach_pdf
							if ($sv_m_antrian[$k]==1){
								$sql_ins_antrian = "INSERT INTO antrian_email(
										...
										".$loading_id[$i].");"; 
								$qry_ins_antrian = pg_query($sql_ins_antrian) or die('ERROR insert into antrian: '.$sql_ins_antrian);//echo $sql_ins_antrian.'::4<br><br>';
								for($c=0;$c<count($arr_attach_file_id);$c++){
									...
									$qry_ins_attach_id = pg_query($sql_ins_attach_id) or die('ERROR insert into antrian_email_attach_file: '.$sql_ins_attach_id);//echo $sql_ins_attach_id.'::3<br><br>';
								}
							...
							$qry_ins_attach_pdf = pg_query($sql_ins_attach_pdf) or die('ERROR insert into antrian_email_attach_pdf: '.$sql_ins_attach_pdf);//echo $sql_ins_attach_pdf.'::2<br><br>';
						}else{
							if($no_rek_exists==""){
					...
					//print_r($nomor_rekening);die();
					$arr_email = explode('##',$email[$i]);
					$sql_sel_antrian = "
						SELECT a.email
						FROM antrian_email a JOIN antrian_email_attach_pdf b ON a.antrian_id=b.antrian_id
							JOIN detail c ON b.detail_id=c.detail_id
						WHERE a.loading_id = ".$loading_id[$i]."
							and c.nomor_rekening = '".$nomor_rekening[$i]."'";
					...
					if(pg_num_rows($qry_sel_antrian)==0){
						for($k=0;$k<count($arr_email);$k++){
							$sql_ins_antrian = "INSERT INTO antrian_email(
									...
									now(), ".$template_id.", '".$arr_email[$k]."',
									".$loading_id[$i].");"; 
							echo $sql_ins_antrian.'<br>';
							$lastid="";							
							$qry_ins_antrian = pg_query($sql_ins_antrian) or die('ERROR insert into antrian: '.$sql_ins_antrian);//echo $sql_ins_antrian.'::4<br><br>';
							$sql_sel_antrian_id = "SELECT last_value as antrian_id FROM antrian_email_antrian_id_seq";
							$qry_sel_antrian_id = pg_query($sql_sel_antrian_id) or die('ERROR select antrian id: '.$sql_sel_antrian_id);
							$row_sel_antrian_id = pg_fetch_assoc($qry_sel_antrian_id);
							$lastid=$row_sel_antrian_id['antrian_id'];
							for($c=0;$c<count($arr_attach_file_id);$c++){
								$sql_ins_attach_id = "INSERT INTO antrian_email_attach_file(antrian_id, m_attach_file_id)
														VALUES($lastid,".$arr_attach_file_id[$c].")";
								echo $sql_ins_attach_id.'<br>';
								$qry_ins_attach_id = pg_query($sql_ins_attach_id) or die('ERROR insert into antrian_email_attach_file: '.$sql_ins_attach_id);//echo $sql_ins_attach_id.'::3<br><br>';
							}
							if(pg_affected_rows($qry_ins_antrian)){
								$jml_record++;
							}
							$sql_ins_attach_pdf="INSERT INTO antrian_email_attach_pdf(antrian_id, detail_id) VALUES ($lastid, ".$detail_id[$i].")"; 
							echo $sql_ins_attach_pdf.'<br>';
							$qry_ins_attach_pdf = pg_query($sql_ins_attach_pdf) or die('ERROR insert into antrian_email_attach_pdf: '.$sql_ins_attach_pdf);//echo $sql_ins_attach_pdf.'::2<br><br>';
						}
					}else{
						if($no_rek_exists==""){
