...
<?
				...
				$buat_tabel = "CREATE TABLE $tabeldetail(						
								 detail_id serial NOT NULL,
								  ...
								  CONSTRAINT $tabeldetail_pk PRIMARY KEY (detail_id),
								   CONSTRAINT $tabeldetail_un UNIQUE (nomor_rekening , m_loading_id )
								)WITHOUT OIDS;
		...
		while(!feof($open_dat)){
			...
			if(trim($row_dat)!=''){
				...
				if ($regtype == '0X' )
				{
					...
					if (empty( $password_admin_pdf ) ) die("ERROR 0X TIDAK ADA");
				...
				if ($regtype == '01' )
				{
					...
					if(trim($email_01) == "")
					{
						...
						$marks = array(100, 65, 70, 87);
						  ...
						  */
						if (in_array($OLS_CUST_NBR, $msg_not_exists))
						{
				...
				if ( $flag_email_kosong == FALSE){
					...
					if($regtype == '02'  ){
						...
						$GLOBALS['flag_bri_samsung']  = false;
					...
					if($flag_cust=='Y'){
						...
						if($jml_customer==0){
							...
							if($regtype == '02'){
								...
								if (in_array($no_card_dat, $msg_not_exists))
								{
	...
	function insert_detail($m_loading_id,$nomor_customer,$nomor_rekening,
		...
		{
			//die("PROSES GAGAL<br>CIF $nomor_rekening ada materainya dengan total : ".$flag_total_materai);
		}
		///////////////////////////////////////////////////////////////////
