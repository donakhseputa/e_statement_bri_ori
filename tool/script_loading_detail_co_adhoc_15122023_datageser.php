		...
		while(!feof($open_dat)){
			...
			if(trim($row_dat)!=''){
				...
				if($rectype=='02'){
					...
					//tanggal 14122023 UAT KKP tapi data geser ke kiri 3 karakter
					$geser = 3;
					$jmltrans		= ltrim(substr($row_dat,147-$geser,14),'0');
					
					$total_jmltrans	+= $jmltrans;
					
					$detail['dttrans'][($im-1)][$ih]		= substr($row_dat,79-$geser,2).'-'.substr($row_dat,77-$geser,2).'-'.substr($row_dat,73-$geser,4);
					$detail['dtbuku'][($im-1)][$ih]			= substr($row_dat,87-$geser,2).'-'.substr($row_dat,85-$geser,2).'-'.substr($row_dat,81-$geser,4);
					$detail['trans'][($im-1)][$ih]			= substr($row_dat,89-$geser,40);
					//$detail['forexcurr'][($im-1)][$ih]		= substr($row_dat,129-$geser,3);
					//dikosongkan saja biar tidak dobel print tulisan
					$detail['forexcurr'][($im-1)][$ih]		= '';
						$forex_nominal						= substr($row_dat,132-$geser,14);
					$detail['forexnominal'][($im-1)][$ih]	= number_format(($forex_nominal/100),2,".",",");
						$kurs_nominal						= substr($row_dat,164-$geser,10);
					...
					$detail['cr'][($im-1)][$ih]				= substr($row_dat,162-$geser,2);
		...
		*/
		...
		for($i=0;$i<$nheader;$i++){
			...
			*/
			...
			rectfill($pdf, 200, 0, $x_kotak, $y_kotak, 40, 10, "FD");
			$pdf->SetFont('Sandy','',9);
			$pdf->SetXY($x_kotak, $y_kotak);
			...
			rectfill($pdf, 0, 0, $x_kotak, $y_kotak + 10, 40, 5, "D");
			$pdf->SetFont('Sandy','');
			$pdf->SetX($x_kotak);
			...
			$pdf->Cell(25, 5, "", 0, 0, 'L');
			$pdf->Cell(25, 5, "", 0, 0, 'L');
			#####$pdf->Cell(60, 5, "Tagihan Bulan Lalu", 0, 0, 'L');
			$arr_summary[$cdata['no_card_dat']][$header['cdnumb'][$i]]['start'] = $pdf->PageNo();###
