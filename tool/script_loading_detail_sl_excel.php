...
/** LOADING FILE DENGAN MENGGUNAKAN NOMOR REKENING (DISAMAKAN DGN TABEL m_customer) **/
	...
	function excel_to_pdf($flagtrans,$menu_id,$blth,$file,$filecust,$con,$connection,$scon_group_nisp)
	{
		...
		#$master_header = "NAMA*NOMOR KARTU*ALAMAT*EMAIL*";
		$master_header = "Tanggal Surat*Kode Surat*No. Surat*Unit Kerja Surat*no_ref*nama*nama_pdf*alamat*email*";
			...
			foreach($sheetData as $row => $columns) 
			{
				...
				if($row != 1) 
				{
					...
					foreach($columns as $cell => $val) 
					{
						...
						#if ( $cell == 'E' || $cell == 'F' || $cell == 'G') continue;
						if ( $cell == 'J' || $cell == 'K' || $cell == 'L') continue;
				...
				//	echo $query .'<br>';
					...
					foreach($columns as $cell => $val) 
					{
						...
						if ( $cell == 'J' || $cell == 'K' || $cell == 'L') continue;
						#$kolom .= $val.'*';
						$kolom .= addslashes(str_replace( array("'",'(',')'), '' , $val) ).'*';
	...
	function to_pdf($flagtrans,$blth,$file,$con,$temp,$jenis_pdf,$tabeldetail,$m_loading_id, $tanggal_nama_file='')
	{
		...
		while($row = pg_fetch_array($qry)){
			...
			if ( empty(  $row['nama_pdf'] ) )
			{
				...
				$nama_file_pdf			=  str_replace('.pdf','',$nama_file_pdf);
			}
			...
			{ 
				...
				$pdf->SetXY($x+20, $y);
				#$pdf->Cell(10, 10, $no_surat.'[NO_SURAT]', 0, 0, "L");
				...
				#$pdf->Cell(10, 10, 'Jakarta, '.$tgl_surat.'[TGL_SURAT]', 0, 0, "L");
				$pdf->Cell(10, 10, 'Jakarta, '.$tgl_surat, 0, 0, "L");
				
				//-------------------------------------------------------------------------
				
				$pdf->SetXY($x, $y+(1*$jeda_baris));
				$pdf->Cell(10, 10, 'Lampiran', 0, 0, "L");
				...
				$pdf->SetXY($x, $y+(2*$jeda_baris));
				$pdf->Cell(10, 10, 'Perihal', 0, 0, "L");
				
				$pdf->SetXY($x+15, $y+(2*$jeda_baris));
				$pdf->Cell(10, 10, ':', 0, 0, "L");
				
				$pdf->SetFont('helvetica', '', 10);
				$pdf->SetXY($x+20, $y+(2*$jeda_baris));
				$pdf->Cell(10, 10, 'Surat Keterangan Lunas', 0, 0, "L");
				
				$pdf->SetFont('helvetica', '', 10);
				$pdf->SetXY($x+20, $y+(3*$jeda_baris));
				$pdf->Cell(10, 10, 'a.n. '.$nama, 0, 0, "L");
				...
				$pdf->Cell(10, 10, 'Di Tempat', 0, 0, "L");
				/*
				$pdf->SetXY($x+$var_start_kanan, $y+(5*$jeda_baris));
				...
				*/
				...
				$pdf->SetXY($x, $y+((13-$selisihx)*$jeda_baris));
				$html = '<span style="text-align:left;">Dengan ini disampaikan bahwa Nasabah Kartu Kredit BRI sebagai berikut :</span>';
				...
				$pdf->SetFont('helvetica', 'B', 10);
				$pdf->SetXY($x, $y+((14.5-$selisihx)*$jeda_baris));
				$pdf->Cell(10, 10, 'NAMA', 0, 0, "L");
				
				$pdf->SetXY($x+35, $y+((14.5-$selisihx)*$jeda_baris));
				$pdf->Cell(10, 10, ':', 0, 0, "L");
				
				$pdf->SetFont('helvetica', 'B', 10);
				$pdf->SetXY($x+40, $y+((14.5-$selisihx)*$jeda_baris));
				$pdf->Cell(10, 10, $nama, 0, 0, "L");
				
				
				$pdf->SetFont('helvetica', 'B', 10);
				$pdf->SetXY($x, $y+((15.5-$selisihx)*$jeda_baris));
				$pdf->Cell(10, 10, 'NO. KARTU KREDIT', 0, 0, "L");
				
				$pdf->SetXY($x+35, $y+((15.5-$selisihx)*$jeda_baris));
				$pdf->Cell(10, 10, ':', 0, 0, "L");
				
				$pdf->SetFont('helvetica', 'B', 10);
				$pdf->SetXY($x+40, $y+((15.5-$selisihx)*$jeda_baris));
				$pdf->Cell(10, 10, $nomor_rekening, 0, 0, "L");
				
				
				
				$pdf->SetFont('helvetica', 'B', 10);
				$pdf->SetXY($x, $y+((16.5-$selisihx)*$jeda_baris));
				$pdf->Cell(10, 10, 'ALAMAT', 0, 0, "L");
				
				$pdf->SetXY($x+35, $y+((16.5-$selisihx)*$jeda_baris));
				$pdf->Cell(10, 10, ':', 0, 0, "L");
				
				$pdf->SetFont('helvetica', 'B', 10);
				$pdf->SetXY($x+40, $y+((17-$selisihx)*$jeda_baris));
				///$pdf->Cell(10, 10, $address3, 0, 0, "L");
				$pdf->MultiCell(88, 4, $address3, 0, 'L', false, 1, '', '', true, 0, false, true, 0);
				...
				//-------------------------------------------------------------------------
					...
					$pdf->SetLeftMargin(20);
					$pdf->SetRightMargin(20);
					$pdf->SetXY($x, $y+((20-$selisihx)*$jeda_baris));
					$html = '<span style="text-align:justify;">Telah menyelesaikan kewajiban terhadap Kartu Kredit BRI pada tanggal  [TGL_LUNAS]  sehingga tidak mempunyai kewajiban lagi terhadap Kartu Kredit BRI atau dinyatakan LUNAS.</span>';
					// output the HTML content
					$pdf->writeHTML($html, true, 0, true, true);
				...
				if ( empty($tgl_lunas_bayar) || $tgl_lunas_bayar =='' )
				{
					...
					$html = '<span style="text-align:justify;">Dapat kami sampaikan, bahwa Kartu Kredit BRI tersebut sudah kami tutup per tanggal <b>'.$tgl_close_card.'</b>. Namun, apabila setelah tanggal penutupan kartu terdapat tagihan transaksi, maka akan dilakukan klarifikasi dan penyelesaian sesuai dengan ketentuan BRI. Sehubungan dengan hal tersebut, kami mohon agar fisik kartu dapat digunting/dihancurkan untuk menghindari hal-hal yang tidak diinginkan.</span>';
					// output the HTML content
					$pdf->writeHTML($html, true, 0, true, true);
				...
				*/
				...
				$pdf->SetLeftMargin(20);
				$pdf->SetRightMargin(20);
				$pdf->SetXY($x, $y+((22.5-$selisihx)*$jeda_baris));
				$html = '<span style="text-align:justify;">Namun apabila dikemudian hari setelah tanggal [TGL_LUNAS] terdapat transaksi dan terbukti valid dilakukan oleh Nasabah, maka akan diperhitungkan sebagai tagihan baru Nasabah.</span>';
				// output the HTML content
				$pdf->writeHTML($html, true, 0, true, true);
				...
				$pdf->SetLeftMargin(20);
				$pdf->SetRightMargin(20);
				$pdf->SetXY($x, $y+((25-$selisihx)*$jeda_baris));
				$html = '<span style="text-align:left;">Apabila Bapak/Ibu memerlukan informasi lebih lanjut, dapat menghubungi Call BRI 14017/1500017. Demikian, atas perhatian dan kerjasama yang baik disampaikan terimakasih.</span>';
				// output the HTML content
				$pdf->writeHTML($html, true, 0, true, true);
				...
				$pdf->SetXY($x+120, $y+((28-$selisihx)*$jeda_baris));
				$pdf->Cell(10, 10, 'PT. Bank Rakyat  Indonesia (Persero) Tbk.', 0, 0, "C");
				
				$pdf->SetXY($x+120, $y+((29-$selisihx)*$jeda_baris));
				$pdf->Cell(10, 10, 'DIVISI KARTU KREDIT', 0, 0, "C");
				
				$pdf->SetXY($x+120, $y+((30-$selisihx)*$jeda_baris));
				$pdf->Cell(10, 10, 'BAGIAN COLLECTION', 0, 0, "C");
				
				
				$pdf->SetFont('helvetica', 'U', 10);
				$pdf->SetXY($x+120, $y+((38-$selisihx)*$jeda_baris));
				$pdf->Cell(10, 10, 'I GUSTI BAGUS IRVAN SETYANUGRA', 0, 0, "C");
				
				$pdf->SetFont('helvetica', '', 10);
				$pdf->SetXY($x+120, $y+((39-$selisihx)*$jeda_baris));
				$pdf->Cell(10, 10, 'WAKIL KEPALA BAGIAN', 0, 0, "C");
				
				
				$url_img_produk = '../images/barcode_SL_2.jpg';
				// Image($file, $x='', $y='', $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false)
				$pdf->Image($url_img_produk,$x+105+5,$y+((31.5-$selisihx)*$jeda_baris), '31', '', '', '', '', true, 300, '', false, false, 0, false, false, false);
				
				//FOOTER
				//-------------------------------------------------------------------------
				
				$pdf->SetFont('helvetica', 'U', 10);
				$pdf->SetXY($x+120-80, $y+((38-$selisihx)*$jeda_baris));
				$pdf->Cell(10, 10, 'IRA DERMAWAN', 0, 0, "C");
				
				$pdf->SetFont('helvetica', '', 10);
				$pdf->SetXY($x+120-80, $y+((39-$selisihx)*$jeda_baris));
				$pdf->Cell(10, 10, 'KEPALA BAGIAN', 0, 0, "C");
				
				
				$url_img_produk = '../images/barcode_SL_1.jpg';
				// Image($file, $x='', $y='', $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false)
				$pdf->Image($url_img_produk,$x+105+5-80,$y+((31.5-$selisihx)*$jeda_baris), '31', '', '', '', '', true, 300, '', false, false, 0, false, false, false);
				//-------------------------------------------------------------------------
