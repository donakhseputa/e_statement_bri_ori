...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <script>
		function upload_detail(val,flag,rad_cust){
			if(flag=='MT'){
				if(rad_cust==1){
					$.post("script_loading_detail_nocust_bc_consolidated_ematerai_convert_only.php",{pr:val},function(respon){
    ...
    <?
		...
		*/
		$file = $_POST['view_folder'];
		#die("nama file foldernya : $file");
		$folder_ori='../file_consolidate/';
		$folder_dest="../tmp/";
		$folder_dest2="../tmp/$file";
		
		if(file_exists($folder_dest2))
		{
			unlink($folder_dest2);
		}
		
		if(rename($folder_ori.$file,$folder_dest.$file)){
			chmod($folder_dest . $file, 0777);
			//docx application/vnd.openxmlformats-officedocument.wordprocessingml.document
			//die ( $_FILES["loading_file"]["type"] );
