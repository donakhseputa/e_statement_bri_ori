...
<?
	...
	?>
		<select name="norek" id="norek" class="combobox" onchange="email()">
			<option selected="selected" value=""></option>
	<?
		$sql_norek = "SELECT DISTINCT b.nomor_rekening, b.nama, substring(b.nomor_rekening,1,8) as norek
							FROM m_customer a
								INNER JOIN detail b
									ON a.nomor_rekening = b.nomor_rekening
							WHERE b.blth = '$blth' and b.flagtrans = '$flagtrans' and b.nama_file = '$file'
							ORDER BY norek, b.nama, b.nomor_rekening";
	...
	function kirim_ke_antrian(){
		...
		$sql_sel_cust_det = "SELECT a.nomor_customer, a.nomor_rekening, a.blth, a.flagtrans, a.email1, a.email2, a.email3,
									b.m_loading_id, b.nama, b.nama_file, c.tr_email_id, c.date_email_send, c.email_callback, b.pdf_name as pdf_name
								FROM m_customer a
									INNER JOIN detail b
										ON a.blth = b.blth
									...
									LEFT JOIN $nama_view c
										ON b.m_loading_id = c.loading_id
											and a.nomor_rekening = c.nomor_rekening
											and a.email1 = c.email
								WHERE a.blth = '".$blth."'";
		if($file!=''){
			$sql_sel_cust_det .= " and b.nama_file = '$file'";
			$sql_sel_cust_det .= " and a.log_customer_id = '$file_cust_id'";
		}
		if($norek!=''){
