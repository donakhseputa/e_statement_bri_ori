...
<?
	...
	function dat_to_pdf($flagtrans,$menu_id,$blth,$file,$url_directory,$pdftk_loc,$file_mcustomer){
			...
			if(trim($row_dat)!=''){
				...
				if($loop != 1){
					if(($prevcardnb<>$no_card_dat)&&($flag_case=='Y')){
						...
						insert_detail($m_loading_id,$cdata,
							...
							,$flag_bea_materai_lunas[$cdata['no_card_dat']]['total_ematerai'], 
							$flag_bea_materai_lunas[$cdata['no_card_dat']]['jumlah_kartu_ematerai']
							,$flag_total_ematerai);
				...
				if($prevcardnb<>$no_card_dat){
					if($rectype=='01'){ //hanya akan masuk saat nosebelum tidak sama dengan nocurrent, dan rectype 01
						...
						if(!isset($flag_total_ematerai[$cdata['no_card_dat']]))
						{
							$flag_total_ematerai[$cdata['no_card_dat']] = 0;
						}else{
							$flag_ematerai_data	= trim(substr($row_dat,1120,1));
							if ( $flag_ematerai_data == 'Y')
							{
								$flag_total_ematerai[$cdata['no_card_dat']] = $flag_total_ematerai[$no_card_dat]+1;
							}
						}
						
						
						$cdata['pass_admin_pdf']= trim(substr($row_dat,1152,13));
						$cdata['file_name_pdf']= trim(substr($row_dat,1122,29));
						...
						/////////////////////////////////////////////////////////////////////////////////////// kz
						//E-MATERAI
						$flag_bea_materai_lunas[$cdata['no_card_dat']]['total_ematerai']	= 0;
						$flag_bea_materai_lunas[$cdata['no_card_dat']]['jumlah_kartu_ematerai']	= '';
						//E-MATERAI
						/////////////////////////////////////////////////////////////////////////////////////// kz
						/////////////////////////////////////////////////////////////////////////////////////// kz
				...
				if($rectype=='02'){
					...
					{
						$flag_bea_materai_lunas[$cdata['no_card_dat']]['total_ematerai']++;
						$flag_bea_materai_lunas[$cdata['no_card_dat']]['jumlah_kartu_ematerai']	.= $kartu_ematerai.' / ';
					}
					///////////////////////////////////////////////////////////
					///////////////////////////////////////////////////////////
		...
		#####die (print_r($flag_total_ematerai));
		return $customer.'|'.$not_exists;
	}
	
	function doPDF($cdata,$header,$password_pdf,$blth,$file,$detail,$flag_case,$flagtrans,$barcode, $dt_kurir1 ,$flag_total_ematerai){
		...
		//////////////////////////////////////////////////////////////////////////////////////////////////
		//PENAMBAHAN HALAMAN 1 STAMP
		if ( $flag_total_ematerai[$cdata['no_card_dat']]  > 0)
		{
		$pdf->AddPage();
		...
		$pdf->MultiCell(190,5,'Lembar Pengesahan E-Meterai',0,'C');
		...
		$pdf_name =$cdata['file_name_pdf'];
		$pass_admin_pdf =$cdata['pass_admin_pdf'];
		...
		$blth_file_cetak_e_meterai = $blth_pdf.$file.'/E_METERAI/';
		...
		if(file_exists($blth_file_cetak_e_meterai)==false) mkdir($blth_file_cetak_e_meterai);
		$file_copy = $pdf_name;
		...
		if($flag_case == "Y"){
			$dir_file_copy = copy_insert_password($blth_file_cetak2, $blth_file_pdf, $file_copy, $password_pdf, $pass_admin_pdf);
			
			if ( $flag_total_ematerai[$cdata['no_card_dat']] >0)
			{
				copy($dir_file_copy,  $blth_file_cetak_e_meterai.$pdf_name.'.pdf');	
			}
		}
	...
	function insert_detail($m_loading_id,$cdata,
		...
		$pdf_name,$password_pdf,$jml_hlm,$tabeldetail,$email,$barcode, $dt_kurir1
		,$total_ematerai,$jumlah_kartu_ematerai
		,$flag_total_ematerai){
		...
		////////////////////////// JIKA MATERAI >1 nama file pdf dan email dikosongkan
		if ( $flag_total_ematerai[$cdata['no_card_dat']]  > 0)
		{
			$pdf_name_emeterai = $pdf_name;
			$email_emeterai = $email;
		...
		$sql_ins_detail = "INSERT INTO $tabeldetail(
								...
								,'$total_ematerai','$jumlah_kartu_ematerai'
								,'$pdf_name_emeterai','".addslashes($email_emeterai)."'
								,'".$flag_total_ematerai[$cdata['no_card_dat']]."'
							)";
	...
	function copy_insert_password($source_location, $destination_location, $pdf_name, $password_pdf, $pass_admin_pdf){
		...
		*/
		...
		if(!file_exists($destination_location.$pdf_name.".pdf")) {
			...
			return $destination_location.$pdf_name.".pdf";
