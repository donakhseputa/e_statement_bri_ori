<?php
/* CONFIG DATABASE */
$www_location = "D:/htdocs/e_statement_bri/";					/*untuk live*/ 
...
$con = pg_connect($connection) or die("Could not connect to database!");
	...
	function update_read($con, $date_email_callback, $ket_error, $email, $message_id1, $pengirim, $tipe_baca)
	{
		$sql='';
		$sql = "UPDATE tr_email
				SET
...
function get_from_mailserver($con,$email_inbox,$email_from,$email_pass)
{
	...
	if ( imap_num_msg($mbox) > 5000 ) $xx=5000;
	...
	for($i = 1 ; $i <= $xx ; $i++)
	{
					...
					)
			{
				...
				*/
				...
				$cek_delete2 = update_read( $con, $xyz, substr(imap_body($mbox, $i), 0, 1000), strtoupper($abcd[0]), $message_id1, strtoupper($pengirim) ,'img_email');
				if ( $cek_delete2 == 'db_berhasil' )
				{
					imap_delete($mbox, $i);
					$n_img++;
	...
	function insert_tabel_gagal($con, $header, $body, $tgl_read, $blth, $tr_email_id, $loading_id, $flagtrans, $email_bouncenya, $sample, $keterangan)
	{
		...
		$sql_insert = " insert into bounce_inbox(header_bounce, body_bounce, date_bounce, status, blth, tr_email_id, m_loading_id, flagtrans, email,sample_status) 
							values('".addslashes ($header)."'::varchar(100), E'".addslashes ($body)."'::text, '$tgl_read', '$keterangan', '$blth', '$tr_email_id','$loading_id','$flagtrans', '".substr($email_bouncenya, 0, 39)."','$sample')";
	...
	function get_tr_email_id_read($con, $email, $message_id1, $pengirim, $tgl_baca)
	{
		...
		if ( !empty( $message_id1) && intval($tr_email_id) )
		{
			...
			$sql=" 
					SELECT tr_email_id FROM tr_email
