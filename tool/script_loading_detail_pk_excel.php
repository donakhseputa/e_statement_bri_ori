...
/** LOADING FILE DENGAN MENGGUNAKAN NOMOR REKENING (DISAMAKAN DGN TABEL m_customer) **/
	...
	function excel_to_pdf($flagtrans,$menu_id,$blth,$file,$filecust,$con,$connection,$scon_group_nisp)
	{
		...
		#$master_header = "No*No Ticket2*Card_number*Nama*Tgl Close Card*Tgl Lunas_Bayar*No Surat*Tgl Surat*ALAMAT_1*ALAMAT_2*ALAMAT_3*ALAMAT_4*ALAMAT_5*CR_ADDR_EMAIL*Nama File convert PDF (Save File)*";
		$master_header = "No*No TT 2*Card_number*Nama*Tgl Tutup Kartu*Tanggal lunas Bayar*No Surat*Tanggal Surat*ALAMAT_1*ALAMAT_2*ALAMAT_3*ALAMAT_4*ALAMAT_5*EMAIL*Save as PDF*Divisi*Jabatan Kadiv*Barcode Kadiv*Nomer HP*";
			...
			foreach($sheetData as $row => $columns) 
			{
				...
				if($row != 1) 
				{
					...
					foreach($columns as $cell => $val) 
					{
						...
						#if ( $cell == 'P' || $cell == 'Q' || $cell == 'R' || $cell == 'S') continue;
						//if ($cell == 'D' || 'O'){
						//	
						...
						if ( $cell == 'T' || $cell == 'U' || $cell == 'V') continue;
					...
					cr_addr_email ,
					nama_pdf ,
					divisi , jabatan_kadiv, barcode_kadiv,hp,
	...
	*/
	...
	function to_pdf($flagtrans,$blth,$file,$con,$temp,$jenis_pdf,$tabeldetail,$m_loading_id, $tanggal_nama_file='')
	{
		...
		while($row = pg_fetch_array($qry)){
			...
			$tgl_lunas_bayar		=  trim($row['tgl_lunas_bayar']); //KEY NYA
			...
			$jabatan_kadiv			=  trim($row['jabatan_kadiv']);
			$hp						=  trim($row['hp']);
			...
			$password_pdf			=  '';
			//$nama_file_pdf			=  $n.'_'.$nomor_rekening.'.pdf';
			...
			{ 
				...
				$pdf->SetFont('helvetica', '', 10);
				$pdf->SetXY($x+$var_start_kanan, $y);
				$pdf->Cell(10, 10, 'Jakarta, '.$tgl_surat, 0, 0, "L");
				...
				$pdf->Cell(10, 10, 'Surat Keterangan Penutupan', 0, 0, "L");
				
				$pdf->SetFont('helvetica', 'U', 10);
				$pdf->SetXY($x+20, $y+(2*$jeda_baris));
				//$pdf->Cell(10, 10, 'Account Ceria', 0, 0, "L");
				...
				$pdf->Cell(10, 10, 'Kepada Yth,  ', 0, 0, "L");
				...
				$pdf->SetXY($x+$var_start_kanan, $y+(5.5*$jeda_baris));
				//$pdf->Cell(10, 10, strtoupper($address1), 0, 0, "L");
				$pdf->MultiCell(80, 5, $address1.' '.$address2.' '.$address3.' '.$city.' '.$zipcode, 0, 'L', 0, 0, '', '', true);
				
				//$y_alamat1 = 4pdf->GetY();
				
				//$pdf->SetXY($x+$var_start_kanan, $y+(7.6*$jeda_baris));
				//$pdf->Cell(10, 10, strtoupper($address2), 0, 0, "L");
				//
				//$pdf->SetXY($x+$var_start_kanan, $y+(8.6*$jeda_baris));
				//$pdf->Cell(10, 10, strtoupper($address3), 0, 0, "L");
				//
				//$pdf->SetXY($x+$var_start_kanan, $y+(9.6*$jeda_baris));
				//$pdf->Cell(10, 10, strtoupper($city), 0, 0, "L");
				//
				//$pdf->SetXY($x+$var_start_kanan, $y+(10.6*$jeda_baris));
				...
				$pdf->SetXY($x, $y+(13*$jeda_baris));
				$html = '<span style="text-align:left;">Sesuai dengan nomer Pelaporan <b>'.$nomor_customer.'</b> perihal Permohonan Penutupan Kartu Kredit BRI atas nama :</span>';
				//$html = '<span style="text-align:left;">Untuk meningkatkan layanan Ceria Anda dikemudian hari, berikut ini kami sampaikan rincian data account Ceria sebagai berikut:</span>';
				...
				$pdf->SetFont('helvetica', '', 10);
				$pdf->SetXY($x, $y+(14.5*$jeda_baris));
				$pdf->Cell(10, 10, 'Nama', 0, 0, "L");
				
				$pdf->SetXY($x+35, $y+(14.5*$jeda_baris));
				$pdf->Cell(10, 10, ':', 0, 0, "L");
				
				$pdf->SetFont('helvetica', 'B', 10);
				$pdf->SetXY($x+40, $y+(14.5*$jeda_baris));
				$pdf->Cell(10, 10, $nama, 0, 0, "L");
				
				
				$pdf->SetFont('helvetica', '', 10);
				$pdf->SetXY($x, $y+(15.5*$jeda_baris));
				//$pdf->Cell(10, 10, 'No Account Ceria', 0, 0, "L");
				$pdf->Cell(10, 10, 'No Kartu Kredit BRI', 0, 0, "L");
				
				$pdf->SetXY($x+35, $y+(15.5*$jeda_baris));
				$pdf->Cell(10, 10, ':', 0, 0, "L");
				
				$pdf->SetFont('helvetica', 'B', 10);
				$pdf->SetXY($x+40, $y+(15.5*$jeda_baris));
				$pdf->Cell(10, 10, $nomor_rekening, 0, 0, "L");
				
				/*ceria
				$pdf->SetFont('helvetica', '', 10);
				$pdf->SetXY($x, $y+(16.5*$jeda_baris));
				$pdf->Cell(10, 10, 'No Handphone', 0, 0, "L");
				...
				$pdf->Cell(10, 10, $hp, 0, 0, "L");
				*/
				
				$pdf->SetFont('helvetica', '', 10);
				//-------------------------------------------------------------------------
				
				if ( empty($tgl_lunas_bayar) || $tgl_lunas_bayar =='' )
				{
					
					
					$pdf->SetLeftMargin(20);
					$pdf->SetRightMargin(20);
					$pdf->SetXY($x, $y+(19*$jeda_baris));
					//$html = '<span style="text-align:justify;">Dapat kami sampaikan, bahwa account Ceria tersebut sudah dilakukan penutupan pada tanggal <b>'.$tgl_close_card.'</b>. Namun, apabila setelah tanggal penutupan account Ceria terdapat tagihan transaksi, maka akan dilakukan klarifikasi dan penyelesaian sesuai dengan ketentuan BRI. </span>';
					$html = '<span style="text-align:justify;">Dapat kami sampaikan, bahwa Kartu Kredit BRI tersebut sudah dilakukan penutupan pada tanggal <b>'.$tgl_close_card.'</b>. Namun, apabila setelah tanggal penutupan Kartu Kredit BRI terdapat tagihan transaksi, maka akan dilakukan klarifikasi dan penyelesaian sesuai dengan ketentuan BRI. </span>';
					// output the HTML content
					$pdf->writeHTML($html, true, 0, true, true);
					...
					$pdf->SetLeftMargin(20);
					$pdf->SetRightMargin(20);
					$pdf->SetXY($x, $y+(18*$jeda_baris));
					//$html = '<span style="text-align:justify;">Dapat kami sampaikan, bahwa account Ceria tersebut sudah lunas per tanggal <b>'.$tgl_lunas_bayar.'</b> dan sudah kami tutup per tanggal <b>'.$tgl_close_card.'</b>. Namun, apabila setelah tanggal penutupan account Ceria terdapat tagihan transaksi, maka akan dilakukan klarifikasi dan penyelesaian sesuai dengan ketentuan BRI. </span>';
					$html = '<span style="text-align:justify;">Dapat kami sampaikan, bahwa Kartu Kredit BRI tersebut sudah lunas per tanggal <b>'.$tgl_lunas_bayar.'</b> dan sudah kami tutup per tanggal <b>'.$tgl_close_card.'</b>. Namun, apabila setelah tanggal penutupan Kartu Kredit BRI terdapat tagihan transaksi, maka akan dilakukan klarifikasi dan penyelesaian sesuai dengan ketentuan BRI. </span>';
					// output the HTML content
					$pdf->writeHTML($html, true, 0, true, true);
				...
				$pdf->SetLeftMargin(20);
				$pdf->SetRightMargin(20);
				$pdf->SetXY($x, $y+(22.5*$jeda_baris));
				$html = '<span style="text-align:justify;">Demikian informasi yang dapat kami sampaikan, mohon agar surat ini dapat dipergunakan dengan semestinya. Atas perhatian dan kerja samanya, kami sampaikan terima kasih.</span>';
				// output the HTML content
				$pdf->writeHTML($html, true, 0, true, true);
				...
				$pdf->SetLeftMargin(20);
				$pdf->SetRightMargin(20);
				$pdf->SetXY($x, $y+(25*$jeda_baris));
				//$html = '<span style="text-align:left;">Untuk informasi lebih lanjut hubungi Call BRI 14017 atau 021-1500017</span>';
				$html = '<span style="text-align:left;">Untuk informasi lebih lanjut hubungi Contact BRI 1500017, Layanan WhatsApp Sabrina 08121214017, dan Layanan Bebas Pulsa Via aplikasi Brimo.</span>';
				// output the HTML content
				$pdf->writeHTML($html, true, 0, true, true);
				...
				//$pdf->SetFont('helvetica', 'B', 10);
				//$pdf->SetXY($x+120, $y+(27*$jeda_baris));
				//$pdf->Cell(10, 10, 'Jakarta, '.$tgl_surat, 0, 0, "C");
				$pdf->SetFont('helvetica', 'B', 10);

				$pdf->SetXY($x+120, $y+(28*$jeda_baris));
				$pdf->Cell(10, 10, 'PT. Bank Rakyat  Indonesia (Persero) Tbk.', 0, 0, "C");
				
				$pdf->SetXY($x+85, $y+(29.5*$jeda_baris));
				//$pdf->Cell(10, 10, $divisi, 0, 0, "C");
				$pdf->MultiCell(80, 5, $divisi, 0, 'C', 0, 0, '', '', true);
				
				$pdf->SetXY($x+120, $y+(40*$jeda_baris));
				#$pdf->Cell(10, 10, 'Executive Vice President', 0, 0, "C");
				$pdf->Cell(10, 10, $jabatan_kadiv, 0, 0, "C");
				
				
				#$url_img_produk = '../images/barcode_n_20201111.jpg';
				$url_img_produk = '../images/barcode_n_20211006.jpg';
				// Image($file, $x='', $y='', $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false)
				$pdf->Image($url_img_produk,$x+105+5,$y+(33*$jeda_baris), '30', '', '', '', '', true, 300, '', false, false, 0, false, false, false);
				...
				$pdf->SetFont('helvetica', '', 10);
				$pdf->SetXY($x, $y+(42*$jeda_baris));
				$pdf->Cell(10, 10, '----------------------------------------------------------------------------------------------------------------------------------------------', 0, 0, "L");
				
				$pdf->SetFont('helvetica', 'I', 8);
				//$pdf->SetXY($x+30, $y+(43*$jeda_baris));
				$pdf->SetXY($x+35, $y+(43*$jeda_baris));
				...
				//-------------------------------------------------------------------------------
				//HEADER
				$url_img_produk = '../images/logo_bri_n.jpg';
				$pdf->Image($url_img_produk,17,10, '30', '', '', '', '', true, 300, '', false, false, 0, false, false, false);
				
				
				$jeda_baris = 4;
				$y = 8;
				$pdf->SetFont('helvetica', 'B', 14);
				$pdf->SetXY($x+80, $y+(1*$jeda_baris));
				...
				//bila alamat 5 bukan kode zip code
				...
				$sql_ins_detail = "INSERT INTO $tabeldetail(
								...
								$m_loading_id,'$nomor_customer','$nomor_rekening',
								E'".addslashes($nama)."',E'".addslashes(substr($address1,0,150))."',E'".addslashes(substr($address2,0,150))."',
								E'".addslashes(substr($address3,0,150))."',E'".addslashes(substr($city,0,40))."','$zipcode',
								'$flagtrans','$blth','$file',
								E'".addslashes($nama_file_pdf)."','$password_pdf',$jml_hlm,
	...
	function insert_detail($m_loading_id,$nomor_customer,$nomor_rekening,
		...
		$sql_ins_detail = "INSERT INTO $tabeldetail(
								...
								'$flagtrans','$blth','$nama_file',
								E'".addslashes($pdf_name)."','$password_pdf',$jml_hlm, '$tipe_kartunya','".addslashes($email)."','$n_email','$filesizePdf'
							)";
		$qry_ins_detail = pg_query($sql_ins_detail) or die('ERROR insert into detail: '.$sql_ins_detail);
