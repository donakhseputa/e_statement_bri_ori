...
<?
	...
	?>
		<select name="blth" id="blth" class="combobox" onchange="
        		Loaddiv('blth_record','script_split_jadwal_kirim_antrian_sk.php','flagtrans=<?=$flagtrans?>'+'&act=blth_record&blth='+this.value);
	...
	$tabeldetail = "detail_" . $blth . "_" . $flagtrans;
		$sql_blth = "SELECT blth
						...
						GROUP BY blth
						ORDER BY substring(blth,3,4) desc, substring(blth,1,2) desc LIMIT 8";
	...
	?>
		<select name="nama_file" id="nama_file" class="combobox" onchange="
        	Loaddiv('nama_file_record','script_split_jadwal_kirim_antrian_sk.php','flagtrans=<?=$flagtrans?>'+'&act=filerecord&blth=<?=$blth?>&file='+this.value);
	...
	function norek($ord,$blth,$flagtrans,$file){
		...
		<?
			$sql_norek = "SELECT DISTINCT nomor_rekening, nama, jml_hlm
								...
								--and nomor_rekening ='4359650100044602'
								$urut";
			...
			$sql_sel_selektif = "
				...
				FROM attach_selektif a
					...
					INNER JOIN m_attach_file c
						ON a.m_attach_file_id::integer = c.m_attach_file_id::integer
				WHERE b.blth = '$blth'
					and b.flagtrans = '$flagtrans'";
			if($nama_file != ""){
	...
	function kirim_ke_antrian(){
		...
		$check_attach_auto = $_POST['check_attach_auto'];
		$jml_record = 0;
		...
		*/
		$sql_sel_cust_det = "SELECT b.nomor_customer, b.nomor_rekening, b.blth, b.flagtrans, b.email,
									b.m_loading_id, b.nama, b.nama_file, b.jml_hlm, c.tr_email_id, c.date_email_send, c.email_callback, b.pdf_name as pdf_name
								FROM (SELECT * FROM $tabeldetail WHERE nama_file = '$file' ) b 
									LEFT JOIN $nama_view c
										ON b.m_loading_id = c.loading_id
											and b.nomor_rekening = c.nomor_rekening
								WHERE b.email!=''";
		
		###$sql_sel_cust_det .= " AND ( lower(b.email) like lower('%@bri.co.id%') OR lower(b.email) like lower('%@corp.bri.co.id%') ) ";
		...
		*/
		...
		///////////////////////// PENAMBAHAN QUEQUE IMPORT DB DARI MAIL SERVER 20210714 nano
		#####$sql_sel_cust_det .= " AND lower(b.email) IN ( select lower(email_tujuan) from _list_queue_mail_server where lower(pengirim)=lower('estatement@ccbri.co.id') )";
		///////////////////////// PENAMBAHAN QUEQUE IMPORT DB DARI MAIL SERVER 20210714 nano
		$sql_sel_cust_det .= " $urut";
		
		/*
		
		$sql_sel_cust_det .= " and b.nomor_rekening IN (
		
select nomor_rekening from tr_email_062020_bc 
WHERE date_email_send < '2020-06-21 14:08:17.352' and date_email_send > '2020-06-01 14:08:17.352'
and tgl_read IS NULL 
and loading_id IN (select m_loading_id from m_loading where blth='062020' and flagtrans='BC')
AND ( lower(email) like lower('%@yahoo%') OR  lower(email) like  lower('%@ymail%') OR  lower(email)  like lower('%@ROCKETMAIL%') ) 
order by date_email_send desc
		) ";
		*/
		
		
		//PENGECEKAN JIKA BELUM DIKIRIM SAMA SEKALI 20210326
			$sql_xyz = "SELECT * from $nama_view ";
			$query_xyz = pg_query($sql_xyz);
			$n_xyz = pg_num_rows($query_xyz);
			if ($n_xyz==0 && $norek=='' ){
				$sql_sel_cust_det = "SELECT b.nomor_customer, b.nomor_rekening, b.blth, b.flagtrans, b.email,
									b.m_loading_id, b.nama, b.nama_file, b.jml_hlm, b.pdf_name as pdf_name
									FROM $tabeldetail b
									WHERE b.email!='' AND  b.nama_file = '$file'";
			}
		//PENGECEKAN JIKA BELUM DIKIRIM SAMA SEKALI 20210326
			...
			for($i=0;$i<count($nomor_rekening);$i++){
				...
				for($k=0;$k<count($arr_email);$k++){
					...
					if(@pg_num_rows($qry_sel_antrian)==0){
						...
						$kode_kirim_x = cek_kode_kirim($arr_email[$k]);
						
						if ($kode_kirim_x == 4 && $flagtrans != 'BC')
						{
							$kode_kirim_x = 3;
						}
						
						$kode_prioritas = prioritas_kirim_antrian_email($nomor_rekening[$i] , $nomor_customer[$i]);
						...
						$sql_ins_antrian = "INSERT INTO antrian_email(
													...
													loading_id, jadwal_id, status_sample, blth, flagtrans,
													log_approval_id
													,kode_kirim
													,prioritas_kirim
												)VALUES(
													...
													".$loading_id[$i].", ".$jadwal_id.", '".$status_sample[$i]."', '".$blthnya[$i]."', '$flagtrans',
													'$log_approval_id'
													,$kode_kirim_x 
													,$kode_prioritas
												);";
						...
						$status_pdf_belakang = false; //kz attachment auto TRUE di matikan oleh bGZ 11072024
		...
		if ( $flagtrans == 'BC' )
		{
			if (empty($m_loading_id)) die ("m_loading_id kosong");
			ubah_cyber_ke_aws($m_loading_id);
			ubah_cyber_ke_aws_log_bulan_lalu($m_loading_id);
		}
	...
	function ubah_cyber_ke_aws($m_loading_id)
	{
			#$total_email = 30000;
			$total_email = 10000;
			$n = array();
			...
			$n_xyz = 0;
			$sql = "
				select kode_kirim, count(*) as jumlah from antrian_email
				where loading_id = $m_loading_id
				group by kode_kirim";
			$query = pg_query($sql) or die('ERROR: CYCBER AWS antrian email');
			while($row = pg_fetch_array($query)){
				$n[ $row['kode_kirim'] ] = $row['jumlah'];
				$n_xyz += $row['jumlah'];
			}
			
			
			if ($n_xyz>$total_email){
			
				if ( $n[4] < $n[2] )
				{
					$total_4 = $n_xyz * ( $persen['4']/100 );
					$total_4 = ceil($total_4);
					
					if ( $n[4] < $total_4 )
					{
						$limit_aws = $total_4 - $n[4];
						$sql_aws = "		
						UPDATE antrian_email
						   SET kode_kirim=4
						 WHERE loading_id = $m_loading_id and kode_kirim = 2
						 and nomor_rekening IN
						 (
							select nomor_rekening from antrian_email 
							WHERE loading_id = $m_loading_id and kode_kirim =2 
							and lower(email) like '%@gmail%'
							order by antrian_id DESC 
							limit $limit_aws
						 )	
						";
						@pg_query($sql_aws);
				...
				if ( $n[3] < $n[2] )
				{
					$total_3 = $n_xyz * ( $persen['3']/100 );
					$total_3 = ceil($total_3);
					
					if ( $n[3] < $total_3 )
					{
						$limit_aws = $total_3 - $n[3];
						$sql_aws = "		
						UPDATE antrian_email
						   SET kode_kirim=3
						 WHERE loading_id = $m_loading_id and kode_kirim = 2
		...
		UPDATE antrian_email
		   SET kode_kirim=2
		 WHERE loading_id = $m_loading_id
		 --AND ( kode_kirim = 3 OR kode_kirim = 4)
		 AND lower(email)  
		 IN 
		 (
			SELECT lower(email) FROM _suppression_list_aws
			WHERE lower(keterangan) like lower('%Suppression%')
		 );";
		$query = pg_query($sql) or die('ERROR: Update SUppression email');
		*/
	}
	...
	function template_email($flagtrans){
		$sql_template = "SELECT template_email_id,nama_template FROM template_email WHERE flagtrans='$flagtrans' and status ='t' ORDER BY template_email_id";
	...
	function ubah_cyber_ke_aws_log_bulan_lalu($m_loading_id)
	{
		//merubah gmail yang read sebelumnya status kirim aws 4/3  (default gmail itu 2)
		$sql_aws = "		
						UPDATE antrian_email
						   SET kode_kirim=4
						 WHERE loading_id = $m_loading_id
						 ...
						 (
							SELECT nomor_rekening
							  FROM _tr_email_log_bulan_lalu
							  where lower(email) like '%@gmail%' and tgl_read IS NOT NULL
							  and (kode_kirim =3 OR kode_kirim =4)
		...
		//merubah yang gagal email ke cyber (2) seluruhnya
		$sql_aws = "		
						UPDATE antrian_email
						   SET kode_kirim=2
						 WHERE loading_id = $m_loading_id
						 ...
						 (
							SELECT lower(email)
							  FROM _tr_email_log_bulan_lalu
							  where date_email_callback IS NOT NULL
