...
<?
	...
	switch($act){
		...
		case 'group_sample'	: group_sample($blth,$flagtrans,$file); break;
	}
	
	function blth($flagtrans){
	?>
		<select name="blth" id="blth" class="combobox" onchange="
            	Loaddiv('blth_record','script_kirim_antrian_sk.php','flagtrans=<?=$flagtrans?>'+'&act=blth_record&blth='+this.value);
                Loaddiv('group_sample','script_kirim_antrian_sk.php','flagtrans=<?=$flagtrans?>'+'&act=group_sample&blth='+this.value);
				...
				$('#div_attach2').removeAttr('disabled');
				cek_attach();
				oncheck_file();
                cekFile();
                cekBlth();
	...
	?>
		<select name="nama_file" id="nama_file" class="combobox" onchange="
        	Loaddiv('nama_file_record','script_kirim_antrian_sk.php','flagtrans=<?=$flagtrans?>'+'&act=filerecord&blth=<?=$blth?>&file='+this.value);
            Loaddiv('group_sample','script_kirim_antrian_sk.php','flagtrans=<?=$flagtrans?>'+'&act=group_sample&blth=<?=$blth?>&file='+this.value);
	...
	function norek($ord,$blth,$flagtrans,$file){
		...
		$flagtrans	= $_REQUEST['flagtrans'];
		$file		= $_REQUEST['file'];
		$nocard_selectif		= $_REQUEST['nocard_selectif'];
		...
		//where upper(nama) in ('IKHSAN SEPTIANSYAH','HUSIN LIM','ISMAN RIVANI')
		//SELECT DISTINCT nomor_rekening, nama, jml_hlm FROM detail_112020_BC WHERE blth = '112020' and flagtrans = 'BC' AND Wnomor_rekening IN ('4365020133516707','4365020230786708') and nama_file = 'EST_BRIC151120.DAT' AND email!='' ORDER BY jml_hlm DESC
		$nocard_selectif_y = '';
		if ( !empty($nocard_selectif) )
		{
			$nocard_selectif_y .= "and (nomor_rekening ='$nocard_selectif' or lower(nomor_customer) like lower('%$nocard_selectif%') )";
		}

		
				$sql_norek = "SELECT DISTINCT nomor_rekening, nama, jml_hlm
								FROM $tabeldetail
								WHERE blth = '$blth' and flagtrans = '$flagtrans' and nama_file = '$file' $wh_em 
								$nocard_selectif_y
								$urut";
								#echo $sql_norek;
		?>
