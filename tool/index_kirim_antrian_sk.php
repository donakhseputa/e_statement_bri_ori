    ...
    <script>
		...
		function kirim(){
			...
			if(cek==true){
				...
				$.post('script_kirim_antrian_sk.php?act=kirim',$("#form_antrian").serialize(),function(respon){
		...
		function oncheck_file(){
			if($("#checkbox_nama_file").attr('checked')==true){
				var pr = $("#pr").val();
				var blth = $("#blth").val();
				Loaddiv('div_file_loading','script_kirim_antrian_sk.php','act=nama_file&blth='+blth+'&flagtrans=<?=$flagtrans?>');
				Loaddiv('div_file_customer','script_kirim_antrian_sk.php','act=file_customer&blth='+blth+'&flagtrans=<?=$flagtrans?>');
			}else{
				$("#nama_file").val('');
		...
		function oncheck_norek(){
			if($("#checkbox_norek").attr('checked')==true){
				...
				var nocard_selectif = $("#div_attach2").val();
				var arr_pr = pr.split('|');
				$("#text_sample_email").val('');
				Loaddiv('div_norek','script_kirim_antrian_sk.php','flagtrans=<?=$flagtrans?>&act=norek&ord='+pilihan_order+'&blth='+blth+'&file='+nama_file+'&nocard_selectif='+nocard_selectif);
			}else{
				$("#norek").val('');
				$("#norek").attr('disabled','disabled');
				clear_sample();
		...
		function goto(pr){
			window.location.href = "index_kirim_antrian_sk.php?pr="+pr;
		}
		...
		function attach_file(){
			...
			Loaddiv('div_attach','script_kirim_antrian_sk.php','flagtrans=<?=$flagtrans?>&act=attach_file&blth='+blth+'&nama_file='+nama_file);
			$("#div_attach").show(500);
		}
		...
		function email(){
			...
			if(norek=='' || $("#checkbox_norek").attr('checked')==false){
				...
				Loaddiv('email_address','script_kirim_antrian_sk.php','flagtrans=<?=$flagtrans?>&act=email&blth='+blth+'&nama_file='+nama_file+'&norek='+norek);
			}
			show_sample();
		}
		...
		function func_sample(){
			if($("#check_sample").attr('checked')==false){
				...
				Loaddiv('email_sample','script_kirim_antrian_sk.php','act=sample');
			}
		}
		
		function template_name(){
			var template_id = $("#combo_template").val();
			Loaddiv('div_template_name','script_kirim_antrian_sk.php','act=template_name&template_id='+template_id);
		}
                                    	...
                                    	<tr>
                                            ...
                                            <td valign="top">
                                                <table>
                                                	...
                                                	<tr>
			                                        	<td width="30%" height="14">Status</td>
                                                	...
                                                	<tr>
														<td height="14">Selectif Attachment Auto</td>
                                                	  <td width="10%"><input type="checkbox" id="check_attach_auto" name="check_attach_auto" checked="checked" /></td>
                                                	  <td>Uncheck untuk tidak kirim selectif attachment auto</td>
                                                    </tr>
                                                    <tr>
													...
													<tr>
                                                    	<td height="50" valign="top">Sample NoCard Tertentu</td>
                                                        <td valign="top">
															&nbsp;
                                                        </td>
                                                    	<td>
															<textarea id="div_attach2" name="div_attach2" cols="25" rows="30" class="textarea_loading" style="height:60px;" disabled="disabled"></textarea>
															</td>
                                                    </tr>
                                                </table>
                                            </td>
...
<script>
	Loaddiv('div_blth','script_kirim_antrian_sk.php','act=blth&flagtrans=<?=$flagtrans?>');
	Loaddiv('div_template','script_kirim_antrian_sk.php','act=template_email&flagtrans=<?=$flagtrans?>');
	cek_status();
</script>
<?php
	@pg_close($con);
