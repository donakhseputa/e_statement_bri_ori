			...
			foreach($sheetData as $row => $columns) 
			{
				...
				if($row != 1) 
				{
					...
					foreach($columns as $cell => $val) 
					{
						...
						if ($cell == 'Q' || $cell == 'R' || $cell == 'S') continue;
	...
	*/
	...
	function to_pdf($flagtrans,$blth,$file,$con,$temp,$jenis_pdf,$tabeldetail,$m_loading_id, $tanggal_nama_file='')
	{
		...
		while($row = pg_fetch_array($qry)){
			...
			#$xy1 = cek_sudah_terkirim($nomor_rekening);
			#if ($xy1 == 'kosong' ) continue;
			...
			{ 
				...
				*/
				...
				$html = '<span style="text-align:justify;">Sehubungan dengan adanya Program Penutupan Kartu Kredit BRI secara otomatis oleh Pihak Bank, bersama ini kami sampaikan, bahwa Kartu Kredit BRI atas nama : </span>';
				// output the HTML content
				$pdf->writeHTML($html, true, 0, true, true);
				...
				$pdf->SetFont('helvetica', '', $fontsize1);
					...
					$pdf->SetLeftMargin(20);
					$pdf->SetRightMargin(20);
					$pdf->SetXY($x, $y+(20*$jeda_baris) -1);
					$html = '<span style="text-align:justify;">Telah dillakukan penutupan secara otomatis  per tanggal <b>'.$tgl_close_card.'</b>. Hal ini bertujuan untuk meminimalisir risiko penyalahgunaan kartu kredit yang sudah terlalu lama tidak aktif dan memberikan kenyamanan kepada nasabah.</span>';
				// output the HTML content
					$pdf->writeHTML($html, true, 0, true, true);
					...
					$pdf->SetLeftMargin(20);
					$pdf->SetRightMargin(20);
					$pdf->SetXY($x, $y+(21*$jeda_baris));
					$html = '<span style="text-align:justify;">
					Namun, apabila setelah tanggal penutupan kartu terdapat tagihan transaksi, maka akan dilakukan klarifikasi dan penyelesaian sesuai dengan ketentuan BRI. Sehubungan dengan hal tersebut, apabila masih terdapat fisik kartu sebagaimana dimaksud, mohon dapat segera digunting/dihancurkan.
					</span>';
					// output the HTML content
					#$pdf->writeHTML($html, true, 0, true, true);
					...
					$pdf->SetLeftMargin(20);
					$pdf->SetRightMargin(20);
					$pdf->SetXY($x, $y+(23*$jeda_baris));
					$html = '<span style="text-align:justify;">Namun, apabila setelah tanggal penutupan kartu terdapat tagihan transaksi, maka akan dilakukan klarifikasi dan penyelesaian sesuai dengan ketentuan BRI. Sehubungan dengan hal tersebut, apabila masih terdapat fisik kartu sebagaimana dimaksud, mohon dapat segera digunting/dihancurkan.</span>';
				// output the HTML content
					$pdf->writeHTML($html, true, 0, true, true);
				...
				$pdf->SetFont('helvetica', '', $fontsize1);
				$pdf->SetXY($x, $y+(49*$jeda_baris));
				$pdf->Cell(10, 10, '------------------------------------------------------------------------------------------------------------------------------------------------------------', 0, 0, "L");
				
				$pdf->SetFont('helvetica', 'I', $fontsize1);
				$pdf->SetXY($x+30, $y+(50*$jeda_baris));
				...
				$sql_ins_detail = "INSERT INTO $tabeldetail(
								...
								'$flagtrans','$blth','$file',
								E'$nama_file_pdf','$password_pdf',$jml_hlm,
								E'".addslashes($nama_produk)."','$tanggal_template','$email',$n_email,$size_pdf
							)";
...
INSERT INTO log_temp_excel_pk(
	...
	function cek_sudah_terkirim($nomor_rekening)
	{
				$sql_cek_dat = "
				select *  from (

select nomor_rekening from
(
select a.*, b.nomor_rekening as cc, c.nomor_rekening as dd from detail_082018_pk a
LEFT JOIN (select * from tr_email where  loading_id=1786 and status_sample='f') b ON a.nomor_rekening=b.nomor_rekening
LEFT JOIN (select * from antrian_email where  loading_id=1786 ) c ON a.nomor_rekening=c.nomor_rekening
where a.m_loading_id=1786 
AND b.nomor_rekening IS NULL and c.nomor_rekening IS NULL 

) x



) xx where nomor_rekening='$nomor_rekening'
";
				$qry_cek_dat = pg_query($sql_cek_dat) or die('ERROR cek m_loading: '.$sql_cek_dat);
				$jml = pg_num_rows($qry_cek_dat);
				if($jml == 0){
