...
<?
	...
	function insert_mcustomer($no_card, $email, 
	...
	function create_csv($m_loading_id,$blth,$file){
		...
		$header = "'nomor_rekening';'nama';'pdf_name'".chr(13);
		$detail = "";
		
		$sql = "SELECT nomor_rekening,nama,pdf_name FROM detail WHERE m_loading_id = $m_loading_id";
		...
		while($row = pg_fetch_array($qry)){
			...
			$detail .= "'".$nomor_rekening."';'".$nama."';'".$pdf_name."'".chr(13);
		}
		...
		while(!feof($open_dat)){
			...
			if(trim($row_dat)!=''){
				...
				if ($flag_case == 'Y' && $regtype == '01')
				{
					...
					insert_mcustomer($no_card_dat, $email, 
