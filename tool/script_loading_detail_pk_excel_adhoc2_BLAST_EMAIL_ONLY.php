	...
	*/
	...
	function to_pdf($flagtrans,$blth,$file,$con,$temp,$jenis_pdf,$tabeldetail,$m_loading_id, $tanggal_nama_file='')
	{
		...
		while($row = pg_fetch_array($qry)){
			...
			$sql_cek_dat = "
select * from tr_email where loading_id = 2000 and status_sample ='f' and nomor_rekening= '$nomor_rekening'";
				$qry_cek_dat = pg_query($sql_cek_dat) or die('ERROR cek listing dikirim: '.$sql_cek_dat);
				$jml = pg_num_rows($qry_cek_dat);
				if($jml == 0){
					continue;
				}
