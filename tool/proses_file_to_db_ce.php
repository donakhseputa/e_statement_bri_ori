<?php
	/*
	/E/:/_CONVERT_BRI_CETAK_EMATERAI/
	/C/:/app/htdocs/_CONVERT_BRI_CETAK_EMATERAI/
	
	script_loading_detail_ce_excel.php
	proses_file_to_db_ce.php
	
	index_loading_detail_ce.php
	...
	*/
	...
	switch($act){
		...
		case 'cek_proses_ce'			: cek_proses_ce(); break;
	...
	function cek_proses_ce()
	{
		$sql = "SELECT * FROM master_proses_after_ematerai_produksi where keterangan='selesai'
		ORDER BY id ASC";
		$query = pg_query($sql) or die('ERROR cek cek_proses_ce: '.$sql);
	...
	function file_to_db( $hidden_dir_1, $hidden_dir_2 )
	{
		$sql		= "TRUNCATE TABLE master_proses_after_ematerai_file_ce_from_bri;
						...
						UPDATE master_proses_after_ematerai_produksi SET keterangan='off', waktu=NULL, waktu_selesai=NULL;
";
		$query 		= pg_query($sql);
...
//$x = array('Status 1','Status 2','Status 3','Status 4', 'Status 5');
		...
		if($jml > 0){
			...
			while($row = pg_fetch_array($query)){
					...
					if(file_exists( $direktori.$file_out )){
						...
						$command = 'C:/pdftk/bin/pdftk ' . $direktori.$file_out5 . ' multistamp ' . 'C:/app/htdocs/_CONVERT_BRI_CETAK_EMATERAI/_master/MASTER_TUTUP_LOGO.PDF output '. $direktori.$file_out6; 
						exec($command);
						...
						//menutup LOGO
						$command = 'C:/pdftk/bin/pdftk C:/app/htdocs/_CONVERT_BRI_CETAK_EMATERAI/_master/MASTER_PRODUKSI.PDF '. ' multistamp ' .$direktori.$file_out6. ' output '. $direktori.$file_out7; 
						exec($command);
	...
	{
		$master_pdf_kosong = 'C:/app/htdocs/_CONVERT_BRI_CETAK_EMATERAI/_master/MASTER_PRODUKSI.PDF';
		$dir_final_stamp = 'C:/app/htdocs/_CONVERT_BRI_CETAK_EMATERAI/_FINAL_PDF/';
		$dir_awal_produksi = 'C:/app/htdocs/_CONVERT_BRI_CETAK_EMATERAI/PDF_PRODUKSI/';
		$array_data=array();
		...
		if($jml > 0){
			...
			while($row = pg_fetch_array($query)){
					...
					if($jml2 > 0){
						while($row2 = pg_fetch_array($query2)){
							...
							copy( 'C:/app/htdocs/_CONVERT_BRI_CETAK_EMATERAI/_master/MASTER_TUTUP_LOGO_XYZ.PDF' , $pdfLoc_xyz.$nama_file_pdf_xyz);
						}
					}else{
						$nama_file_pdf			=  str_pad($mulai, 5, 0, STR_PAD_LEFT).'_'.'SPLIT_EMATERAI_KOSONG_'.$cif.'.PDF';
						$nama_file_pdf_xyz		=  str_pad($mulai, 5, 0, STR_PAD_LEFT).'_'.'XYZ_SPLIT_EMATERAI_KOSONG_'.$cif.'_'.$tgl_cycle.'.PDF';
			...
			foreach( $array_data as $key => $value ) 
			{
				...
				//stamp dengan master
				$status_baris_lebih_besar_1200_1 = explode('/', $file_siap_stamp);
				$status_baris_lebih_besar_1200 = $status_baris_lebih_besar_1200_1[5].'.TXT';
				//////////////////////////////////////////////////////////////
				$sql_1200 = "SELECT selesai FROM master_proses_after_ematerai_file_ce_from_produksi_ocr_summary where nama_file='$status_baris_lebih_besar_1200' ORDER BY id DESC LIMIT 1";
				$query_1200 = pg_query($sql_1200) or die('ERROR cek status_baris_lebih_besar_1200: '.$sql_1200);
				$jml_1200 = @pg_num_rows($query_1200);
				if($jml_1200 > 0){
					//$array_data['rincian'] = '';
					while($row_1200 = pg_fetch_array($query_1200)){
						$total_halaman_1200 = $row_1200['selesai'];
					}
				}else{
					$total_halaman_1200 = 0;
				}
				
				$jumlah_halaman_pdft_error = 800;
				//////////////////////////////////////////////////////////////
				
				$command = 'C:/pdftk/bin/pdftk ' . $file_siap_stamp . ' multistamp ' . $file_awal_produksi.' output '.$file_finalisasi_stamp; 
				
				$command_lokal = '"C:/Program Files/PDFtk Server/bin/pdftk.exe" ' . 'C:/nano/PDF_KURANG_LAPOR_APPDEV/'.$array_nama_siap_stamp[$key] . ' multistamp ' . 'C:/nano/PDF_KURANG_LAPOR_APPDEV/'.$array_nama_file_awal[$key].' output '.'C:/nano/PDF_KURANG_LAPOR_APPDEV/'.$array_data_final_stamp[$key]; 
				
				if ( $total_halaman_1200 > $jumlah_halaman_pdft_error)
				{
					
					$lokasi_kurang_pdf = $dir_final_stamp.'PDF_KURANG_LAPOR_APPDEV/';
					if(!file_exists($lokasi_kurang_pdf)) mkdir($lokasi_kurang_pdf)or die('Error mkdir '.$lokasi_kurang_pdf);
					
					//_list_script_pdftk
					$sql_list_script_pdftk = "INSERT INTO _list_script_pdftk(
						script_pdftk, waktu, script_pdftk_lokal)
					VALUES ( '$command', now(), '$command_lokal');";
					pg_query($sql_list_script_pdftk) or die('ERROR cek _list_script_pdftk: '.$sql_list_script_pdftk);
					
					
					copy( $file_siap_stamp , $lokasi_kurang_pdf.$array_nama_siap_stamp[$key]);
					copy( $file_awal_produksi , $lokasi_kurang_pdf.$array_nama_file_awal[$key]);
					
					$lokasi_txt_log_file_txt = $lokasi_kurang_pdf.str_replace('.PDF','_SCRIPT_PDFTK.TXT',$array_data_final_stamp[$key]);
					cek_log_file($lokasi_txt_log_file_txt, $command_lokal);
				...
				$command_lokal = '"C:/Program Files/PDFtk Server/bin/pdftk.exe" ' . 'C:/nano/PDF_KURANG_LAPOR_APPDEV/'.$array_data_final_stamp[$key] . ' multistamp ' . 'C:/nano/PDF_KURANG_LAPOR_APPDEV/'.$array_nama_siap_stamp_xyz[$key].' output '.'C:/nano/PDF_KURANG_LAPOR_APPDEV/'.$array_data_final_stamp_fix[$key]; 
				if ( $total_halaman_1200 > $jumlah_halaman_pdft_error)
				{
					//_list_script_pdftk
					$sql_list_script_pdftk = "INSERT INTO _list_script_pdftk(
					...
					copy( $file_siap_stamp_xyz , $lokasi_kurang_pdf.$array_nama_siap_stamp_xyz[$key]);
	...
	function update_proses($n, $status)
	{
		...
		$sql = "UPDATE master_proses_after_ematerai_produksi
			...
			$sql = "UPDATE master_proses_after_ematerai_produksi
			SET keterangan='$status' , waktu_selesai=now()
			WHERE id = $n";
		}
...
function cek_masih_running($filePath)
{
	...
	*/
	...
	function cek_log_file($direktori, $kata)
	{
		
		
		//$folder = 'log_read/';
		//$folder = 'log_read_folder_file/';
		//if(file_exists($folder)==false) mkdir($folder);
		
		//$nama_file_tanggal = date("YmdH");
		//$filename = $folder.'log_read_file_'.$nama_file_tanggal.'.txt';
		$filename = $direktori;
		$fh = fopen($filename,'a+');
			$kata = $kata."\r\n"."\r\n";	
			fwrite($fh,$kata);
