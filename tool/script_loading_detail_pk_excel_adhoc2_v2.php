				...
				if($row != 1) 
				{
					...
					foreach($columns as $cell => $val) 
					{
						...
						if ($cell == 'W'){
							...
							$abcde = str_replace(' ','_',trim($val) );
							$value[] = str_replace("'",'_',trim($abcde) );
						}else{
							$value[] = addslashes(str_replace( array('(',')'), '' , $val) ) ;
						}
		...
		while($row = pg_fetch_array($qry)){
			...
			{ 
				...
				*/
				...
				$html = '<span style="text-align:justify;">Dengan ini kami informasikan bahwa <b>Kartu Kredit BRI '.$tipe_kartu.'</b> Anda sudah tidak aktif dan tidak dapat digunakan. Demi memberikan keamanan dan kenyamanan kepada Anda, bersama ini kami sampaikan bahwa Kartu Kredit BRI Anda dengan rincian:</span>';
				// output the HTML content
				$pdf->writeHTML($html, true, 0, true, true);
				...
				// output the HTML content
					...
					$html = '<span style="text-align:justify;">Apabila Anda masih berminat untuk menggunakan kembali Kartu Kredit BRI Anda. Anda dapat melakukan pengajuan ulang sebagaimana pengajuan aplikasi Kartu Kredit BRI bagi nasabah baru.</span>';
				// output the HTML content
					$pdf->writeHTML($html, true, 0, true, true);
				...
				$html = '<span style="text-align:justify;">Demikian informasi yang dapat kami sampaikan, mohon agar surat ini dapat dipergunakan dengan semestinya. Terima kasih atas kepercayaan yang selama ini Anda berikan kepada Bank BRI.</span>';
				// output the HTML content
				$pdf->writeHTML($html, true, 0, true, true);
				...
				$pdf->Cell(10, 10, 'Card & Digital Lending Division', 0, 0, "C");
				
				$pdf->SetXY($x+120, $y+((51.5+$v_turun)*$jeda_baris));
				$pdf->Cell(10, 10, 'Executive Vice President', 0, 0, "C");
				
				
				$url_img_produk = '../images/barcode_n3.jpg';
				...
				$pdf->SetFont('helvetica', '', $fontsize1);
				$pdf->SetXY($x, $y+(60*$jeda_baris));
				$pdf->Cell(10, 10, '------------------------------------------------------------------------------------------------------------------------------------------------------------', 0, 0, "L");
				
				$pdf->SetFont('helvetica', 'I', $fontsize1);
				$pdf->SetXY($x+30, $y+(61*$jeda_baris));
				...
				$sql_ins_detail = "INSERT INTO $tabeldetail(
								...
								E'".addslashes($address3)."',
								E'".addslashes($city)."',
								'$zipcode',
								'$flagtrans','$blth','$file',
								'$nama_file_pdf','$password_pdf',$jml_hlm,
								'".addslashes($nama_produk)."','$tipe_kartu','$email',$n_email,$size_pdf
							)";
