...
<? #define('FPDF_FONTPATH','../include/fpdf184/font/'); ?>
<? #require_once("../include/fpdf184/fpdf.php"); ?>
<? #require_once("../include/fpdi/FPDI_Protection.php"); ?>

<? #require_once("../include/fpdi8/src/Fpdi.php"); ?>

<? require_once("../include/tcpdf8/examples/tcpdf_include.php"); ?>
<?
	...
	function dat_to_pdf($flagtrans,$menu_id,$blth,$file){
			...
			if(!$exe_tabel){
				$buat_tabel = "CREATE TABLE $tabeldetail(						
								 detail_id serial NOT NULL,
								  ...
								  tanggal text,
								  total_produk text,
								  kode_cab character varying(50),
								  cabang character varying(50),
								  
								   total_ematerai text,
								  jumlah_kartu_ematerai text,
								  tipe_proses_ematerai text,
								  password_admin_pdf text,
								  
								  CONSTRAINT $tabeldetail_pk PRIMARY KEY (detail_id),
								   CONSTRAINT $tabeldetail_un UNIQUE (nomor_rekening , pdf_name )
		...
		*/
		...
		#$nomor_kartu_all = '';
		#$nomor_kartu_all_cover = '';
		$nomor_kartu_all_cover = array();
		$flag_bea_materai_lunas = array();
		...
		while(!feof($open_dat)){
			...
			if(trim($row_dat)!=''){
				...
				if ($regtype == '0X' )
				{
					$password_admin_pdf = substr($row_dat,1518,13);
					$password_admin_pdf = trim($password_admin_pdf);
					if (empty( $password_admin_pdf ) ) die("ERROR 0X TIDAK ADA".$row_dat);
					$folder_cycle = substr($password_admin_pdf,-8);
					
					$pdf_name_CIF_x = substr($row_dat,1488,29);
					$pdf_name_CIF_x = trim($pdf_name_CIF_x);
				...
				if ($regtype == '01' )
				{
					...
					if ( $email_01 == '' ) $email_01 = 'TIDAK ADA';
					if(trim($email_01) == "")
					{
					...
					$OLS_CURR_BAL 				= number_format($OLS_CURR_BAL,0,'','.');
					...
					$CR_LIMIT 					= number_format($CR_LIMIT,0,'','.');
					...
					$OLS_TOT_AMOUNT_DUE 		= number_format($OLS_TOT_AMOUNT_DUE,0,'','.');
					...
					$CR_CASH_LINE 				= number_format($CR_CASH_LINE,0,'','.');
					...
					$CR_AVAIL_CASH 				= number_format($CR_AVAIL_CASH,0,'','.');
					...
					$CR_AVAIL_CREDIT 			= number_format($CR_AVAIL_CREDIT,0,'','.');
					...
					if ( substr($OLS_CUST_NBR,0,8) == "43597201" || substr($OLS_CUST_NBR,0,8) == "43597202") //Hana bank Platinum - 20160727 Bowo --> telah berubah menjadi tokopedia 20211125
					{
					...
					*/
					...
					$text1_cover_summary = pesan_wording($OLS_CUST_NBR);
					///////////////////////////////////////////////////////////////////// PESAN KOTAK SUMMARY ///////////////////////////////////////////		
					///////////////////////////////////////////////////////////////////// PESAN KOTAK SUMMARY ///////////////////////////////////////////	
					...
					//$OLS_PYMT_DUE_DATE_x_materai = substr($OLS_PYMT_DUE_DATE,6,2).''.substr($OLS_PYMT_DUE_DATE,4,2).''.substr($OLS_PYMT_DUE_DATE,0,4);
					//$OLS_PYMT_DUE_DATE_x = '';
					$statement_date_summary_ematerai = str_replace('-','',$statement_date_summary);
					
					$tagihan_baru_summary = substr($row_dat,413+$var_cover_summary,14);
					#$tagihan_baru_summary_int += $tagihan_baru_summary;
					$tagihan_baru_summary_int = $OLS_CURR_BAL;
					$tagihan_baru_summary_int_insert_db = $OLS_CURR_BAL;
					
					$pembayaran_minimum_summary = substr($row_dat,263+$var_cover_summary,14);
					#$pembayaran_minimum_summary_int += $pembayaran_minimum_summary;
					...
					////////////////////////////////////////////////////////////////////////////////////
					if ( !isset($nomor_kartu_all_cover[$OLS_CUST_NBR]))
					{
						$nomor_kartu_all_cover[$OLS_CUST_NBR] = $no_card_dat_summary.' / ';
					}else{
						$nomor_kartu_all_cover[$OLS_CUST_NBR] .= $no_card_dat_summary.' / ';
					}
					////////////////////////////////////////////////////////////////////////////////////
					...
					$credit_limit_summary 		= number_format($credit_limit_summary,0,'','.');//perubahan pemisah desimal titik
					...
					$avail_credit_limit_summary 		= number_format($avail_credit_limit_summary,0,'','.');//perubahan pemisah desimal titik
					...
					$min_payment_summary 		= number_format($min_payment_summary,0,'','.');//perubahan pemisah desimal titik
					
					
					$new_balance_summary = number_format(substr($row_dat,413+$var_cover_summary,14),0,'','.');//perubahan pemisah desimal titik
					...
					////////////////////////////////////////////////////// PENAMBAHAN ERROR JJIKA DI HALAMAN DI PRIN 1 TOGAL TAGIHAN 20220517
					//MEMBUAT Kolektibilitas mengikuti data sebelumnya
					$sblmnya_barcode = NULL;
					$sblmnya_posko = NULL;
					$sblmnya_kurir = NULL;
					$sblmnya_nourut = NULL;
					$sblmnya_kolektibilitas = NULL;
					
					$sblmnya_barcode = $barcode;
					$sblmnya_posko = $posko;
					$sblmnya_kurir = $kurir;
					$sblmnya_nourut = $nourut;
					$sblmnya_kolektibilitas = $kolektibilitas;
					$flag_halaman_1baris_aja = '0';
					////////////////////////////////////////////////////// PENAMBAHAN ERROR JJIKA DI HALAMAN DI PRIN 1 TOGAL TAGIHAN 20220517
					...
					/////////////////////////////////////////////////////////////////////////////////////// kz
					//E-MATERAI 20220615
						$flag_bea_materai_lunas[$OLS_CUST_NBR]['total_ematerai']	= 0;
						$flag_bea_materai_lunas[$OLS_CUST_NBR]['jumlah_kartu_ematerai']	= '';
					//E-MATERAI 20220615
					/////////////////////////////////////////////////////////////////////////////////////// kz
					/////////////////////////////////////////////////////////////////////////////////////// kz
					...
					if ( preg_match("/55200233/i", $nomor_kartu_all_cover[$OLS_CUST_NBR])  )
					{
						$url_pdf_jpg_summary = "../images/Layout_C1_Summary_ovo.jpg";
					}else if ( preg_match("/43597201/i", $nomor_kartu_all_cover[$OLS_CUST_NBR])  ){
						$url_pdf_jpg_summary = "../images/Layout_C1_Summary_tokopedia.jpg";
					}else if ( preg_match("/43597202/i", $nomor_kartu_all_cover[$OLS_CUST_NBR])  ){
						$url_pdf_jpg_summary = "../images/Layout_C1_Summary_tokopedia.jpg";
					}else if ( preg_match("/55200205/i", $nomor_kartu_all_cover[$OLS_CUST_NBR])  ){
						$url_pdf_jpg_summary = "../images/Layout_C1_Summary_nexcard.jpg";	
					}else if ( preg_match("/43596503/i", $nomor_kartu_all_cover[$OLS_CUST_NBR])  ){
						$url_pdf_jpg_summary = "../images/Layout_C1_Summary_samsung.jpg";	
					}else if ( preg_match("/43596504/i", $nomor_kartu_all_cover[$OLS_CUST_NBR])  ){
						$url_pdf_jpg_summary = "../images/Layout_C1_Summary_samsung.jpg";	
					}else if ( preg_match("/43368300/i", $nomor_kartu_all_cover[$OLS_CUST_NBR])  ){
						$url_pdf_jpg_summary = "../images/Layout_C1_Summary_paperid.jpg";	
					}else if ( preg_match("/40557700/i", $nomor_kartu_all_cover[$OLS_CUST_NBR])  ){
						$url_pdf_jpg_summary = "../images/Layout_C1_Summary_paperid.jpg";
					}else{
						$url_pdf_jpg_summary = "../images/Layout_C1_Summary.jpg";
					}
					$url_pdf_jpg_ematerai = "../images/Layout_C1_Ematerai.jpg";
					#$url_pdf_jpg_ematerai = "../images/5188560268955206_30032022_ematerai_final_xyz_001.jpg";
...
if ( $flag_email_kosong == FALSE){
				...
				if ($regtype == '04' )
				{
					...
					$pdf_name_CIF = $pdf_name_CIF_x;
					#####$pdf_name = str_pad($customer, 6, "0", STR_PAD_LEFT).'_'.$pdf_name_CIF;
					$pdf_name = $pdf_name_CIF;
				...
				###if ($flag_case == 'Y' && $regtype == '02' && ( $GLOBALS['flag_hana_bank']  === true || $GLOBALS['flag_hana_bank_2']  === true ) )
				{
					...
					if( ($record >= ($hlm*$max_record)-5 )  ){
						...
						////////////////////////////////////////////////////// PENAMBAHAN ERROR JJIKA DI HALAMAN DI PRIN 1 TOGAL TAGIHAN 20220517
						if ($sblmnya_kolektibilitas != $kolektibilitas)
						{
							$flag_halaman_1baris_aja = '1';
						}
						$barcode		= $sblmnya_barcode;
						$posko			= $sblmnya_posko;
						$kurir			= $sblmnya_kurir;
						$nourut			= $sblmnya_nourut;
						$kolektibilitas	= $sblmnya_kolektibilitas;
					...
					//nama file ematerai
					#####$pdf_name=null;
					#####$pdf_name = str_pad($customer, 6, "0", STR_PAD_LEFT).'_'.$nama_pdf_2_cif.'_'.$statement_date_summary_ematerai;
					//////////////////////////////////////////////
					#die("bb");
					#die("$email - $baris");
					...
					if ( ($key_CIF != $key_CIF_x && $key_CIF != '') )
					{
						...
						output_pdf($pdf,$pdf_name,$flagtrans,$blth,$file, $folder_cycle,$folder_tipe,$nama_pdf_2_cif);
						#$yyyyyy = "AAAAA ";
					}
					...
					insert_detail($m_loading_id,$nomor_kartu_all_cover,$nama_pdf_2_cif,
									$nama,$address1,$address2,
									$address3,$city,$zipcode,
									$flagtrans,$blth,$file,
									$pdf_name,$password_pdf,$hlm_tabel, $email, $tabeldetail, $ket_produk
									,$flag_halaman_1baris_aja
									,$flag_bea_materai_lunas
									,$password_admin_pdf
									,$folder_cycle,$folder_tipe
									,$tagihan_baru_summary_int_insert_db);
				...
				if($regtype == '02'  ){
					...
					$GLOBALS['flag_bri_ovo']  = false;
					$GLOBALS['flag_bri_pegadaian']  = false;
					$GLOBALS['flag_bri_traveloka']  = false;
					$GLOBALS['flag_bri_tokopedia']  = false;
					$GLOBALS['flag_bri_nexcard']  = false;
					$GLOBALS['layout_bri_samsung']  = false;
					$GLOBALS['flag_bri_paperid']  = false;
					$GLOBALS['flag_bri_gpn']  = false;
					$GLOBALS['flag_bri_private']  = false;
					
					$GLOBALS['flag_tanpa_kotak']  = false;
					...
					if ( $email == '' ) $email = 'TIDAK ADA EMAIL';
					...
					$nomor_kartu_all .= trim($no_card_dat).'/';
					...
					if (substr($no_card_dat,0,1) == "4") //VISA
					{
						...
						if ( substr($no_card_dat,0,8) == "46874001" || substr($no_card_dat,0,8) == "46874002") //Infinite - 20160419 Bowo
						{
							...
							$url_img = "../images/Layout_C1_VISA_tanpa_kotak.jpg"; //OK
							$GLOBALS['flag_tanpa_kotak']  = true;
							
							$card_name="visa";
							$ket_produk="VISA Infinite";
							#$yimage = 3.5;//kz
							#$y = 24;//kz
						...
						else if ( substr($no_card_dat,0,8) == "43650203" ) //traveloka - 20160419 Bowo
						{
							...
							$url_img = "../images/layout_traveloka3_r1_tanpa_kotak.jpg";
							//$GLOBALS['flag_tanpa_kotak']  = false;
							$GLOBALS['flag_tanpa_kotak']  = true;
							$card_name="visa";
							$ket_produk="VISA traveloka";
							...
							$GLOBALS['flag_bri_traveloka']  = true;
						}
						else if ( substr($no_card_dat,0,8) == "43650204" ) //BRI Pegadaian - 20201123
						{
							...
							$url_img = "../images/layout_pegadaian_tanpa_kotak.jpg";
							$GLOBALS['flag_tanpa_kotak']  = true;
							
							$card_name="visa";
							$ket_produk="VISA Pegadaian";
							#$yimage = 3.5;
							#$y = 24;
							$yimage = 0;
							$y = 24;
							...
							$y_traveloka_tambahan = 2;
							$GLOBALS['flag_bri_pegadaian']  = true;
						}
						elseif ( substr($no_card_dat,0,8) == "43597201" || substr($no_card_dat,0,8) == "43597202") //Hana bank Platinum - 20160727 Bowo --> telah berubah menjadi tokopedia 20211125
						{
							/*
							//$url_pdf = '';
							...
							$url_img = "../images/layout_hana_bank_tanpa_kotak.jpg";//kz
							$GLOBALS['flag_tanpa_kotak']  = true;
							...
							*/
							...
							//BRI BTN
								...
								$url_img = "../images/layout_bri_tokopedia_r1_tanpa_kotak.jpg";
								$GLOBALS['flag_tanpa_kotak']  = true;
								
								$card_name="visa toped";
 								$ket_produk="VISA TOPED";
								#$yimage = 3.5;
								#$y = 24;
								$yimage = 0;
								$y = 24;
								...
								$GLOBALS['flag_bri_tokopedia']  = true;
								
								
								
						}
						elseif ( substr($no_card_dat,0,8) == "43368300" || substr($no_card_dat,0,8) == "40557700") //PAPER ID
						{
							...
							//PAPERID
								...
								if ( substr($no_card_dat,0,8) == "43368300"  )
								{
									$url_img = "../images/layout_paperid_spacecard_r1_tanpa_kotak.jpg";
								}else{
									$url_img = "../images/layout_paperid_universe_r1_tanpa_kotak.jpg";
								}
								...
								$card_name="visa paperid";
								$ket_produk="VISA PAPERID";
								#$yimage = 3.5;
								#$y = 24;
								$yimage = 0;
								$y = 24;
								...
								$GLOBALS['flag_bri_paperid']  = true;
								
								
								
						}
						else if ( substr($no_card_dat,0,8) == "43596503"  || substr($no_card_dat,0,8) == "43596504") //SAMSUNG - 20062024
						{
							//$url_pdf = 'Promo_Premium.pdf';
							$card_name="visa";
							$ket_produk="Visa SAMSUNG";
							$url_pdf_jpg = '';
							
							$url_img = "../images/layout_bri_samsung.jpg";							
							#$yimage = 3.5;
							#$y = 24;
							$yimage = 0;
							$y = 24;
						...
						elseif ( substr($no_card_dat,0,8) == "43596501" || substr($no_card_dat,0,8) == "43596502" ) //Hana bank - 20160727 Bowo
						{
							...
							)
							{
								...
								$url_img = "../images/layout_hana_bank_tanpa_kotak.jpg";
								$GLOBALS['flag_tanpa_kotak']  = true;
								
								$card_name="visa";
								$ket_produk="VISA Hana bank";
								#$yimage = 0;//kz
								$y = 21.5;//kz
								...
								$url_img = "../images/layout_bri_btn_r2_tanpa_kotak.jpg";
								$GLOBALS['flag_tanpa_kotak']  = true;
						...
						else //VISA
						{
							...
							$url_img = "../images/Layout_C1_VISA_tanpa_kotak.jpg";
							$GLOBALS['flag_tanpa_kotak']  = true;
					...
					elseif (substr($no_card_dat,0,1) == "5") //MASTER
					{
						if ( substr($no_card_dat,0,6) == "518828" || substr($no_card_dat,0,6) == "518856" )  // gold dan silver
						{
							...
							{
								$card_name="master";
								$ket_produk="Master World Access";
							...
							{
								$card_name="master";
								$ket_produk="Master Wonderful Indonesia";
							...
							{
								$card_name="master";
								$ket_produk="Master Promoter";
								$url_pdf_jpg = 'Promo_Promoter.jpg';
								$GLOBALS['flag_tanpa_kotak']  = true;
							}else{
								$card_name="master";
								$ket_produk="Master Easy Card";
						...
						//}elseif ( substr($no_card_dat,0,8) == "55200233" ) {  //BNP Platinum
							...
							$card_name="master";
							$ket_produk="Master Business Card";
							...
							$card_name="master";
							$ket_produk="Master Corporate Card";
						...
						else if ( substr($no_card_dat,0,8) == "55200233" ) //OVO - 20210909
						{
							//$url_pdf = 'Promo_Premium.pdf';
							$card_name="master";
							$ket_produk="Master OVO";
							$url_pdf_jpg = '';
							
							$url_img = "../images/layout_bri_ovo_r2.jpg";
							#$yimage = 3.5;
							#$y = 24;
							$yimage = 0;
							$y = 24;
							...
							$y_traveloka_tambahan = 2;
							$GLOBALS['flag_bri_ovo']  = true;
						}
						else if ( substr($no_card_dat,0,8) == "55200205" ) //NEXCARD - 11012024
						{
							//$url_pdf = 'Promo_Premium.pdf';
							$card_name="master";
							$ket_produk="Master NEXCARD";
							$url_pdf_jpg = '';
							
							$url_img = "../images/layout_bri_nexcard.jpg";							
							#$yimage = 3.5;
							#$y = 24;
							$yimage = 0;
							$y = 24;
							...
							$y_traveloka_tambahan = 2;
							$GLOBALS['flag_bri_nexcard']  = true;
						}
						else{
							$url_img = "../images/Layout_C1_MasterCard.jpg";
						}
							
						if ( $GLOBALS['flag_tanpa_kotak']  === true )
						{
							$url_img = "../images/Layout_C1_MasterCard_tanpa_kotak.jpg";
						}
					...
					elseif (substr($no_card_dat,0,1) == "3") //JCB
					{
						...
						$url_img = "../images/Layout_C1_JCB_tanpa_kotak.jpg";
						$GLOBALS['flag_tanpa_kotak']  = true;
					...
					elseif (substr($no_card_dat,0,1) == "6") //GPN
					{
						
						//Bin : 61895101 ( GPN STAFF ) 
						//Bin : 61895102 ( GPN CUSTOMER ) 						
						
							if ( substr($no_card_dat,0,8) == "61895101" || substr($no_card_dat,0,8) == "61895102" ) //JCB 
							{
								$url_pdf_jpg = '';
								if ( substr($no_card_dat,0,8) == "61895101" )
								{
									$url_pdf_jpg = '';
								}
							}else{
						...
						$url_img = "../images/Layout_C1_GPN_tanpa_kotak.jpg";
						$GLOBALS['flag_tanpa_kotak']  = true;
						...
						$GLOBALS['geser'] = 0;
						//$y_traveloka_tambahan = 2;
						$GLOBALS['flag_bri_gpn']  = true;
						
					}
					
					elseif (substr($no_card_dat,0,1) == "1") //PRIVATE LABEL
					{
						
						//Bin : 18959901
						//Bin : 18959902						
						
							if ( substr($no_card_dat,0,8) == "18959901" || substr($no_card_dat,0,8) == "18959902" ) //PRIVATE LABEL 
							{
								$url_pdf_jpg = '';
								if ( substr($no_card_dat,0,8) == "18959901" )
								{
									$url_pdf_jpg = '';
								}
							}else{
						...
						$url_img = "../images/Layout_C1_PRIV_tanpa_kotak.jpg";
						$GLOBALS['flag_tanpa_kotak']  = true;
					...
					if ( substr($no_card_dat,0,8) == "55200233" ) //OVO COVER
					{
						$url_pdf_jpg = 'Promo_Generic_OVO.jpg';
					}else if ( substr($no_card_dat,0,8) == "43597201" || substr($no_card_dat,0,8) == "43597202" ) //TOKOPEDIA - 20211116
					{
						$url_pdf_jpg = "Promo_Generic_tokopedia.jpg";
						}else if ( substr($no_card_dat,0,8) == "43368300" || substr($no_card_dat,0,8) == "40557700" ) //PAPERID - 202308221
					{
						$url_pdf_jpg = "Promo_Generic_paperid.jpg";
					}else if ( substr($no_card_dat,0,8) == "43650204" ) //BRI Pegadaian - 20201123	
					{
						$url_pdf_jpg = 'Promo_Generic_Pegadaian.jpg';
					}else if ( substr($no_card_dat,0,8) == "43650203" ) //traveloka - 20160419 Bowo
					{
						$url_pdf_jpg = 'Promo_Generic_Traveloka.jpg';
					}
					else if ( substr($no_card_dat,0,8) == "55200205" ) //nexcard - 11012024 bgz
					{
						$url_pdf_jpg = 'Promo_Generic_nexcard.jpg';
					}
					else if ( substr($no_card_dat,0,8) == "43596503" || substr($no_card_dat,0,8) == "43596504" ) //SAMSUNG - 18062024
					{
						$url_pdf_jpg = "Promo_Generic_Samsung.jpg";
					}
					else if ( substr($no_card_dat,0,8) == "61895101" || substr($no_card_dat,0,8) == "61895102" ) //GPN --25032025
					{
						$url_pdf_jpg = 'Promo_Generic.jpg';
					}
					else if ( substr($no_card_dat,0,8) == "18959901" || substr($no_card_dat,0,8) == "18959902" ) //PRIVATE LABEL --13072025
					{
						$url_pdf_jpg = 'Promo_Generic.jpg';
					}
					else{
						if ( $ket_produk == "VISA BTN" )
						{
							$url_pdf_jpg = 'Promo_Generic_BTN.jpg';
						}else{
							$url_pdf_jpg = 'Promo_Generic.jpg';
						}
						...
						switch($regtype){
							case '02':		
								...
								if ( $key_CIF != $key_CIF_x || $key_CIF == '' ){
									...
									#$pdf = new FPDI('P','mm',array(216,302));
									
									#$pdf = new TCPDF('P','mm',array(210, 297));
									#$pdf = new TCPDF('P','mm','A4');
									$pdf = new TCPDF('P','mm',array(216,302));
									...
									/*
									/////////////////////////////// materai dimatiikan passwordnya 20220617
									//ematerai dimatikan password
									#$password_admin_pdf = '@ppd3v#'.$password_pdf;
									#$password_admin_pdf = '@BRI#30032022';
									if ( $password_untuk_pdf == 'ada')
									{
										$pdf->SetProtection(array(''), $password_pdf, $password_admin_pdf);
									}
...
$kolektibilitas = NULL;
									...
									$min_payment = number_format(substr($row_dat,263,14),0,'','.');//perubahan pemisah desimal titik
									$credit_limit = number_format(substr($row_dat,278,14),0,'','.');//perubahan pemisah desimal titik
									$avail_credit_limit = number_format(substr($row_dat,293,14),0,'','.');//perubahan pemisah desimal titik
									$cash_advan_limit = number_format(substr($row_dat,308,14),0,'','.');//perubahan pemisah desimal titik
									$prev_balance = number_format(substr($row_dat,323,14),0,'','.');//perubahan pemisah desimal titik
									...
									$payment_credit = number_format(substr($row_dat,353,14),0,'','.');//perubahan pemisah desimal titik
									$interest_service_charge = number_format(substr($row_dat,398,14),0,'','.');//perubahan pemisah desimal titik
									$new_balance = number_format(substr($row_dat,413,14),0,'','.');//perubahan pemisah desimal titik
									...
									$purchase_debit = number_format(substr($row_dat,428,14),0,'','.');//perubahan pemisah desimal titik
									$cash_advance = number_format(substr($row_dat,443,14),0,'','.');//perubahan pemisah desimal titik
									
									
											//20200506 KZ
											#$retail_interest = substr($row_dat,458,5);
											...
											//20200506 KZ
											#$cash_advan_interest = substr($row_dat,464,5);
									...
									if ( $regtype == '02' )
									{
										...
										if ( $flag_summary )
										{
											#$url_pdf_jpg = '';
											//$folder_cycle,$folder_tipe
											//ditambahkan promo jika E = estatement , CE = Cetak estatement
											//untuk kebutuhan ematerai
											$password_untuk_pdf = 'ada';
											if ( preg_match("/_EST_/i", $file) || preg_match("/_E_/i", $file) )
											{
												$ket1 = add_page1_jpg($pdf, $url_pdf_jpg); //tambahan gambar 1 kz 
												$folder_tipe = "E";
											}else if ( preg_match("/_CE_/i", $file) )	{
												$ket1 = add_page1_jpg($pdf, $url_pdf_jpg); //tambahan gambar 1
												$folder_tipe = "CE";
											}else if(  preg_match("/_C_/i", $file) ){
												$folder_tipe = "C";
												$password_untuk_pdf = 'tidak ada';
											}
											...
											//////////////////////////////////////////// 20220617 ematerai
											add_halaman_ematerai_jpg($pdf, $url_pdf_jpg_ematerai, $arr_summary, $jml_hlm , $barcode, $posko, $kuri, $nourut, $kolektibilitas, $file); 
											//////////////////////////////////////////// 20220617 ematerai
											
											$halaman_cetak_kertas = add_page2_summary_jpg($pdf, $url_pdf_jpg_summary, $arr_summary, $jml_hlm , $barcode, $posko, $kuri, $nourut, $kolektibilitas, $file); 
											
											
											$halaman_cetak_kertas = $halaman_cetak_kertas+1;
...
$dr_cr_indicator = NULL;
									...
									$amount = number_format(substr($row_dat,147,14),0,'','.');//perubahan pemisah desimal titik
									...
									///////////////////////////////////////////////////////////
									//ematerai
									$flag_ematerai = trim(substr($row_dat,89,20));
									$kartu_ematerai	= substr($row_dat,27,16);
									if ( preg_match("/bea materai lunas/i", $flag_ematerai )  )
									{
										$flag_bea_materai_lunas[$OLS_CUST_NBR]['total_ematerai']++;
										$flag_bea_materai_lunas[$OLS_CUST_NBR]['jumlah_kartu_ematerai']	.= $kartu_ematerai.' / ';
									}
									///////////////////////////////////////////////////////////
									///////////////////////////////////////////////////////////
									...
									$pdf->Cell(17,4,$posting_date,0,0,'L');
									$pdf->Cell(49,4,$description,0,0,'L');
									$pdf->Cell(4,4," ".$forex_curr,0,0,'L');//20230529 forex +5
									$pdf->Cell(16,4,$forex_nominal,0,0,'R');//20230529 forex +5
									$pdf->Cell(14,4,$kurs_nominal,0,0,'R');//20230529 forex -4
		...
		###if($flag_case == 'Y' && feof($open_dat) && ( $GLOBALS['flag_hana_bank']  === true || $GLOBALS['flag_hana_bank_2']  === true ) )
		{
			...
			//////////////////////////////////////////////
			//nama file ematerai
			#####$pdf_name = str_pad($customer, 6, "0", STR_PAD_LEFT).'x_x'.$nama_pdf_2_cif.'_'.$statement_date_summary_ematerai;
			//////////////////////////////////////////////
		...
		#create_csv_ematerai($m_loading_id,$flagtrans,$blth,$file,$tabeldetail);
	...
	function add_page($pdf,
		...
		if ( !empty($url_img) )
		{
			...
			#$pdf->Image($url_img, 0, 0, 210, 297, '', '', '', false, 300, '', false, false, 0);
			$pdf->Image($url_img, 0, 0,216,302, '', '', '', false, 300, '', false, false, 0);
		...
		*/
		...
		$pdf->write1DBarcode($barcode, $font_barcode, $x_barcode-2, $y_barcode-5.5, '', $h_barcode, $w_barcode, $style, 'N');
		//////////////////////// BARCODE
		...
		///////////////////////////////////////////////////////////////////// PESAN KOTAK SUMMARY ///////////////////////////////////////////
		///////////////////////////////////////////////////////////////////// PESAN KOTAK SUMMARY ///////////////////////////////////////////			
			$message_atas = pesan_wording($kodekartu);
		///////////////////////////////////////////////////////////////////// PESAN KOTAK SUMMARY ///////////////////////////////////////////		
		///////////////////////////////////////////////////////////////////// PESAN KOTAK SUMMARY ///////////////////////////////////////////	
		...
		$pdf->MultiCell(70,3.5,$message_atas,0,'J');//20201223
		...
		switch($kolektibilitas)
		{
			...
			break;
			default:
					$teks_kolektibilitas = "LANCAR"; //perubahan dari MACET ke LANCAR 20221207
		}
		...
		else if ( substr($kodekartu,0,8) == "55200233" ) //OVO 20211004
		{
			$flag_poinx = 'false';
		}
		else if ( substr($kodekartu,0,8) == "55200205" ) //NEXCARD 11012024
		{
			$flag_poinx = 'false';
		}
		elseif ( substr($kodekartu,0,8) == "43596503" || substr($kodekartu,0,8) == "43596504") //SAMSUNG 20062024
		{	
			$flag_poinx = 'false';
		}
		...
		elseif ( substr($kodekartu,0,8) == "43597201" || substr($kodekartu,0,8) == "43597202")
		{	
			$flag_poinx = 'false';
		}
		elseif ( substr($kodekartu,0,8) == "61895101" || substr($kodekartu,0,8) == "61895102") //GPN
		{	
			$flag_poinx = 'false';
		}
		elseif ( substr($kodekartu,0,8) == "18959901" || substr($kodekartu,0,8) == "18959902") //PRIVATE LABEL
		{	
			$flag_poinx = 'false';
		}
		else{
		...
		if ($flag_poinx == 'true')
		{
			...
			if ( substr($kodekartu,0,8) == "55200233" ) //OVO - 20211004
			{
		...
		if($hlm > 0){ //KEY POINT DINYALAKAN JIKA ADA BROSUR HLM = 2 , JIKAT TIDAK ADA HLM 1 (memunculkan di semua halaman)
			...
			if ( $GLOBALS['flag_bri_traveloka']  === true )
			{
				////////////////////////////////////////////////// PENAMBAHAN TRAVELOKA
				$pdf->SetFont('BRICARD02','',14);
				$pdf->SetX($x+31-3);
				...
				////////////////////////////////////////////////// PENAMBAHAN BRI BTN
			}else if ( $GLOBALS['flag_bri_ovo']  === true ){
				////////////////////////////////////////////////// PENAMBAHAN BRI OVO
				$pdf->SetFont('BRICARD02','',14);
				$pdf->SetX($x+31-3);
				$pdf->Cell(21,5+8,'',0,0,'C');
				...
				////////////////////////////////////////////////// PENAMBAHAN BRI OVO
			}else if ( $GLOBALS['flag_bri_tokopedia']  === true ){
				////////////////////////////////////////////////// PENAMBAHAN BRI TOKOPEDIA
				//$pdf->SetFont('BRICARD02','',14);
				$pdf->SetFont('BRICARD02','',6);
				$pdf->SetX($x+10);
				//$pdf->Cell(21,5+8,$earned_bonus,0,0,'C');
				...
				//$pdf->Cell(21,5+17,$earned_bonus,0,0,'C');
			...
			else if ( $GLOBALS['flag_bri_nexcard']  === true ){
				////////////////////////////////////////////////// PENAMBAHAN BRI NEXCARD
				//$pdf->SetFont('BRICARD02','',14);
				$pdf->SetFont('BRICARD02','',6);
				$pdf->SetX($x+10);
				//$pdf->Cell(21,5+8,$earned_bonus,0,0,'C');
			...
			else if ( $GLOBALS['flag_bri_private']  === true ){
				////////////////////////////////////////////////// PENAMBAHAN BRI PRIVATE LABEL
				//$pdf->SetFont('BRICARD02','',14);
				$pdf->SetFont('BRICARD02','',6);
				$pdf->SetX($x+10);
				//$pdf->Cell(21,5+8,$earned_bonus,0,0,'C');
			...
			else if ( $GLOBALS['flag_bri_samsung']  === true ){
				////////////////////////////////////////////////// PENAMBAHAN BRI SAMSUNG
				//$pdf->SetFont('BRICARD02','',14);
				$pdf->SetFont('BRICARD02','',6);
				$pdf->SetX($x+10);
				//$pdf->Cell(21,5+8,$earned_bonus,0,0,'C');
			...
			else if ( $GLOBALS['flag_bri_gpn']  === true ){
				////////////////////////////////////////////////// PENAMBAHAN BRI GPN
				//$pdf->SetFont('BRICARD02','',14);
				$pdf->SetFont('BRICARD02','',6);
				$pdf->SetX($x+10);
			...
			else if ( $GLOBALS['flag_bri_paperid']  === true ){
				////////////////////////////////////////////////// PENAMBAHAN BRI PAPERID
				//$pdf->SetFont('BRICARD02','',14);
				$pdf->SetFont('BRICARD02','',6);
				$pdf->SetX($x+10);
				//$pdf->Cell(21,5+8,$earned_bonus,0,0,'C');
	...
	function end_detail($pdf,$x,$y,$new_balance,$interest_service_charge,$dr_cr_indicator,$statement_date, $halaman_cetak_kertas){
		if($interest_service_charge!=0){
			...
			$pdf->Cell(18+1,4,$interest_service_charge,0,0,'R');
		}
	...
	function output_pdf($pdf,$nama_file_pdf,$flagtrans,$blth,$file,$folder_cycle='',$folder_tipe='',$nama_pdf_2_cif=''){
		...
		$folder_cycle_pdf = $blth_file_pdf.$folder_cycle.'/';
		$folder_cycle_pdf_tipe = $folder_cycle_pdf.$folder_tipe.'/';
		...
		if(file_exists($folder_cycle_pdf)==false) mkdir($folder_cycle_pdf);
		if(file_exists($folder_cycle_pdf_tipe)==false) mkdir($folder_cycle_pdf_tipe);
		#$pdf_output = $blth_file_pdf.$nama_file_pdf.'.pdf';
		
		$nama_pdf_2_cif = trim($nama_pdf_2_cif);
		$sql_cek_dat = "SELECT * FROM _list_convert_emeterai WHERE nomor_rekening = '$nama_pdf_2_cif' ";
		$qry_cek_dat = pg_query($sql_cek_dat) or die('ERROR cek m_loading: '.$sql_cek_dat);
		$jml = pg_num_rows($qry_cek_dat);
		if($jml == 0){
			$pdf_output = $folder_cycle_pdf_tipe.$nama_file_pdf.'.pdf';
		}else{
			$folder_listing_ematerai = $folder_cycle_pdf_tipe.'LISTING_EMETERAI'.'/';
			if(file_exists($folder_listing_ematerai)==false) mkdir($folder_listing_ematerai);
			$pdf_output = $folder_listing_ematerai.$nama_file_pdf.'.pdf';
			
			$sql_u ="UPDATE _list_convert_emeterai
				   SET status='t'
				   WHERE nomor_rekening = '$nama_pdf_2_cif';
				";
			$qry_u = pg_query($sql_u) or die('ERROR update status _list_convert_emeterai: '.$sql_u);
		}
		...
		$xyz = "C:/app/htdocs/e_statement_bri/";		
		//$xyz = "D:/app/htdocs/e_statement_bri_uat/";
		$xyz = $xyz. str_replace('../pdf','pdf',$pdf_output);
		#$pdf->Output(__DIR__ . $pdf_output,'F');
		$pdf->Output( $xyz,'F');
	}
	...
	function insert_detail($m_loading_id,$nomor_customer,$nomor_rekening,
								...
								$pdf_name,$password_pdf,$jml_hlm, $email, $tabeldetail, $ket_produk, $tanggal='0'
								,$flag_bea_materai_lunas
								,$password_admin_pdf
								,$folder_cycle='',$folder_tipe=''
								,$cabang=''){
		...
		///////////////////////////////////////////////////////////////////
		//pengecekan jika materai > 1 die error salah data
		//dikarenakan khusus non materai
		$flag_total_materai = $flag_bea_materai_lunas[$nomor_rekening]['total_ematerai'];
		if ( $flag_total_materai == 0)
		{
			die("PROSES GAGAL<br>Flag khusus DENGAN ADA EMATERAI<br>CIF $nomor_rekening ada materainya dengan total : ".$flag_total_materai);
		}
		///////////////////////////////////////////////////////////////////
		...
		$sql_cek_dat = "SELECT * FROM _list_convert_emeterai WHERE nomor_rekening = '$nomor_rekening' ";
		$qry_cek_dat = pg_query($sql_cek_dat) or die('ERROR cek m_loading: '.$sql_cek_dat);
		$jml = pg_num_rows($qry_cek_dat);
		if($jml == 0){
			$document = $document.$file.'/'.$folder_cycle.'/'.$folder_tipe.'/'.$pdf_name.'.pdf';
		}else{
			$document = $document.$file.'/'.$folder_cycle.'/'.$folder_tipe.'/LISTING_EMETERAI/'.$pdf_name.'.pdf';
		}
		
		$filesizePdfBaru = fsize($document);
		
		$x_2m = str_replace('.','',$cabang);
		if ( $x_2m > 2147483647)
		{
			$flag_2m = 'lebih 2M';
		}else{
		...
		/////////////////////////////////////////////////////////////////
		//cek_double_tabeldetail($tabeldetail, $nomor_rekening, $m_loading_id);
		/////////////////////////////////////////////////////////////////
		...
		#{
			...
			#$password_pdf = '';
				...
				$sql_ins_detail = "INSERT INTO $tabeldetail(
										...
										$m_loading_id,'".$nomor_customer[$nomor_rekening]."','$nomor_rekening',
										E'".addslashes($nama)."',E'".addslashes($address1)."',E'".addslashes($address2)."',
										E'".addslashes($address3)."',E'".addslashes($city)."','$zipcode',
										'$flagtrans','$blth','$nama_file',
										'$pdf_name','$password_pdf',$jml_hlm,E'".addslashes($email)."','$n_email','$filesizePdfBaru','$ket_produk'
										,'$tanggal'
										,'".$flag_bea_materai_lunas[$nomor_rekening]['total_ematerai']."'
										,'".$flag_bea_materai_lunas[$nomor_rekening]['jumlah_kartu_ematerai']."'
										,'".'EMATERAI'."'
										,E'".$password_admin_pdf."'
										,E'".$cabang."'
										,E'".str_replace('.','',$cabang)."'
										,E'".$flag_2m."'
									)";
	...
	function create_csv($m_loading_id,$flagtrans, $blth,$file,$tabeldetail){
		...
		$data = array ();
		$data[]=array('nomor_rekening', 'nama', 'pdf_name', 'password_pdf',
		'password_admin_pdf','total_ematerai','jumlah_kartu_ematerai');
		
		#$header = "'nomor_rekening';'nama';'pdf_name';'password_pdf'".chr(13);
		#$detail = "";
		
		$sql = "SELECT nomor_rekening,nama,pdf_name,password_pdf
		,total_ematerai, jumlah_kartu_ematerai, password_admin_pdf
		FROM $tabeldetail WHERE m_loading_id = $m_loading_id";
		$qry = pg_query($sql) or die('ERROR: '.$sql);
		while($row = pg_fetch_array($qry)){
			...
			$password_admin_pdf = $row['password_admin_pdf'];
			$total_ematerai = $row['total_ematerai'];
			$jumlah_kartu_ematerai = $row['jumlah_kartu_ematerai'];
			
			#$detail .= "'".$nomor_rekening."';'".$nama."';'".$pdf_name."';'".$password_pdf."'".chr(13);
			//"=\"" .$line["phone"]. "\"";
			/*
			$data[]=array("=\"" .$nomor_rekening . "\""
			, "=\"" .$nama. "\""
			,"=\"" .$pdf_name. "\""
			,"=\"" .$password_pdf. "\""
			,"=\"" .$password_admin_pdf. "\""
			,"=\"" .$total_ematerai. "\""
			,"=\"" .$jumlah_kartu_ematerai. "\""
	...
	function create_csv_ematerai($m_loading_id,$flagtrans, $blth,$file,$tabeldetail){
		$csv = str_replace(substr($file,-4),'',$file);
		/*
		$file_csv = '../pdf/'.$flagtrans.'/'.$blth.'/'.$csv.'/'.$csv.'.csv';
		$header = "'nomor_rekening';'nama';'pdf_name';'password_pdf'".chr(13);
		...
		*/
		...
		$file_csv = '../pdf/'.$flagtrans.'/'.$blth.'/'.$csv.'/'.$csv.'_x.csv';
		/*
		$data = array (
		...
		*/
		$data = array ();
		$data[]=array('Delftstack1', 'Delftstack2', 'Delftstack3', 'Delftstack4');
		$data[]=array('Delftstack1', 'Delftstack2', 'Delftstack3', 'Delftstack4');
		$data[]=array('Delftstack1', 'Delftstack2', 'Delftstack3', 'Delftstack4');
		$data[]=array('Delftstack1', 'Delftstack2', 'Delftstack3', 'Delftstack4');
		//Create a CSV file
	...
	function add_page1_jpg($pdf, $url_pdf='')
	{
		if ( !empty($url_pdf) )
		{
			...
			#$pdf->Image($url_img, 0, 0, 210, 297, '', '', '', false, 300, '', false, false, 0);
			$pdf->Image($url_img, 0, 0, 216,302, '', '', '', false, 300, '', false, false, 0);
			#$pdf->Image($url_img,0,0,$w1,$h1);
			#$pdf->Image($url_img, 0, 0, 210+10.5, 297+8.91, 'JPG', '', '', true, 200, '', false, false, 0, false, false, true);
			#$pdf->Image($url_img,0,0,216.00,304.80);
			#$pdf->Image($url_img,0,0,217,302);
			#$pdf->Image($url_img, 0, 0, 210+14, 297+5, '', '', '', false, 300, '', false, false, 0);
			/*
			...
			*/
			#$pdf->Image($url_img, 0, 0, 0, 0, '', '', '', false, 300, '', false, false, 0, true, false, true);
			#$pdf->Image($url_img, 0, 0, 600 ,600 ,'', '', '', false, 300,'', false, false, 0, true, false, true);
			$pdf->setAutoPageBreak($auto_page_break, $bMargin);
	...
	function add_page2_summary_jpg($pdf, $url_pdf='', $arr_summary, $jml_hlm , $barcode, $posko, $kurir, $nourut, $kolektibilitas, $file )
	{
		...
		if ( !empty($url_pdf) )
		{
			...
			#$pdf->Cell(20,5,'Halaman: '.$pdf->getAliasNumpage().' / '.$pdf->getAliasNbPages()); //20200120 dimatikan karena error halaman KZ
			$pdf->Cell(20,5,'Halaman: 2'.' / '.$jml_hlm); //20200120 dimatikan karena error halaman KZ
			$pdf->SetFont('BRICARD02','',$ukuran_font_xx);
				...
				foreach ($value as $key2 => $value2) 
				{
					...
					if ( $key2 == 'header1' || $key2 == 'header2' )
					{
						...
						for($x_tambahan=0;$x_tambahan<4;$x_tambahan++)
						{
							...
							{
								#$pdf->Cell(67,4, number_format($arr_isi[$x_tambahan],0,'',',') ,0,0,'L');
								#$pdf->Cell(67,4, number_format($arr_isi[$x_tambahan],0,'','.') ,0,0,'C');
						...
						for($x_tambahan=0;$x_tambahan<=count($arr_isi);$x_tambahan++)
						{
							...
							$pdf->SetFont('BRICARD02','',$ukuran_font_xx);
							#$pdf->Cell(67,4, number_format($arr_isi[$x_tambahan],0,'',',') ,0,0,'R');
							$pdf->Cell(67,4, $arr_isi[$x_tambahan] ,0,0,'R');
						}
					}else if (  $key2 == 'footer2' ){
						#$summary_footer = number_format($arr_isi[0],0,'','.');//perubahan pemisah desimal titik
						$summary_footer = $arr_isi[0];//perubahan pemisah desimal titik
					}else if (  $key2 == 'footer5' ){
						#$summary_footer_x1 = number_format($arr_isi[0],0,'','.');//perubahan pemisah desimal titik
						$summary_footer_x1 = $arr_isi[0];
					}else if (  $key2 == 'footer6' ){
						#$summary_footer_x2 = number_format($arr_isi[0],0,'','.');//perubahan pemisah desimal titik
						...
						for($x_tambahan=0;$x_tambahan<=count($arr_isi);$x_tambahan++)
						{
								...
								if ($x_tambahan == 3)
								{
									$total_tagihan_1 += $arr_isi[$x_tambahan];
								}else if($x_tambahan == 4){
									$total_tagihan_2 += $arr_isi[$x_tambahan];
								}
...
$objWriter->save($dir);
	...
	function add_halaman_ematerai_jpg($pdf, $url_pdf='', $arr_summary, $jml_hlm , $barcode, $posko, $kurir, $nourut, $kolektibilitas, $file )
	{
		#die(print_r($arr_summary));
		#if(empty($arr_summary)) return 'no';
		...
		if ( !empty($url_pdf) )
		{
			...
			$pdf->setImageScale(1.25);
			#$pdf->Image($url_img, 0, 0, 210, 297, '', '', '', false, 300, '', false, false, 0);
			$pdf->Image($url_img, 0, 0, 217,302, '', '', '', false, 300, '', false, false, 0);
			#$pdf->Image($url_img,0,0,217,302);
			...
			#####$pdf->SetXY(20+3,20);
			$var_x_ematerai_lurus = 14.5;
			$pdf->SetXY($var_x_ematerai_lurus,20);
			$pdf->SetFont('BRICARD02','', 12);
			$pdf->Cell(67,4,'Lembar Pengesahan E-Meterai',0,0,'L');
				...
				foreach ($value as $key2 => $value2) 
				{
					...
					if (  $key2 == 'header3' ){
						...
						/////////////////////////////////////////////////////////////////////////////////////
								...
								$geser_atas = 30;
								####$x_barcode = 23;
								$x_barcode = $var_x_ematerai_lurus;
								$y_barcode =95 -$geser_atas;
								$pdf->SetXY($x_barcode,$y_barcode-5.5);
						...
						for($x_tambahan=0;$x_tambahan<=count($arr_isi);$x_tambahan++)
						{
							if ($x_tambahan >6) continue;
							if ($x_tambahan+1 == count($arr_isi) )
							{
								$pdf->SetXY(123,85-14-6+5+2-2-$geser_atas);
								$pdf->SetFont('BRICARD02','',$ukuran_font_xx);
								#$pdf->MultiCell(70,3.5,$arr_isi[$x_tambahan],0,'C');
								$pdf->MultiCell(70,3.5,$arr_isi[$x_tambahan],0,'J'); //20201223
								$pdf->SetFont('BRICARD02','',$ukuran_font_xx);
							}else{
								#####$pdf->SetXY(20+3,87+(3.5*$x_tambahan)-18-$geser_atas);
								$pdf->SetXY($var_x_ematerai_lurus,87+(3.5*$x_tambahan)-18-$geser_atas);
								$pdf->SetFont('BRICARD02','',$ukuran_font_xx);
								$pdf->Cell(67,4,$arr_isi[$x_tambahan],0,0,'L');
							}
			...
			$pdf->SetXY($var_x_ematerai_lurus,270);
			$pdf->SetFont('BRICARD02','', 9);
			$pdf->MultiCell(160,2,"Lembar pengesahan e-meterai ini merupakan lembar untuk pembubuhan e-meterai, pembebanan bea meterai telah dicantumkan dalam lembar tagihan dan disetorkan ke negara"."\n",0,'C');
