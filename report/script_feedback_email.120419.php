	...
	function show_report(){
		...
		if($report_menu==1){
			...
			if($arr_respon[0]=='connect'){
				...
				$sql_sel_tr_email = "SELECT distinct a.email, a.nama_customer AS nama, a.nomor_rekening, to_char(a.date_email_send,'DD-Mon-YYYY HH24:MI:SS') as date_email_send, nama_file
										FROM tr_email a, vw_email b
										WHERE a.loading_id = b.m_loading_id
											and a.email = b.email
											and b.blth = '$blth'
				...
				if($tipe_report==2){
					...
					$dlxls='<a href="#" onClick="window.open(\'script_feedback_email.php?act=show_report&flagtrans=BC&blth='.$blth.'&nama_file='.$nama_file.'&report_menu=1&tipe_report=2\');">DOWNLOAD</a> (.XLS FORMAT)';
				}
				if($arr_respon[1]==1){
					...
					<tr>
						<td>
							<table width="1000" align="center" border="0" cellpadding="0" cellspacing="0">
								...
								<tr>
									<td height="20" colspan="6">PERIODE: <?=$dlxls?></td>
								</tr>
							</table>
							...
							<table width="1000" align="center" border="1" cellpadding="2" cellspacing="0">
								...
								while($row_sel_tr_email=pg_fetch_array($qry_sel_tr_email)){
									...
									<tr>
										...
										<td><?=$row_sel_tr_email['email']?></td>
										<td nowrap><?=$row_sel_tr_email['date_email_send']?></td>
										<td valign='top'><?=$row_sel_tr_email['email_callback']?></td>
									</tr>
									<?php
								}
								?>
		...
		}elseif($report_menu==5){
			$get_respon = get_from_mailserver($blth, $flagtrans, $nama_file);
			$arr_respon = explode("|",$get_respon);
			if($arr_respon[0]=='connect'){
				$sql_sel_tr_email = "SELECT a.email, a.nama, a.nomor_rekening, b.nama_file, b.pdf_name
									FROM vw_email a, detail b
									WHERE a.m_loading_id = b.m_loading_id
										and b.blth = '$blth'
										and b.flagtrans = '$flagtrans'";						
				if($nama_file!='' && $nama_file!='null'){
					$sql_sel_tr_email .= " and b.nama_file='$nama_file'";
				...
				$sql_sel_tr_email .= " GROUP BY  
					a.email, a.nama, a.nomor_rekening, b.nama_file, b.pdf_name
				ORDER BY b.nama_file, a.nomor_rekening ASC";
				$qry_sel_tr_email = pg_query($sql_sel_tr_email) or die('ERROR select tr_email: '.$sql_sel_tr_email);
					...
					<tr>
						<td>
							<table width="1000" align="center" border="0" cellpadding="0" cellspacing="0">
								...
								<tr>
									<td height="20" colspan="5">PERIODE: <?=$blth?></td>
								</tr>
							</table>
							<table width="1000" align="center" border="1" cellpadding="2" cellspacing="0">
								<tr bgcolor="#FF0000" style="color:#FFF">
								...
								while($row_sel_tr_email=pg_fetch_array($qry_sel_tr_email)){
									...
									<tr>
										...
										<td><?=str_replace('#','<br>',$row_sel_tr_email['nama'])?></td>
										<td><?=$row_sel_tr_email['email']?></td>
										<td nowrap><?=$row_sel_tr_email['pdf_name']?></td>
									</tr>
									<?php
								}
								?>
