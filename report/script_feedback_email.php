...
//	die($act);
	...
	function get_from_mailserver($blth, $flagtrans, $nama_file){
		...
		while($row_mail_server = pg_fetch_array($qry_sel_mail_server)){
			...
			/*$email_inbox = "{202.51.252.22:110/pop3/tls/novalidate-cert}INBOX";
			$email_from = "noreply@indocorp.com";
			$email_pass = "appdev123";*/
			...
			for($abc = $bnyk_loop; $abc > 0; $abc--){
			//for($abc = 1; $abc <= $bnyk_loop; $abc++){
				$rec_terakhir = ($abc * $jum_max_rec);
				if($abc == $bnyk_loop) $rec_terakhir = $jum_mbox;
				$mbox = imap_open($email_inbox, $email_from, $email_pass) or die("can't connect: " . imap_last_error());
				$email_bounce_message='';
				for($i = ((($abc - 1) * $jum_max_rec) + 1); $i <= $rec_terakhir; $i++){
					...
					or (strtolower(substr($header_info->subject, 0, 14)) == "returned mail:")
				...
				(
					  ((strtolower(substr($header_info->subject, 0, 5)) == "read:"  ))
								or ((strtolower(substr($header_info->subject, 0, 6)) == "return"  ))
								or ((strtolower(substr($header_info->subject, 0, 7)) == "checked"  ))	
								or ((strtolower(substr($header_info->subject, 0, 8)) == "checked "  ))	
								or ((strtolower(substr($header_info->subject, 0, 8)) == "dpriksa:"  ))	
								or ((strtolower(substr($header_info->subject, 0, 8)) == "dpriksa "  ))	
								or ((strtolower(substr($header_info->subject, 0, 6)) == "dibaca"  ))	
								or ((strtolower(substr($header_info->subject, 0, 7)) == "dibaca "  ))
								or ((strtolower(substr($header_info->subject, 0, 12)) == "laporan baca"  ))
								or ((substr($header_info->subject, 0, 13) == "Read report :"  ))
								or ((strtolower(substr($header_info->subject, 0, 6)) == "sudah "  ))	
								or ((strtolower(substr($header_info->subject, 0, 13)) == "sudah dibaca:"  ))	
								or ((strtolower(substr($header_info->subject, 0, 26)) == "return receipt (displayed)"  ))
								or ((strtolower(substr($header_info->subject, 0, 14)) == "return receipt"  ))
								or ((strtolower(substr($header_info->subject, 0, 14)) == "returned mail:"  ))
								or ((strtolower(substr(trim($header_info->subject), 0, 6)) == "return"  ))
								or ((strtolower(substr($header_info->subject, 0, 3)) == "re:"  ))	
								or (substr(trim($header_info->subject), 0, 5) == "Read:"  )
					...
					
				{
					...
					cekRead($mbox, $i, $tr_email_id, $blth,strtolower($email_bounce));
						...
						((strtolower(substr($header_info->subject, 0, 9)) == "not read:") or
						(strtolower(substr($header_info->subject, 0, 4)) == "hal:"))
						{
								imap_delete($mbox, $i);
						}else{
			...
			if((int)$tr_email_id)
				{
					...
					"INSERT INTO read_email (
							...
							".$tr_email_id.", '".$tgl_read."', '".$slice."'::varchar(1000), 'outlook'
						)";
			 ...
			 else  //jika in_reply_to kosong, pakai alamat email utk masukkan ke read_email 
			 {
				...
				//die($sql_insert) . "<br>";
				$qry_sql_insert = @pg_query($sql_insert);
	...
	function show_report(){
		...
		if($report_menu==1){
						...
						<td>
							...
							<table width="1000" align="center" border="1" cellpadding="2" cellspacing="0">
								...
								while($row_sel_tr_email=pg_fetch_array($qry_sel_tr_email)){
									...
									<tr>
										<td><?=$no?></td>
										<td><?php echo "&nbsp;".str_replace(" ","",$row_sel_tr_email['nomor_rekening_'])?></td>
			...
			if($get_respon=='connect'){
				...
				$sql_sel_tr_email .= " ORDER BY a.date_email_callback ASC";
				$qry_sel_tr_email = pg_query($sql_sel_tr_email) or die('ERROR select tr_email: '.$sql_sel_tr_email);
