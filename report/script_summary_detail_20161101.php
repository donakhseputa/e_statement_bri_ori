...
$objWriter->save('php://output');
	...
	$m_loading_id = $_REQUEST['m_loading_id'];
		...
		if($sum_sql_datas > 60000){
			$limit = 60000;
			$offset = 0;
			$hitung_pecah = ceil($sum_sql_datas/$limit); // 4
			...
			for ($i=0; $i < $hitung_pecah; $i++){
				...
				$sql_datas .= ' OFFSET '. $offset . ' LIMIT ' . $limit ;
				$qry_datas = pg_query($sql_datas) or die('ERROR select data: '.$sql_datas);
				...
				$dir_part = '../temp_file/report/dikirim/'; // agar tidak tertukar dengan file yang lain 
				$filename='list_email_dikirim_'.$m_loading_id.'_'.$loading_file . '_part_' . $i.'.xls';

				$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');
				
				$objWriter->save($dir_part.$filename);
				
				$offset += $limit;
				$sql_datas = '';
			}
			$name_file_zip = $m_loading_id.'_'.$loading_file .'.zip';
			create_zip($dir_part, $name_file_zip);
		}else{
			$objPHPExcel = new PHPExcel();
			$F=$objPHPExcel->getActiveSheet();
	...
	//** 18 Des 2013 **//
		...
		if($sum_sql_datas > 60000){
			...
			for ($i=0; $i < $hitung_pecah; $i++){
				...
				$dir_part = '../temp_file/report/sukses/';
				$filename='list_email_sukses_'.$m_loading_id.'_'.$loading_file . '_part_' . $i.'.xls';
				// Redirect output to a clients web browser (Excel5)
				//header('Content-Type: application/vnd.ms-excel');
				...
				$offset += $limit;
				$sql_datas = ''; // reset query
			}
			// create file zip dengan partfile
			$name_file_zip = $m_loading_id.'_'.$loading_file .'.zip';
			create_zip($dir_part, $name_file_zip);
		}else{
				$objPHPExcel = new PHPExcel();
