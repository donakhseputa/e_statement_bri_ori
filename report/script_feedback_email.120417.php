	...
	function show_report(){
		...
		if($report_menu==1){
			...
			if($arr_respon[0]=='connect'){
				...
				$sql_sel_tr_email = "SELECT distinct a.email, a.nama_customer AS nama, a.nomor_rekening, to_char(a.date_email_send,'DD-Mon-YYYY HH24:MI:SS') as date_email_send
										FROM tr_email a, vw_email b
										WHERE a.loading_id = b.m_loading_id
											and a.email = b.email
											and b.blth = '$blth'
				...
				$sql_sel_tr_email .= " ORDER BY date_email_send, a.nomor_rekening ASC";
				...
				if($tipe_report==2){
					...
					header("Content-Disposition: attachment; filename=".$blth."_emailSukses.xls");
					header("Expires: 0");
			...
			if($arr_respon[0]=='connect'){
				...
				/*$sql_sel_tr_email = "SELECT a.email, to_char(a.date_email_send,'DD-Mon-YYYY HH24:MI:SS') as date_email_send, a.ket_error,
									...
									WHERE a.loading_id = b.m_loading_id
										...
										and a.email_callback is not null";*/	
				$sql_sel_tr_email = "SELECT a.email, to_char(a.date_email_send,'DD-Mon-YYYY HH24:MI:SS') as date_email_send, a.ket_error,
											a.nama_customer AS nama, a.nomor_rekening, b.loading_file
									FROM tr_email a, m_loading b
									WHERE a.loading_id = b.m_loading_id
										and b.blth = '$blth'
										and b.flagtrans = '$flagtrans'
										and a.email_sukses = TRUE
										and a.email_callback is not null";										
				if($nama_file!='' && $nama_file!='null'){
					$sql_sel_tr_email .= " and b.loading_file='$nama_file'";
				}
				$sql_sel_tr_email .= " ORDER BY a.nomor_rekening ASC";
				$qry_sel_tr_email = pg_query($sql_sel_tr_email) or die('ERROR select tr_email: '.$sql_sel_tr_email);
				
				if($tipe_report==2){
					header("Content-type: application/vnd.ms-excel");
					header("Content-Disposition: attachment; filename=".$blth."_emailGagal.xls");
					header("Expires: 0");
					header("Cache-Control: must-revalidate, post-check=0, pre-check=0");
					header("Pragma: public");
				}else{
			...
			if($arr_respon[0]=='connect'){
				...
				if($tipe_report==2){
					header("Content-type: application/vnd.ms-excel");
					header("Content-Disposition: attachment; filename=".$blth."_emailOthers.xls");
					header("Expires: 0");
					header("Cache-Control: must-revalidate, post-check=0, pre-check=0");
					header("Pragma: public");
				}
				...
				<table align="center">
					<tr>
						<td>
							<table width="1000" align="center" border="0" cellpadding="0" cellspacing="0">
								<tr>
									<td align="center" valign="middle" class="title" colspan="6"><strong>REPORT - LAPORAN EMAIL STATUS OTHER</strong></td>
								</tr>
								<tr>
									<td height="20" colspan="6">PERIODE: <?=$blth?></td>
								</tr>
