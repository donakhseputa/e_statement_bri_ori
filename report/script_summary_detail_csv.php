        ...
        <html xmlns="http://www.w3.org/1999/xhtml">
            ...
            <body>
				...
				<div align="center">
				 <?php
                      if($_REQUEST['xls']<>'y'){  
					  echo pagings($offset,$jumlah_data,$limit,'div_paging','script_summary_detail.php','act=show_list_pdf&blth='.$blth.'&pr='.$flagtrans.'&m_loading_id='.$m_loading_id.'&tipe='.$tipe);
					  }
                    ?>
					</div>
            </body>
        ...
        <html xmlns="http://www.w3.org/1999/xhtml">
            ...
            <body>
            	<table align="center" border="0" width="1000">
					...
					<td><div align="center">';
                      if($_REQUEST['xls']<>'y'){  
					  $vetable .= pagings($offset,$jumlah_data,$limit,'div_paging','script_summary_detail.php','act=show_list_email&pr='.$flagtrans.'&blth='.$blth.'&m_loading_id='.$m_loading_id.'&tipe='.$tipe);
					  }
					$vetable .='</div></td>
					</tr>
					</table>
            ...
            <body>
            	<table align="center" border="0" width="1000">
                    	...
                    	<td>
                        	<table align="center" border="0" cellpadding="2" cellspacing="0" width="100%">
								...
								<tr height="20">
									...
									<a onClick="window.open('script_summary_detail.php?act=export_show_total_email&pr=<?php echo $flagtrans;?><?php if (!empty($sent_from)){echo '&periode_kirim='.$_GET['periode_kirim'];}?>&blth=<?php echo $blth;?>&m_loading_id=<?php echo $m_loading_id;?>&sof=<?php echo $sof;?>&xls=y&tipe=<?php echo $tipe;?>')"><img height="40px" title="download excel" src="../images/icons/xls_icon.gif" /></a>
					...
					<?php 
					  ...
					  echo pagings($offset,$jumlah_data,$limit,'div_paging','script_summary_detail.php','act=show_total_email&blth='.$blth.'&pr='.$flagtrans.'&sof='.$sof.'&m_loading_id='.$m_loading_id.$periodekirim);
					  } ?>
					</div></td>
					</tr>
					</table>
...
$objWriter->save('php://output');
	...
	//** 18 Des 2013 **//
		...
		// Create new PHPExcel object
		// count based this query
		$sql_datas_sum = "SELECT
									 te.antrian_id, te.nomor_rekening, te.email, te.date_email_send,
									 d.nama, d.size_pdf,
									 ml.loading_file,
									 SUM(maf.ukuran)AS total_attach
									...
									GROUP BY te.antrian_id, te.nomor_rekening, te.email, te.date_email_send,
									 ...
									 ml.loading_file
									  ,te.tgl_read, te.body_email_read, te.read_method
									ORDER BY te.date_email_send";
		$qry_datas_sum = pg_query($sql_datas_sum) or die('ERROR select data: '.$sql_datas);
		// fungsi untuk memecah file sebanyak 50000
		$qry_sum_datas = pg_num_rows($qry_datas_sum);
		...
		if($qry_sum_datas >= 0){
			...
			$dest = '../csv/';
			$new_filename = 'list_email_sukses_'.$m_loading_id.'_'.$loading_file. '.csv';
			$handle = fopen('datacsv_new.csv', 'a+');
			$nl_dos	 = "\r\n";
			
			
			$data_h = "No" . ",";
			$data_h  .= "Nama File" . ",";
			$data_h  .= "Nomor Rekening" . ",";
			$data_h  .= "Nama" . ","; 
			$data_h .= "Email" . ",";
			$data_h .= "Tanggal Kirim" . ",";
			...
			$data_h .= "PDF Size" . ",";
			$data_h .= "Attachment Size/Brosur" . ",";
			$data_h .= "Total Size". ",";
			$data_h .= "Berbayar" . ",";
			
			fwrite($handle, $data_h. $nl_dos);
			...
			while($row_data = pg_fetch_array($qry_datas)){//extract each record
				...
				if ( $tgl_read=='01-01-1970 07:00:00' )
				{
					...
					$data .= $tgl_read . ",";
					//$data .= str_replace(",", ",", $row_data['body_email_read']) . ",";
			...
			header('Content-Disposition: attachment; filename="datacsv_new.csv"');
			// File to download                                
			readfile('datacsv_new.csv'); 
			...
			// fungsi untuk memecah file sebanyak 50000
			$objPHPExcel = new PHPExcel();
			$F = $objPHPExcel->setActiveSheetIndex($index);
			...
			$objPHPExcel->setActiveSheetIndex($index)->getStyle('A'.$Line.':M'.$Line.'')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB('ffdfdfdf');
			...
			header('Content-Type: application/vnd.ms-excel');
			header('Content-Disposition: attachment;filename="list_email_sukses_'.$m_loading_id.'_'.$loading_file.'_part_'.$i.'.xls"');
			//header('Cache-Control: max-age=0');
        ...
        <html xmlns="http://www.w3.org/1999/xhtml">
            ...
            <body>
            	<table align="center" border="0" width="1000">
				 ...
				 <?php
                      if($_REQUEST['xls']<>'y'){  
					  echo pagings($offset,$jumlah_data,$limit,'div_paging','script_summary_detail.php','act=show_email_terbaca&blth='.$blth.'&source='.$source.'&pr='.$flagtrans.'&m_loading_id='.$m_loading_id.'&tipe='.$tipe);
					  }
                    ?>
					</div>
					</td>
