...
<?
                    	...
                    	<td>
                        	<table align="center" border="1" cellpadding="2" cellspacing="0" width="100%">
                            	<tr height="30">
                                	...
                                	<td align="center" colspan="2" width="10%" bgcolor="#79BAEC"><b>Email</b></td>
								...
								<td width="5%" align="center"><b>(>=5hlm)</b></td>
								</tr>
                                <?php
									if (!empty($cycle)){
									...
									$sql_data = "
										...
										FROM m_loading a
											...
											LEFT JOIN (
												...
												FROM vw_email_gratis
												GROUP BY m_loading_id
											) c
												ON a.m_loading_id = c.m_loading_id
											LEFT JOIN (
												SELECT count(*) as jml_email_bayar, m_loading_id
												FROM vw_email_bayar
												GROUP BY m_loading_id
											) cb
										...
										GROUP BY a.m_loading_id, to_char(a.create_date,'yyyy-mm-dd HH24:MI'), a.loading_file, a.total_customer, a.total_halaman,
											bc.total_email, c.jml_email, cb.jml_email_bayar, d.jml_antrian, e.email_sukses, f.email_gagal, et.email_terbaca, g.sample_sukses, h.sample_gagal
									...
									<tr height="30">
										...
										<td align="right"><a onClick="window.open('script_summary_detail.php?act=show_list_pdf&pr=<?=$flagtrans?>&m_loading_id=<?php echo $m_loading_id?>')"><?php echo number_format($row_data['jml_pdf'],0,'',',')?></a>&nbsp;&nbsp;</td>
										<td align="right"><a onClick="window.open('script_summary_detail.php?act=show_list_pdf&pr=<?=$flagtrans?>&m_loading_id=<?php echo $m_loading_id?>')"><?php echo number_format($row_data['jml_lembar'],0,'',',')?></a>&nbsp;&nbsp;</td>
										<td align="right" bgcolor="#afeeee"><a onClick="window.open('script_summary_detail.php?act=show_list_email&tipe=0&pr=<?=$flagtrans?>&m_loading_id=<?=$m_loading_id?>')"><?php echo number_format($row_data['jml_email'],0,'',',');?></a>&nbsp;&nbsp;</td>
										<td align="right" bgcolor="#afeeee"><a onClick="window.open('script_summary_detail.php?act=show_list_email&tipe=1&pr=<?=$flagtrans?>&m_loading_id=<?=$m_loading_id?>')"><?php echo number_format($row_data['jml_email_bayar'],0,'',',');?></a>&nbsp;&nbsp;</td>
                                ...
                                <tr height="30" style="font-weight:bold" bgcolor="#87ceeb">
                                	...
                                	<td align="right" bgcolor="#87ceeb"><a onClick="window.open('script_summary_detail.php?act=show_list_email&pr=<?=$flagtrans?>&tipe=0&blth=<? echo $blth?><?php if (!empty($_GET['periode_kirim'])){echo '&periode_kirim='.$_GET['periode_kirim'];}?>&m_loading_id=<?php echo trim($m_loading_id_all,'|')?>')"><?php echo number_format($total_email,0,'',',');?></a>&nbsp;&nbsp;</td>
	...
	function export_excel($curloc, $locateFile) {
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('H'.$y_post.'',"Total Email");
		$objPHPExcel->getActiveSheet()->SetCellValue('I'.$y_post.'',"Email di Antrian");
		$objPHPExcel->getActiveSheet()->SetCellValue('J'.$y_post.'',"Email Sukses");
		$objPHPExcel->getActiveSheet()->SetCellValue('K'.$y_post.'',"Email Gagal");
		$objPHPExcel->getActiveSheet()->SetCellValue('L'.$y_post.'',"Sample Sukses");
		$objPHPExcel->getActiveSheet()->SetCellValue('M'.$y_post.'',"Sample Gagal");
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('H'.$atrow.'',$row_data['total_email']);
		$objPHPExcel->getActiveSheet()->SetCellValue('I'.$atrow.'',$row_data['jml_antrian']);
		$objPHPExcel->getActiveSheet()->SetCellValue('J'.$atrow.'',$row_data['email_sukses']);
		$objPHPExcel->getActiveSheet()->SetCellValue('K'.$atrow.'',$row_data['email_gagal']);
		$objPHPExcel->getActiveSheet()->SetCellValue('L'.$atrow.'',$row_data['sample_sukses']);
		$objPHPExcel->getActiveSheet()->SetCellValue('M'.$atrow.'',$row_data['sample_gagal']);
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('H'.$typost.'',$t_jml_total_email);
		$objPHPExcel->getActiveSheet()->SetCellValue('I'.$typost.'',$t_jml_antrian);
		$objPHPExcel->getActiveSheet()->SetCellValue('J'.$typost.'',$t_email_sukses);
		$objPHPExcel->getActiveSheet()->SetCellValue('K'.$typost.'',$t_email_gagal);
		$objPHPExcel->getActiveSheet()->SetCellValue('L'.$typost.'',$t_sample_sukses);
		$objPHPExcel->getActiveSheet()->SetCellValue('M'.$typost.'',$t_sample_gagal);
	...
	function get_RowData($blth, $flagtrans, $cycle, $sent_from, $sent_to){
		...
		$sql_data = "
			...
			GROUP BY a.m_loading_id, to_char(a.create_date,'yyyy-mm-dd'), a.loading_file, a.total_customer, a.total_halaman,
				bc.total_email, c.jml_email, cb.jml_email_bayar, d.jml_antrian, e.email_sukses, f.email_gagal, g.sample_sukses, h.sample_gagal
			ORDER BY a.m_loading_id
		";
		$qry_data = pg_query($sql_data) or die('ERROR select summary detail: '.$sql_data);
	...
	function export_ba($client, $curloc, $locateFileBA) {
		...
		while($row_data = pg_fetch_array($getRowData_dtail)){																																	
			...
			#$pdf->Cell(25,5,number_format($row_data['jml_email'],0,'',','),1,0,'R');
			#$pdf->Cell(17.5,5,number_format($row_data['jml_tanpa_email'],0,'',','),1,0,'R');
			#$pdf->Cell(17.5,5,number_format($row_data['jml_tanpa_email_bayar'],0,'',','),1,0,'R');
			$pdf->Cell(18,5,number_format($row_data['jml_email'],0,'',','),1,0,'R');
			$pdf->Cell(18,5,number_format($row_data['jml_email_bayar'],0,'',','),1,0,'R');
