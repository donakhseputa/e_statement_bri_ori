...
<?
	...
	if ($flagtrans == 'CO' AND $blth =='052020' )
	{
		$GLOBALS['tr_email_sk_br'] = 'tr_email';
	}
	...
	switch($act){
		...
		case 'show_terdeteksi_suppression': show_terdeteksi_suppression($header_utama); break;
		case 'show_terdeteksi_spam': show_terdeteksi_spam($header_utama); break;
		case 'show_terdeteksi_delay': show_terdeteksi_delay($header_utama); break;
                    	...
                    	<td>
                        	<table align="center" border="1" cellpadding="1" cellspacing="0" width="100%" style="border-collapse:collapse;">
                            	<tr height="30">
                                	...
                                	<td align="center" rowspan="2" bgcolor="#79BAEC"><b>Tanggal_Check</b></td>
                                	<td align="center" rowspan="2" bgcolor="#79BAEC"><b>Tanggal_Aproval</b></td>
                                	<td align="center" rowspan="2" bgcolor="#79BAEC"><b>Awal_Kirim</b></td>
                                	<td align="center" rowspan="2" bgcolor="#79BAEC"><b>Akhir_Kirim</b></td>
                                	<td align="center" rowspan="2" bgcolor="#79BAEC"><b>Waktu_Kirim</b></td>
                                	<td align="center" rowspan="2" bgcolor="#79BAEC"><b>(±) Σ Lampiran</b></td>
                                	<td align="center" rowspan="2" bgcolor="#79BAEC"><b>Grafik</b></td>
                                	...
                                	<td align="center" rowspan="2" bgcolor="#79BAEC"><b> >2M</b></td>
                                	...
                                	<td align="center" colspan="10" width="30%" bgcolor="#79BAEC"><b>EMAIL</b></td>
                                	<td align="center" colspan="5" width="30%" bgcolor="#79BAEC"><b>FLAG</b></td>
								...
								<td align="center" width="4%" bgcolor="#CC9999"><b>Terbaca (All)</b></td>
								<td align="center" width="4%" bgcolor="#CC9999"><b>Terbaca (img)</b></td>
								<td align="center" width="4%" bgcolor="#CC9999"><b>% Terbaca</b></td>
								<td align="center" width="5%" bgcolor="#CC9999"><b>Others</b></td>
								
								<td align="center" width="5%" bgcolor="#CC9999"><b>% Baca Cyber (kode 1/2)</b></td>
								<td align="center" width="5%" bgcolor="#CC9999"><b>% Baca AWS (kode 3/4)</b></td>
								<td align="center" width="5%" bgcolor="#CC9999"><b>Suppression list</b></td>
								<td align="center" width="5%" bgcolor="#CC9999"><b>Gagal (terdeteksi spam)</b></td>
								<td align="center" width="5%" bgcolor="#CC9999"><b>Pengiriman Delay</b></td>
                                ...
                                <?php
									...
									if (!empty($sent_from)){
										...
										$p_kirim = "and a.m_loading_id in (select loading_id from ".$GLOBALS['tr_email_sk_br']." where date_email_send BETWEEN '".$sent_from."' AND '".$sent_to."') ";
...
a.m_loading_id,
 ...
 gt.gambar_terbaca,
											...
											, (select sum(xxc.total_lampiran)  from
											(
											SELECT ac.loading_id, ac.antrian_id, bc.size_pdf, cc.m_attach_file_id, (dc.ukuran/1000)::numeric(6,2) as brosur, 0 as ukuran_email,
											(bc.size_pdf+
											(case when dc.ukuran is null then 0 else (dc.ukuran/1000)::numeric(6,2) end )
											+0) as total_lampiran
											  FROM ".$GLOBALS['tr_email_sk_br']." ac
											  left join $tabeldetail bc ON ac.loading_id=bc.m_loading_id AND ac.nomor_rekening=bc.nomor_rekening
											  LEFT JOIN antrian_email_attach_file cc ON ac.antrian_id = cc.antrian_id
											  LEFT JOIN m_attach_file dc ON cc.m_attach_file_id::integer = dc.m_attach_file_id::integer
											where ac.loading_id = a.m_loading_id 
											
											) xxc ) as total_lampiran_kirim

											,(select count(*) from $tabeldetail where m_loading_id = a.m_loading_id AND nama_produk ='lebih 2M') as lebih_2m
											
											,(select min(date_email_send) from ".$GLOBALS['tr_email_sk_br']." where loading_id = a.m_loading_id AND status_sample <>'t') as awal_kirim
											,(select max(date_email_send) from ".$GLOBALS['tr_email_sk_br']." where loading_id = a.m_loading_id AND status_sample <>'t' ) as akhir_kirim
											
											,(select check_date from log_approval where m_loading_id = a.m_loading_id AND note_check <> 'sample only' order by log_approval_id desc limit 1 ) as tgl_check
											,(select approval_date from log_approval where m_loading_id = a.m_loading_id AND note_check <> 'sample only'  order by log_approval_id desc limit 1 ) as tgl_approval
											
											,(select count(*) from ".$GLOBALS['tr_email_sk_br']." where loading_id = a.m_loading_id AND status_sample <>'t' AND tgl_read IS NOT NULL AND (kode_kirim = 1 OR kode_kirim = 2) ) as persen_baca_cyber
											,(select count(*) from ".$GLOBALS['tr_email_sk_br']." where loading_id = a.m_loading_id AND status_sample <>'t' AND tgl_read IS NOT NULL AND (kode_kirim = 3 OR kode_kirim = 4) ) as persen_baca_aws
											
											
											,(select count(*) from ".$GLOBALS['tr_email_sk_br']." where loading_id = a.m_loading_id AND status_sample <>'t' AND date_email_callback IS NULL AND (kode_kirim = 1 OR kode_kirim = 2) ) as total_persen_baca_cyber
											,(select count(*) from ".$GLOBALS['tr_email_sk_br']." where loading_id = a.m_loading_id AND status_sample <>'t' AND date_email_callback IS NULL AND (kode_kirim = 3 OR kode_kirim = 4) ) as total_persen_baca_aws
											
											
											,(select count(*) from ".$GLOBALS['tr_email_sk_br']." where loading_id = a.m_loading_id AND status_sample <>'t' AND tipe_gagal like '%Suppression%' ) as terdeteksi_suppression_list
											,(select count(*) from ".$GLOBALS['tr_email_sk_br']." where loading_id = a.m_loading_id AND status_sample <>'t' AND tipe_gagal like '%spam%' ) as terdeteksi_spam
											,(select count(*) from ".$GLOBALS['tr_email_sk_br']." where loading_id = a.m_loading_id AND status_sample <>'t' AND delay_info IS NOT NULL AND tgl_read IS NULL AND ket_error IS NULL ) as terdeteksi_delay
...
LEFT JOIN(
select loading_id as m_loading_id, count(*) as total_email from ".$GLOBALS['tr_email_sk_br']." 
...
LEFT JOIN(
select loading_id as m_loading_id, count(*) as email_sukses from ".$GLOBALS['tr_email_sk_br']." 
WHERE loading_id IN (select m_loading_id from m_loading where blth='$blth')  
...
LEFT JOIN(
select loading_id as m_loading_id, count(*) as email_gagal from ".$GLOBALS['tr_email_sk_br']." 
WHERE loading_id IN (select m_loading_id from m_loading where blth='$blth')  
and (status_sample = 'f' or status_sample is null) 
...
LEFT JOIN(
select loading_id as m_loading_id, count(*) as email_terbaca from ".$GLOBALS['tr_email_sk_br']." 
WHERE loading_id IN (select m_loading_id from m_loading where blth='$blth')  
and (status_sample = 'f' or status_sample is null) 
...
LEFT JOIN(
select loading_id as m_loading_id, count(*) as gambar_terbaca from ".$GLOBALS['tr_email_sk_br']." 
WHERE loading_id IN (select m_loading_id from m_loading where blth='$blth')  
and (status_sample = 'f' or status_sample is null) 
...
LEFT JOIN(
select loading_id as m_loading_id, count(*) as sample_sukses from ".$GLOBALS['tr_email_sk_br']." 
WHERE loading_id IN (select m_loading_id from m_loading where blth='$blth')  
and status_sample = 't'
and date_email_callback is null 
...
LEFT JOIN(
select loading_id as m_loading_id, count(*) as sample_gagal from ".$GLOBALS['tr_email_sk_br']." 
WHERE loading_id IN (select m_loading_id from m_loading where blth='$blth')  
and status_sample = 't' 
and date_email_callback is not null 
...
LEFT JOIN antrian_email_history e ON d.antrian_id = e.antrian_id $wh_total GROUP BY a.m_loading_id 
											...
											LEFT JOIN (
												...
												FROM (select * from ".$GLOBALS['tr_email_sk_br']." where tgl_read is not null) a
												WHERE date_email_callback is null ".$js_pkirim."
												AND status_sample IS NOT TRUE
												GROUP BY loading_id
											) et
									...
									*/
									...
									while($row_data = pg_fetch_array($qry_data)){
										...
										$hours = range_2_tanggal( $row_data['akhir_kirim'], $row_data['awal_kirim']);
										
										$row_data['tgl_check'] = (empty($row_data['tgl_check'])) ? '-' : date("d-m-Y H:i:s",strtotime($row_data['tgl_check']));
										$row_data['tgl_approval'] = (empty($row_data['tgl_approval'])) ? '-' : date("d-m-Y H:i:s",strtotime($row_data['tgl_approval']));
										$row_data['awal_kirim'] = (empty($row_data['awal_kirim'])) ? '-' : date("d-m-Y H:i:s",strtotime($row_data['awal_kirim']));
										$row_data['awal_kirimx'] = (empty($row_data['awal_kirim'])) ? '-' : date("Y-m-d H:i:s",strtotime($row_data['awal_kirim']));
										$row_data['akhir_kirim'] = (empty($row_data['akhir_kirim'])) ? '-' : date("d-m-Y H:i:s",strtotime($row_data['akhir_kirim']));
										$row_data['akhir_kirimx'] = (empty($row_data['akhir_kirim'])) ? '-' : date("Y-m-d H:i:s",strtotime($row_data['akhir_kirim']));
										$row_data['total_lampiran_kirim'] = (empty($row_data['total_lampiran_kirim'])) ? '-' : formatBytes( $row_data['total_lampiran_kirim'], 2) ;
									...
									<tr height="30">
										...
										<?php echo $row_data['tgl_check'] ;?><br/>
										</td>
										<td align="center" style="font-size:8pt;" >
										<?php echo $row_data['tgl_approval'] ;?><br/>
										</td>
										<td align="center" style="font-size:8pt;"  bgcolor="#79BAEC">
										<?php echo $row_data['awal_kirim'] ;?><br/>
										</td>
										<td align="center" style="font-size:8pt;"  bgcolor="#79BAEC">
										<?php echo $row_data['akhir_kirim'] . "";?><br/>
										</td>
										...
										<td align="center" style="font-size:8pt;"  bgcolor="#79BAEC">
										<?php echo $row_data['total_lampiran_kirim'] . "";?><br/>
										</td>
										
										<td align="center"><a target="_blank" href="grafik_pengiriman_estat.php?pr=<?=$flagtrans?>&blth=<?=$blth;?>&m_loading_id=<?=$m_loading_id?>&start=<?php echo $row_data['awal_kirimx'];?>&end=<?php echo $row_data['akhir_kirimx'] ;?>&cycle=<?php echo $row_data['loading_file']?>&total_email=<?php echo $row_data['jml_email'];?>">[grafik]</a></td>
										
										<td align="left">&nbsp;<?php echo $download_csv;?></td>
										<td align="left">&nbsp;<?php echo $download_hana_bank;?></td>
										<td align="left">&nbsp;<?php echo $lebih_2m;?></td>
										...
										$xyz= ($row_data['jml_antrian']+$row_data['jml_email'])-$row_data['jml_pdf'];
										echo ( !empty( $xyz ) ? 'F08080': '3CB371'); ?>"><?php echo number_format( $xyz,0,'',',');?>&nbsp;&nbsp;</td>
										...
										<td align="right" bgcolor="#DDBBBB" valign="middle" style="font-size:8pt;background-color:#fff0f5;" >
										...
										<td align="right" bgcolor="#DDBBBB" valign="middle" style="font-size:8pt;background-color:#fff0f5;" >
										<?php 
										if ( empty($row_data['email_terbaca']))
										{
											echo '0%';
										}else{
											echo number_format((($row_data['email_terbaca']/$row_data['email_sukses'])*100),2)."%";
										}
											?>
										</td>
										...
										<?php 
										//$persen_cyber = ($row_data['persen_baca_cyber']/$row_data['total_persen_baca_cyber']*100);
										//$persen_aws = ($row_data['persen_baca_aws']/$row_data['total_persen_baca_aws']*100);
										
										if ( $flagtrans == 'BC')
										{
											if ( $row_data['total_persen_baca_cyber']==0)
											{
												$persen_cyber = '-';
											}else{
												$persen_cyber = ($row_data['persen_baca_cyber']/$row_data['total_persen_baca_cyber']*100);
												$persen_cyber = number_format($persen_cyber,2,'.',',').'%';
											}
											
											if ( $row_data['total_persen_baca_aws']==0)
											{
												$persen_aws = '-';
											}else{
												$persen_aws = ($row_data['persen_baca_aws']/$row_data['total_persen_baca_aws']*100);
												$persen_aws = number_format($persen_aws,2,'.',',').'%';
											}
											...
											if ( $row_data['total_persen_baca_cyber']==0)
											{
												...
												$t_persen_cyber = ($row_data['persen_baca_cyber']+$row_data['persen_baca_aws']);
												$t_persen_cyber2 =($row_data['total_persen_baca_cyber']+$row_data['total_persen_baca_aws']);
												$row_data['persen_baca_cyber'] = $t_persen_cyber;
												$row_data['total_persen_baca_cyber'] = $t_persen_cyber2;
												
												$persen_cyber = ( $t_persen_cyber/$t_persen_cyber2 *100);
												$persen_cyber = number_format($persen_cyber,2,'.',',').'%';
											}
											...
											$row_data['persen_baca_aws'] = 0;
											$row_data['total_persen_baca_aws'] = 0;
										}
										
										
										?>
										<td align="right" bgcolor="#DDBBBB" style="font-size:8pt;">&nbsp;&nbsp;<?php echo $persen_cyber;?>&nbsp;&nbsp;<br><?php echo "(".$row_data['persen_baca_cyber']."/".$row_data['total_persen_baca_cyber'].")";?></td>
										<td align="right" bgcolor="#DDBBBB" style="font-size:8pt;">&nbsp;&nbsp;<?php echo $persen_aws?>&nbsp;&nbsp;<br><?php echo "(".$row_data['persen_baca_aws']."/".$row_data['total_persen_baca_aws'].")";?></td>
										
										<td align="right" bgcolor="#DDBBBB"><a onClick="window.open('script_summary_detail_rev.php?act=show_terdeteksi_suppression&pr=<?=$flagtrans?>&blth=<?=$blth?>&m_loading_id=<?php echo $m_loading_id?><?php if (!empty($_GET['periode_kirim'])){echo '&periode_kirim='.$_GET['periode_kirim'];}?>')"><?php echo number_format($row_data['terdeteksi_suppression_list'],0,'',',')?></a>&nbsp;&nbsp;</td>
										<td align="right" bgcolor="#DDBBBB"><a onClick="window.open('script_summary_detail_rev.php?act=show_terdeteksi_spam&pr=<?=$flagtrans?>&blth=<?=$blth?>&m_loading_id=<?php echo $m_loading_id?><?php if (!empty($_GET['periode_kirim'])){echo '&periode_kirim='.$_GET['periode_kirim'];}?>')"><?php echo number_format($row_data['terdeteksi_spam'],0,'',',')?></a>&nbsp;&nbsp;</td>
										<td align="right" bgcolor="#DDBBBB"><a onClick="window.open('script_summary_detail_rev.php?act=show_terdeteksi_delay&pr=<?=$flagtrans?>&blth=<?=$blth?>&m_loading_id=<?php echo $m_loading_id?><?php if (!empty($_GET['periode_kirim'])){echo '&periode_kirim='.$_GET['periode_kirim'];}?>')"><?php echo number_format($row_data['terdeteksi_delay'],0,'',',')?></a>&nbsp;&nbsp;</td>
									...
									<?php
										...
										$total_semua_2m = $total_semua_2m + $row_data['lebih_2m'];
										...
										$total_e_terdeteksi_suppression_list = $total_e_terdeteksi_suppression_list + $row_data['terdeteksi_suppression_list'];
										$total_e_terdeteksi_spam = $total_e_terdeteksi_spam + $row_data['terdeteksi_spam'];
										$total_e_terdeteksi_delay = $total_e_terdeteksi_delay + $row_data['terdeteksi_delay'];
										$total_s_sukses = $total_s_sukses + $row_data['sample_sukses'];
										$total_s_gagal = $total_s_gagal + $row_data['sample_gagal'];
										
										$total_selisih = $total_selisih + $xyz;
									}
								?>
                                <tr height="30" style="font-weight:bold" bgcolor="#87ceeb">
                                	<td align="center" colspan="13">T O T A L</td>
									...
									<!-- TABLE EMAIL TERBACA -->
									<td align="right" bgcolor="#CC9999" valign="middle" style="font-size:8pt;background-color:#fff0f5;" >
									...
									<td align="right" bgcolor="#CC9999" valign="middle" style="font-size:8pt;background-color:#fff0f5;" >
										<?php 
										if ( empty($total_e_terbaca))
										{
											echo '0%';
										}else{
											echo number_format((($total_e_terbaca/$total_email)*100),2)."%";
										}
									...
									<td align="right" bgcolor="#CC9999">&nbsp;&nbsp;-</td>
									<td align="right" bgcolor="#CC9999">&nbsp;&nbsp;-</td>
									
									
									<td align="right" bgcolor="#CC9999"><a onClick="window.open('script_summary_detail_rev.php?act=show_terdeteksi_suppression&pr&pr=<?=$flagtrans?>&blth=<? echo $blth?><?php if (!empty($_GET['periode_kirim'])){echo '&periode_kirim='.$_GET['periode_kirim'];}?>&m_loading_id=<?php echo trim($m_loading_id_all,'|')?>')"><?php echo number_format($total_e_terdeteksi_suppression_list,0,'',',')?></a>&nbsp;&nbsp;</td>
									<td align="right" bgcolor="#CC9999"><a onClick="window.open('script_summary_detail_rev.php?act=show_terdeteksi_spam&pr&pr=<?=$flagtrans?>&blth=<? echo $blth?><?php if (!empty($_GET['periode_kirim'])){echo '&periode_kirim='.$_GET['periode_kirim'];}?>&m_loading_id=<?php echo trim($m_loading_id_all,'|')?>')"><?php echo number_format($total_e_terdeteksi_spam,0,'',',')?></a>&nbsp;&nbsp;</td>
									<td align="right" bgcolor="#CC9999"><a onClick="window.open('script_summary_detail_rev.php?act=show_terdeteksi_delay&pr&pr=<?=$flagtrans?>&blth=<? echo $blth?><?php if (!empty($_GET['periode_kirim'])){echo '&periode_kirim='.$_GET['periode_kirim'];}?>&m_loading_id=<?php echo trim($m_loading_id_all,'|')?>')"><?php echo number_format($total_e_terdeteksi_delay,0,'',',')?></a>&nbsp;&nbsp;</td>
...
$objWriter->save('php://output');
                        	...
                        	<table align="center" border="1" cellpadding="2" cellspacing="0" width="100%" style="border-collapse:collapse;border-color:#000000">
								...
								<?php
									...
									*/
									...
									FROM ".$GLOBALS['tr_email_sk_br']." te
	...
	function show_list_antrian($header_utama){
		...
		$condv_nama = "WHERE m_loading_id in (".str_replace('|',',', $m_loading_id).")";
		$group_order ="GROUP BY d.nama_file, a.loading_id, tgl_antrian, d.nomor_rekening, a.nama, a.email, a.pdf_name, b.tgl_jadwal, a.error_info 
		ORDER BY a.loading_id, tgl_antrian DESC";
		}else{
		$add_select ="";
		$condv = "a.loading_id = $m_loading_id";
		$condv_nama = "WHERE m_loading_id = $m_loading_id";
		$group_order ="GROUP BY tgl_antrian, d.nomor_rekening, a.nama, a.email, a.pdf_name, b.tgl_jadwal, a.error_info 
                    	...
                    	<td>
                        	<table align="center" border="1" cellpadding="2" cellspacing="0" width="100%">
                            	<tr height="30" bgcolor="#FF0000" style="color:#FFF">
									...
									<td align="center"><b>Status Reject</b></td>
                                </tr>
                                <?php
								...
								$sql_datas = "
									SELECT ".$add_select." to_char(tgl_antrian,'DD-Mon-YYYY HH24:MI:SS')as tgl_antrian, d.nomor_rekening, a.nama, a.email, a.pdf_name, a.error_info,
										CASE
                                ...
                                <tr height="30">
									...
									<td align="center"><?php echo $row_data['error_info']?></td>
                                </tr>
                                <?php
									}
								?>
	...
	//** 18 Des 2013 **//
		...
		if($sum_sql_datas > 20000){
			...
			$sqlyy = "SELECT loading_file,blth FROM m_loading WHERE m_loading_id=$m_loading_id";
			$qryyy = pg_query($sqlyy) or die('ERROR query: '.$sqlyy);
			while($rowyy = pg_fetch_array($qryyy)){
				$loading_file = $rowyy['loading_file'];
			}
			// create pembuatan csv
			
			$dest = '../tmp/csv/';
			$name_file = 'list_email_sukses_'.$m_loading_id.'_'.$loading_file . '_part'.'.csv';
			//die($dest.$name_file);
			if(file_exists($dest.$name_file)){ // check if file already
				unlink($dest.$name_file);
			}
	...
	*/
	...
	function show_terdeteksi_suppression($header_utama){
	$SIZE_BERBAYAR = SIZE_BERBAYAR;
		$m_loading_id = $_REQUEST['m_loading_id'];
		$blth = $_REQUEST['blth'];
		$flagtrans = $_REQUEST['pr'];
        ...
        <html xmlns="http://www.w3.org/1999/xhtml">
            <head>
                <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
                <title>LIST EMAIL GAGAL YANG TERDETEKSI SUPPRESSION LIST</title>
            </head>
            <? require_once("../include/script.php"); ?>
            <body>
            	<table align="center" border="0" width="1000">
                	<tr>
                    	<td>
                        	<table align="center" border="0" cellpadding="2" cellspacing="0" width="100%">
                            	<tr height="30">
                                	<td class="title" colspan="5" align="center" valign="middle">
                                    	LIST EMAIL GAGAL YANG TERDETEKSI SUPPRESSION LIST
                                    </td>
                                </tr>
                                <tr height="20">
                                	<td colspan="2"><b><u><?=$header_utama?></u></b></td>
                    	...
                    	<td>
                        	<table align="center" border="1" cellpadding="2" cellspacing="0" width="100%" style="border-collapse:collapse;border-color:#000000;">
								...
								$sql_datas = "SELECT
									...
									LEFT JOIN antrian_email_attach_file aeaf ON te.antrian_id = aeaf.antrian_id
									LEFT JOIN m_attach_file maf ON aeaf.m_attach_file_id::integer = maf.m_attach_file_id::integer
									$condv AND te.status_sample is not true AND te.tipe_gagal like '%Suppression%' 
				 ...
				 <?php
                      if($_REQUEST['xls']<>'y'){  
					  echo pagings($offset,$jumlah_data,$limit,'div_paging','script_summary_detail_rev.php','act=show_terdeteksi_spam&blth='.$blth.'&pr='.$flagtrans.'&m_loading_id='.$m_loading_id.'&tipe='.$tipe);
					  }
                    ?>
					</div>
					</td>
	...
	function show_terdeteksi_spam($header_utama){
	$SIZE_BERBAYAR = SIZE_BERBAYAR;
		$m_loading_id = $_REQUEST['m_loading_id'];
		$blth = $_REQUEST['blth'];
		$flagtrans = $_REQUEST['pr'];
        ...
        <html xmlns="http://www.w3.org/1999/xhtml">
            <head>
                <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
                <title>LIST EMAIL GAGAL YANG TERDETEKSI SPAM</title>
            </head>
            <? require_once("../include/script.php"); ?>
            <body>
            	<table align="center" border="0" width="1000">
                	<tr>
                    	<td>
                        	<table align="center" border="0" cellpadding="2" cellspacing="0" width="100%">
                            	<tr height="30">
                                	<td class="title" colspan="5" align="center" valign="middle">
                                    	LIST EMAIL GAGAL YANG TERDETEKSI SPAM
                                    </td>
                                </tr>
                                <tr height="20">
                                	<td colspan="2"><b><u><?=$header_utama?></u></b></td>
                    	...
                    	<td>
                        	<table align="center" border="1" cellpadding="2" cellspacing="0" width="100%" style="border-collapse:collapse;border-color:#000000;">
								...
								$sql_datas = "SELECT
									...
									LEFT JOIN antrian_email_attach_file aeaf ON te.antrian_id = aeaf.antrian_id
									LEFT JOIN m_attach_file maf ON aeaf.m_attach_file_id::integer = maf.m_attach_file_id::integer
									$condv AND te.status_sample is not true AND te.tipe_gagal like '%spam%' 
									AND te.ket_error IS NOT NULL 
									GROUP BY te.antrian_id, te.nomor_rekening, te.email, te.date_email_send,
									 d.nama, d.size_pdf, te.date_email_callback, te.ket_error, te.nama_customer, 
									 ml.loading_file
	...
	function show_terdeteksi_delay($header_utama){
	$SIZE_BERBAYAR = SIZE_BERBAYAR;
		$m_loading_id = $_REQUEST['m_loading_id'];
		$blth = $_REQUEST['blth'];
		$flagtrans = $_REQUEST['pr'];
        ...
        <html xmlns="http://www.w3.org/1999/xhtml">
            <head>
                <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
                <title>LIST EMAIL GAGAL YANG TERDETEKSI DELAY</title>
            </head>
            <? require_once("../include/script.php"); ?>
            <body>
            	<table align="center" border="0" width="1000">
                	<tr>
                    	<td>
                        	<table align="center" border="0" cellpadding="2" cellspacing="0" width="100%">
                            	<tr height="30">
                                	<td class="title" colspan="5" align="center" valign="middle">
                                    	LIST EMAIL GAGAL YANG TERDETEKSI DELAY
                                    </td>
                                </tr>
                                <tr height="20">
                                	<td colspan="2"><b><u><?=$header_utama?></u></b></td>
                    	...
                    	<td>
                        	<table align="center" border="1" cellpadding="2" cellspacing="0" width="100%" style="border-collapse:collapse;border-color:#000000;">
                            	<tr height="30" bgcolor="#FF0000" style="color:#FFF">
                                	...
                                	<td align="center" rowspan="2"><b>Tgl Kirim</b></td>
                                	<td align="center" rowspan="2"><b>Tgl Feedback</b></td>
                                	<td align="center" rowspan="2"><b>Pesan Delay</b></td>
                                	<td align="center" rowspan="2"><b>Klasifikasi Error</b></td>
									<td align="center" colspan="4" bgcolor="#cd853f"><b>Size (KB)</b></td>
                                	<td align="center"><b>Template</b></td>
                                </tr>
								...
								$sql_datas = "SELECT
									 te.antrian_id, te.nomor_rekening, te.email, te.date_email_send, te.date_email_callback, te.delay_info as ket_error,
									 d.nama, d.size_pdf, te.nama_customer, 
									 ml.loading_file,
									 SUM(maf.ukuran)AS total_attach
									FROM ".$GLOBALS['tr_email_sk_br']." te
									...
									LEFT JOIN antrian_email_attach_file aeaf ON te.antrian_id = aeaf.antrian_id
									LEFT JOIN m_attach_file maf ON aeaf.m_attach_file_id::integer = maf.m_attach_file_id::integer
									$condv AND te.status_sample is not true AND te.tgl_read IS NULL AND te.ket_error IS NULL
									AND te.delay_info IS NOT NULL 
									GROUP BY te.antrian_id, te.nomor_rekening, te.email, te.date_email_send,
									 d.nama, d.size_pdf, te.date_email_callback, te.delay_info, te.nama_customer, 
									 ml.loading_file
									ORDER BY te.date_email_send";
                                ...
                                <tr height="30">
                                	...
                                	<td align="left">
										...
										<?php 
											...
											$xyz1 = explode(' //',$row_data['ket_error'] );
											foreach	( $xyz1 as $key9 => $value9)
											{
												if (empty($value9)) continue;
												echo "<li>".$value9."</li>";
											}
											//echo $row_data['ket_error'];
										?>
											</ul>
				 ...
				 <?php
                      if($_REQUEST['xls']<>'y'){  
					  echo pagings($offset,$jumlah_data,$limit,'div_paging','script_summary_detail_rev.php','act=show_terdeteksi_delay&blth='.$blth.'&pr='.$flagtrans.'&m_loading_id='.$m_loading_id.'&tipe='.$tipe);
					  }
                    ?>
					</div>
					</td>
                    ...
                    <tr>
                    	<td>
                        	<table align="center" border="1" cellpadding="2" cellspacing="0" width="100%" style="border-collapse:collapse">
								...
								FROM bounce_inbox a 
								LEFT JOIN ".$GLOBALS['tr_email_sk_br']." b ON b.tr_email_id::integer=a.tr_email_id::integer WHERE a.blth='$blth' 
								$condv 
	...
	function load_template(){
		...
		$blth_balik = substr($blth,-4).substr($blth,0,2);
			...
			$tr_email_x = "tr_email_".$blth.'_'.$flagtrans;
	...
	/** FUNGSI MEMBUKA TEMPLATE EMAIL TERBACA **/
		...
		$sql_data = "SELECT body_email_read FROM ".$GLOBALS['tr_email_sk_br']." WHERE tr_email_id = $tr_email_id";
		$qry_data = pg_query($sql_data) or die('ERROR select summary detail: '.$sql_data);
