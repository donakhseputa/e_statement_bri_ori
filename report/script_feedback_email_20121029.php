		...
		if($report_menu==1){
			...
			if($arr_respon[0]=='connect'){
				...
				if($tipe_report==2){
					...
					header("Content-Disposition: attachment; filename=".$blth."_emailSukses_".$nama_file_.".xls");
					header("Expires: 0");
					header("Cache-Control: must-revalidate, post-check=0, pre-check=0");
					header("Pragma: public");
					$dlxls="";
				}else{
					$dlxls='<a href="#" onClick="window.open(\'script_feedback_email.php?act=show_report&flagtrans=BC&blth='.$blth.'&nama_file='.$nama_file.'&report_menu=1&tipe_report=2\');">DOWNLOAD</a> (XLS)';
				}
						...
						<td>
							...
							<table width="1000" align="center" border="1" cellpadding="2" cellspacing="0">
								<tr bgcolor="#FF0000" style="color:#FFF">
									...
									<td align="center" valign="middle" nowrap><strong>Nomor Rekening</strong></td>
									<td align="center" valign="middle" nowrap><strong>Nama Customer</strong></td>
									<td align="center" valign="middle" nowrap><strong>Alamat e-mail</strong></td>
									<td align="center" valign="middle" nowrap><strong>Tanggal Kirim</strong></td>
									<?php 
										if($tipe_report==1)
			...
			if($arr_respon[0]=='connect'){
				/*$sql_sel_tr_email = "SELECT a.email, to_char(a.date_email_send,'DD-Mon-YYYY HH24:MI:SS') as date_email_send, a.ket_error,
									...
									WHERE a.loading_id = b.m_loading_id
										...
										and a.nomor_rekening = b.nomor_rekening";*/
				$sql_sel_tr_email = "SELECT a.email, to_char(a.date_email_send,'DD-Mon-YYYY HH24:MI:SS') as date_email_send, a.ket_error,
											a.nama_customer AS nama, a.nomor_rekening, b.nama_file, a.antrian_id
									FROM tr_email a, detail b
									WHERE a.loading_id = b.m_loading_id
										and b.blth = '$blth'
										and b.flagtrans = '$flagtrans'
				...
				$sql_sel_tr_email .= " GROUP BY  
					a.email, date_email_send, a.ket_error, a.nama_customer, a.nomor_rekening, b.nama_file, a.antrian_id 
				ORDER BY b.nama_file, date_email_send, a.nomor_rekening ASC";
				$qry_sel_tr_email = pg_query($sql_sel_tr_email) or die('ERROR select tr_email: '.$sql_sel_tr_email);
				
				//echo $sql_sel_tr_email."<br>";
				if($tipe_report==2){
					header("Content-type: application/vnd.ms-excel");
					header("Content-Disposition: attachment; filename=".$blth."_emailGagal_".$nama_file_.".xls");
					header("Expires: 0");
					header("Cache-Control: must-revalidate, post-check=0, pre-check=0");
					header("Pragma: public");
					$dlxls="";
					...
					<tr>
						<td>
							<table width="1000" align="center" border="0" cellpadding="0" cellspacing="0">
								...
								<tr>
									<td height="20" colspan="4">PERIODE: <?=$blth?></td>
									<td height="20" colspan="3" align="right"><?=$dlxls?></td>
								</tr>
							</table>
							<table width="1000" align="center" border="1" cellpadding="2" cellspacing="0">
								<tr bgcolor="#FF0000" style="color:#FFF">
									...
									<td align="center" valign="middle" nowrap><strong>Alamat e-mail</strong></td>
									<td align="center" valign="middle" nowrap><strong>Tanggal Kirim</strong></td>
									<td align="center" valign="middle" nowrap><strong>Pesan Error</strong></td>
									<?php 
										if($tipe_report==1)
											echo "<td align='center' valign='middle'><strong>Template</strong></td>";
									?>
			...
			if($arr_respon[0]=='connect'){
				$sql_sel_tr_email = "SELECT a.email, to_char(a.date_email_send,'DD-Mon-YYYY HH24:MI:SS') as date_email_send,
											a.nama_customer AS nama, a.nomor_rekening, b.nama_file, a.email_callback, a.antrian_id, a.ket_error
									FROM tr_email a, detail b
									WHERE a.loading_id = b.m_loading_id
										and b.blth = '$blth'
										and b.flagtrans = '$flagtrans'
				...
				$sql_sel_tr_email .= " GROUP BY  
					a.email, date_email_send, a.nama_customer, a.nomor_rekening, b.nama_file, a.email_callback, a.antrian_id, a.ket_error 
				ORDER BY b.nama_file, a.nomor_rekening, date_email_send ASC";
				$qry_sel_tr_email = pg_query($sql_sel_tr_email) or die('ERROR select tr_email: '.$sql_sel_tr_email);
				
				if($tipe_report==2){
					header("Content-type: application/vnd.ms-excel");
					header("Content-Disposition: attachment; filename=".$blth."_emailReport_".$nama_file_.".xls");
					header("Expires: 0");
					header("Cache-Control: must-revalidate, post-check=0, pre-check=0");
					header("Pragma: public");
					$dlxls="";
				}else{
					$dlxls='<a href="#" onClick="window.open(\'script_feedback_email.php?act=show_report&flagtrans=BC&blth='.$blth.'&nama_file='.$nama_file.'&report_menu=4&tipe_report=2\');">DOWNLOAD</a> (XLS)';//echo "x: ".$sql_sel_tr_email."<br><br>";
				}
			...
			if($arr_respon[0]=='connect'){
				...
				$sql_sel_tr_email = "SELECT b.nama, b.nomor_rekening, b.nama_file, b.pdf_name, b.jml_hlm, $jmlkr AS jml_kirim
				...
				$sql_sel_tr_email .= " GROUP BY  
					b.nama, b.nomor_rekening, b.nama_file, b.pdf_name, b.jml_hlm
				ORDER BY b.nama_file, b.nomor_rekening ASC";
				$qry_sel_tr_email = pg_query($sql_sel_tr_email) or die('ERROR select tr_email: '.$sql_sel_tr_email);
				
				if($tipe_report==2){
					header("Content-type: application/vnd.ms-excel");
					header("Content-Disposition: attachment; filename=".$blth."_pdfList_".$nama_file_.".xls");
					header("Expires: 0");
					header("Cache-Control: must-revalidate, post-check=0, pre-check=0");
					header("Pragma: public");
					$dlxls="";
						...
						<td>
							...
							<table width="1000" align="center" border="1" cellpadding="2" cellspacing="0">
								<tr bgcolor="#FF0000" style="color:#FFF">
									...
									<td align="center" valign="middle" nowrap><strong>Nama PDF</strong></td>
									<td align="center" valign="middle" nowrap><strong>Halaman</strong></td>
									<td align="center" valign="middle" nowrap><strong>Jumlah Terkirim</strong></td>
								</tr>
								<?php
								$totkirim=0;
								$tothlm=0;
								while($row_sel_tr_email=pg_fetch_array($qry_sel_tr_email)){
									$no++;$totkirim=$totkirim+$row_sel_tr_email['jml_kirim'];$tothlm=$tothlm+$row_sel_tr_email['jml_hlm'];
									...
									<tr>
										...
										<td align='right' nowrap><?=$row_sel_tr_email['jml_hlm']?></td>
										<td align='right' nowrap><?=$row_sel_tr_email['jml_kirim']?></td>
									</tr>
									<?php
								}
								echo "<tr><td></td><td colspan=4><b>TOTAL</b></td><td align=right>$tothlm</td></td><td align=right>$totkirim</td></tr>";
								?>
							</table>
						</td>
					</tr>
			...
			if($arr_respon[0]=='connect'){
				...
				if($tipe_report==2){
					header("Content-type: application/vnd.ms-excel");
					header("Content-Disposition: attachment; filename=".$blth."_emailBounce.xls");
					header("Expires: 0");
					header("Cache-Control: must-revalidate, post-check=0, pre-check=0");
					header("Pragma: public");
				}
				...
				<table align="center">
					<tr>
						<td>
							<table width="1000" align="center" border="0" cellpadding="0" cellspacing="0">
								<tr>
									<td align="center" valign="middle" class="title" colspan="6"><strong>REPORT - LAPORAN BOUNCE EMAIL</strong></td>
								</tr>
								<tr>
